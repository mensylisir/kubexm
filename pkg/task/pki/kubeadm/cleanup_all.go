...
func (t *CleanupTask) Plan(ctx runtime.TaskContext) (*plan.ExecutionFragment, error) {
	...
	localCleanupStep := kubeadmstep.NewKubeadmLocalCleanupStepBuilder(*runtimeCtx, "CleanupLocalWorkspace").Build()
	remoteCleanupStep := kubeadmstep.NewKubeadmRemoteCleanupStepBuilder(*runtimeCtx, "CleanupRemoteBackups").Build()
