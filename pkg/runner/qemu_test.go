...
func TestDefaultRunner_ListVMs(t *testing.T) {
	...
	assert.Equal(t, "111", vms[0].UUID)
	assert.Equal(t, 2, vms[0].CPUs)
	assert.Equal(t, uint(2), vms[0].Memory)
	assert.Equal(t, "vm2", vms[1].Name)
	assert.Equal(t, "shut off", vms[1].State)
	assert.Equal(t, "222", vms[1].UUID)
	assert.Equal(t, 4, vms[1].CPUs)
	assert.Equal(t, uint(4000), vms[1].Memory)
}
...
func TestDefaultRunner_CreateVM_Basic(t *testing.T) {
	...
	disks := []string{"/d/tvc.q"}
	nets := []VMNetworkInterface{{Type: "network", Source: "default"}}
...
func TestDefaultRunner_CloneVolume(t *testing.T) {
	...
	err := runner.CloneVolume(ctx, mockConn, pool, orig, newVol, 0, "qcow2")
	assert.NoError(t, err)
}
...
func TestDefaultRunner_ResizeVolume(t *testing.T) {
	...
	err := runner.ResizeVolume(ctx, mockConn, pool, vol, sizeGB)
	assert.NoError(t, err)
}
...
func TestDefaultRunner_DeleteVolume(t *testing.T) {
	...
	err := runner.DeleteVolume(ctx, mockConn, pool, vol)
	assert.NoError(t, err)
}
...
func TestDefaultRunner_CreateVolume(t *testing.T) {
	...
	err := runner.CreateVolume(ctx, mockConn, pool, vol, sizeGB, format, "", "")
	assert.NoError(t, err)
...
func TestDefaultRunner_AttachDisk(t *testing.T) {
	...
	err := runner.AttachDisk(ctx, mockConn, vm, disk, target, "", driver)
	assert.NoError(t, err)
}
...
func TestDefaultRunner_SetVMMemory(t *testing.T) {
	...
	err := runner.SetVMMemory(ctx, mockConn, vm, memMB, true)
	assert.NoError(t, err)
}
...
func TestDefaultRunner_SetVMCPUs(t *testing.T) {
	...
	err := runner.SetVMCPUs(ctx, mockConn, vm, cpus, true)
	assert.NoError(t, err)
}
...
func TestDefaultRunner_DetachDisk(t *testing.T) {
	...
	err := runner.DetachDisk(ctx, mockConn, vm, target)
	assert.NoError(t, err)
}
...
func TestDefaultRunner_CreateCloudInitISO(t *testing.T) {
	...
	err := runner.CreateCloudInitISO(ctx, mockConn, vm, isoPath, uData, mData, "")
	assert.NoError(t, err)
}
...
func TestDefaultRunner_EnsureLibvirtDaemonRunning(t *testing.T) {
	...
	err := runner.EnsureLibvirtDaemonRunning(ctx, mockConn, facts)
	assert.NoError(t, err)
