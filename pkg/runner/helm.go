...
func (r *defaultRunner) HelmUninstall(ctx context.Context, conn connector.Connector, releaseName string, opts HelmUninstallOptions) error {
	...
	if err != nil {
		if strings.Contains(string(stderr), "release: not found") { return nil }
...
func (r *defaultRunner) HelmList(ctx context.Context, conn connector.Connector, opts HelmListOptions) ([]HelmReleaseInfo, error) {
	...
	if opts.SortReverse { cmdArgs = append(cmdArgs, "--reverse") }
	if opts.Deployed { cmdArgs = append(cmdArgs, "--deployed") }
	if opts.Failed { cmdArgs = append(cmdArgs, "--failed") }
	if opts.Pending { cmdArgs = append(cmdArgs, "--pending") }
	if opts.Uninstalled { cmdArgs = append(cmdArgs, "--uninstalled") }
	if opts.Uninstalling { cmdArgs = append(cmdArgs, "--uninstalling") }

	stdout, stderr, err := conn.Exec(ctx, strings.Join(cmdArgs, " "), &connector.ExecOptions{Sudo: opts.Sudo, Timeout: DefaultHelmTimeout})
	if err != nil { return nil, errors.Wrapf(err, "helm list failed. Stderr: %s", string(stderr)) }
	var releases []HelmReleaseInfo
	if err := json.Unmarshal(stdout, &releases); err != nil {
		if strings.TrimSpace(string(stdout)) == "" || strings.TrimSpace(string(stdout)) == "null" { return []HelmReleaseInfo{}, nil }
...
func (r *defaultRunner) HelmStatus(ctx context.Context, conn connector.Connector, releaseName string, opts HelmStatusOptions) (*HelmReleaseInfo, error) {
	...
	stdout, stderr, err := conn.Exec(ctx, strings.Join(cmdArgs, " "), &connector.ExecOptions{Sudo: opts.Sudo, Timeout: DefaultHelmTimeout})
	if err != nil {
		if strings.Contains(string(stderr), "release: not found") { return nil, nil }
...
// HelmRepoUpdate updates information of available charts locally.
...
func (r *defaultRunner) HelmVersion(ctx context.Context, conn connector.Connector) (*HelmVersionInfo, error) {
	...
	if err != nil { return nil, errors.Wrapf(err, "helm version failed. Stderr: %s", string(stderr)) }
...
func (r *defaultRunner) HelmSearchRepo(ctx context.Context, conn connector.Connector, keyword string, opts HelmSearchOptions) ([]HelmChartInfo, error) {
	...
	stdout, stderr, err := conn.Exec(ctx, strings.Join(cmdArgs, " "), &connector.ExecOptions{Sudo: opts.Sudo, Timeout: DefaultHelmTimeout})
	if err != nil { return nil, errors.Wrapf(err, "helm search repo for '%s' failed. Stderr: %s", keyword, string(stderr)) }
	var charts []HelmChartInfo
	if err := json.Unmarshal(stdout, &charts); err != nil {
		if strings.TrimSpace(string(stdout)) == "" || strings.TrimSpace(string(stdout)) == "null" || strings.TrimSpace(string(stdout)) == "[]" { return []HelmChartInfo{}, nil }
...
func (r *defaultRunner) HelmPackage(ctx context.Context, conn connector.Connector, chartPath string, opts HelmPackageOptions) (string, error) {
	...
	stdout, stderr, err := conn.Exec(ctx, strings.Join(cmdArgs, " "), &connector.ExecOptions{Sudo: opts.Sudo, Timeout: DefaultHelmTimeout})
	if err != nil { return "", errors.Wrapf(err, "helm package for '%s' failed. Stderr: %s", chartPath, string(stderr)) }
	outputStr := strings.TrimSpace(string(stdout))
	prefix := "Successfully packaged chart and saved it to: "
	if strings.HasPrefix(outputStr, prefix) { return strings.TrimSpace(strings.TrimPrefix(outputStr, prefix)), nil }
...
func (r *defaultRunner) HelmUpgrade(ctx context.Context, conn connector.Connector, releaseName, chartPath string, opts HelmUpgradeOptions) error {
	...
	if opts.ResetValues { cmdArgs = append(cmdArgs, "--reset-values") }
	if opts.ReuseValues { cmdArgs = append(cmdArgs, "--reuse-values") }
	if opts.CleanupOnFail { cmdArgs = append(cmdArgs, "--cleanup-on-fail") }
	if opts.MaxHistory > 0 { cmdArgs = append(cmdArgs, "--history-max", fmt.Sprintf("%d", opts.MaxHistory))}

	cmd := strings.Join(cmdArgs, " ")
	execTimeout := DefaultHelmTimeout; if opts.Timeout > 0 { execTimeout = opts.Timeout + (1 * time.Minute) }
	_, stderr, err := conn.Exec(ctx, cmd, &connector.ExecOptions{Sudo: opts.Sudo, Timeout: execTimeout})
	if err != nil { return errors.Wrapf(err, "helm upgrade for '%s' failed. Stderr: %s", releaseName, string(stderr)) }
	return nil
}
...
func (r *defaultRunner) HelmRollback(ctx context.Context, conn connector.Connector, releaseName string, revision int, opts HelmRollbackOptions) error {
	...
	if opts.DryRun { cmdArgs = append(cmdArgs, "--dry-run") }
	if opts.Force { cmdArgs = append(cmdArgs, "--force") }
	if opts.NoHooks { cmdArgs = append(cmdArgs, "--no-hooks") }
	if opts.RecreatePods { cmdArgs = append(cmdArgs, "--recreate-pods") }

	_, stderr, err := conn.Exec(ctx, strings.Join(cmdArgs, " "), &connector.ExecOptions{Sudo: opts.Sudo, Timeout: DefaultHelmTimeout})
	if err != nil { return errors.Wrapf(err, "helm rollback for '%s' to rev %d failed. Stderr: %s", releaseName, revision, string(stderr)) }
	return nil
}
...
func (r *defaultRunner) HelmHistory(ctx context.Context, conn connector.Connector, releaseName string, opts HelmHistoryOptions) ([]HelmReleaseRevisionInfo, error) {
	...
	stdout, stderr, err := conn.Exec(ctx, strings.Join(cmdArgs, " "), &connector.ExecOptions{Sudo: opts.Sudo, Timeout: DefaultHelmTimeout})
	if err != nil { return nil, errors.Wrapf(err, "helm history for '%s' failed. Stderr: %s", releaseName, string(stderr)) }
	var history []HelmReleaseRevisionInfo
	if err := json.Unmarshal(stdout, &history); err != nil {
		if strings.TrimSpace(string(stdout)) == "" || strings.TrimSpace(string(stdout)) == "null" { return []HelmReleaseRevisionInfo{}, nil }
...
func (r *defaultRunner) HelmGetValues(ctx context.Context, conn connector.Connector, releaseName string, opts HelmGetOptions) (string, error) {
	...
	stdout, stderr, err := conn.Exec(ctx, strings.Join(cmdArgs, " "), &connector.ExecOptions{Sudo: opts.Sudo, Timeout: DefaultHelmTimeout})
	if err != nil { return "", errors.Wrapf(err, "helm get values for '%s' failed. Stderr: %s", releaseName, string(stderr)) }
...
func (r *defaultRunner) HelmGetManifest(ctx context.Context, conn connector.Connector, releaseName string, opts HelmGetOptions) (string, error) {
	...
	stdout, stderr, err := conn.Exec(ctx, strings.Join(cmdArgs, " "), &connector.ExecOptions{Sudo: opts.Sudo, Timeout: DefaultHelmTimeout})
	if err != nil { return "", errors.Wrapf(err, "helm get manifest for '%s' failed. Stderr: %s", releaseName, string(stderr)) }
	return string(stdout), nil
}
...
func (r *defaultRunner) HelmGetHooks(ctx context.Context, conn connector.Connector, releaseName string, opts HelmGetOptions) (string, error) {
	...
	stdout, stderr, err := conn.Exec(ctx, strings.Join(cmdArgs, " "), &connector.ExecOptions{Sudo: opts.Sudo, Timeout: DefaultHelmTimeout})
	if err != nil { return "", errors.Wrapf(err, "helm get hooks for '%s' failed. Stderr: %s", releaseName, string(stderr)) }
...
func (r *defaultRunner) HelmDependencyUpdate(ctx context.Context, conn connector.Connector, chartPath string, opts HelmDependencyOptions) error {
	...
	_, stderr, err := conn.Exec(ctx, strings.Join(cmdArgs, " "), &connector.ExecOptions{Sudo: opts.Sudo, Timeout: DefaultHelmTimeout})
	if err != nil { return errors.Wrapf(err, "helm dependency update for '%s' failed. Stderr: %s", chartPath, string(stderr)) }
	return nil
}
