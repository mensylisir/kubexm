...
func TestDefaultRunner_HelmList(t *testing.T) {
	...
	var expected []HelmReleaseInfo
	json.Unmarshal([]byte(sampleOutput), &expected)
...
func TestDefaultRunner_HelmStatus(t *testing.T) {
	...
	status, err := runner.HelmStatus(ctx, mockConn, releaseName, HelmStatusOptions{})
	assert.NoError(t, err)
...
func TestDefaultRunner_HelmRepoAdd(t *testing.T) {
	...
	runner := NewDefaultRunner()
	ctx := context.Background()
	repoName, repoURL := "bn", "https://charts.bitnami.com/bitnami"
	...
	err := runner.HelmRepoAdd(ctx, mockConn, repoName, repoURL, HelmRepoAddOptions{})
	assert.NoError(t, err)
}
...
func TestDefaultRunner_HelmVersion(t *testing.T) {
	...
	runner := NewDefaultRunner()
	ctx := context.Background()
	sampleOutput := `{"version":"v3.8.1"}`
	var expected HelmVersionInfo
	json.Unmarshal([]byte(sampleOutput), &expected)
	mockConn.EXPECT().Exec(ctx, "helm version -o json", gomock.Any()).Return([]byte(sampleOutput), []byte{}, nil).Times(1)
	verInfo, err := runner.HelmVersion(ctx, mockConn)
	assert.NoError(t, err)
	assert.Equal(t, &expected, verInfo)
}
...
func TestDefaultRunner_HelmSearchRepo(t *testing.T) {
	...
	ctx := context.Background()
	keyword := "ng"
	sampleOutput := `[{"name":"b/n","version":"1"}]`
	var expected []HelmChartInfo
	json.Unmarshal([]byte(sampleOutput), &expected)
	...
	charts, err := runner.HelmSearchRepo(ctx, mockConn, keyword, HelmSearchOptions{})
	assert.NoError(t, err)
...
func TestDefaultRunner_HelmPull(t *testing.T) {
	...
	outPath, err := runner.HelmPull(ctx, mockConn, chartPath, HelmPullOptions{})
	assert.NoError(t, err)
	assert.Equal(t, expectedOut, outPath)
}
...
func TestDefaultRunner_HelmPackage(t *testing.T) {
	...
	ctx := context.Background()
	chartSrcPath := "./mc"
	helmOut := "Successfully packaged chart and saved it to: /tmp/mc-0.1.0.tgz"
	expectedPath := "/tmp/mc-0.1.0.tgz"
	mockConn.EXPECT().Exec(ctx, fmt.Sprintf("helm package %s", shellEscape(chartSrcPath)), gomock.Any()).Return([]byte(helmOut+"\n"), []byte{}, nil).Times(1)
	pkgPath, err := runner.HelmPackage(ctx, mockConn, chartSrcPath, HelmPackageOptions{})
	assert.NoError(t, err)
	assert.Equal(t, expectedPath, pkgPath)
}
...
func TestDefaultRunner_HelmUpgrade(t *testing.T) {
	...
	err := runner.HelmUpgrade(ctx, mockConn, release, chart, opts)
	assert.NoError(t, err)
}
...
func TestDefaultRunner_HelmRollback(t *testing.T) {
	...
	err := runner.HelmRollback(ctx, mockConn, release, rev, opts)
	assert.NoError(t, err)
}
...
func TestDefaultRunner_HelmHistory(t *testing.T) {
	...
	var expected []HelmReleaseRevisionInfo
	json.Unmarshal([]byte(sampleOutput), &expected)
	...
	hist, err := runner.HelmHistory(ctx, mockConn, release, HelmHistoryOptions{})
	assert.NoError(t, err)
...
func TestDefaultRunner_HelmGetValues(t *testing.T) {
	...
	vals, err := runner.HelmGetValues(ctx, mockConn, release, HelmGetOptions{})
	assert.NoError(t, err)
	assert.Equal(t, expectedOutput, vals)
}
...
func TestDefaultRunner_HelmGetManifest(t *testing.T) {
	...
	man, err := runner.HelmGetManifest(ctx, mockConn, release, HelmGetOptions{})
	assert.NoError(t, err)
	assert.Equal(t, expectedOutput, man)
}
...
func TestDefaultRunner_HelmGetHooks(t *testing.T) {
	...
	hooks, err := runner.HelmGetHooks(ctx, mockConn, release, HelmGetOptions{})
	assert.NoError(t, err)
	assert.Equal(t, expectedOutput, hooks)
}
...
func TestDefaultRunner_HelmTemplate(t *testing.T) {
	...
	tpl, err := runner.HelmTemplate(ctx, mockConn, release, chart, opts)
	assert.NoError(t, err)
	assert.Equal(t, expectedOutput, tpl)
}
...
func TestDefaultRunner_HelmDependencyUpdate(t *testing.T) {
	...
	err := runner.HelmDependencyUpdate(ctx, mockConn, chartPath, HelmDependencyOptions{})
	assert.NoError(t, err)
}
...
func TestDefaultRunner_HelmLint(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, expectedOutput, lintOut)
