...
func TestDefaultRunner_KubectlGet(t *testing.T) {
	...
	out, err := runner.KubectlGet(ctx, mockConn, resType, resName, opts1)
...
func TestDefaultRunner_KubectlVersion(t *testing.T) {
	...
	var expected KubectlVersionInfo
	json.Unmarshal([]byte(sampleJSON), &expected)
	mockConn.EXPECT().Exec(ctx, "kubectl version -o json", gomock.Any()).Return([]byte(sampleJSON), nil, nil).Times(1)
	ver, err := runner.KubectlVersion(ctx, mockConn)
	assert.NoError(t, err)
...
func TestDefaultRunner_KubectlDescribe(t *testing.T) {
	...
	out, err := runner.KubectlDescribe(ctx, mockConn, resType, resName, opts)
	assert.NoError(t, err)
	assert.Equal(t, expectedOut, out)
}
...
func TestDefaultRunner_KubectlLogs(t *testing.T) {
	...
	logs, err := runner.KubectlLogs(ctx, mockConn, podName, opts)
	assert.NoError(t, err)
...
func TestDefaultRunner_KubectlExec(t *testing.T) {
	...
	out, err := runner.KubectlExec(ctx, mockConn, podName, container, cmdToRun, opts)
	assert.NoError(t, err)
	assert.Equal(t, expectedOut, out)
}
...
func TestDefaultRunner_KubectlGetNodes(t *testing.T) {
	...
	json.Unmarshal([]byte(sampleJSON), &struct {
...
func TestDefaultRunner_KubectlGetPods(t *testing.T) {
	...
	pods, err := runner.KubectlGetPods(ctx, mockConn, ns, KubectlGetOptions{})
	assert.NoError(t, err)
...
func TestDefaultRunner_KubectlGetServices(t *testing.T) {
	...
	svcs, err := runner.KubectlGetServices(ctx, mockConn, ns, KubectlGetOptions{})
	assert.NoError(t, err)
...
func TestDefaultRunner_KubectlGetDeployments(t *testing.T) {
	...
	deps, err := runner.KubectlGetDeployments(ctx, mockConn, ns, KubectlGetOptions{})
	assert.NoError(t, err)
...
func TestDefaultRunner_KubectlRolloutStatus(t *testing.T) {
	...
	out, err := runner.KubectlRolloutStatus(ctx, mockConn, resType, resName, opts)
	assert.NoError(t, err)
	assert.Equal(t, expectedOut, out)
}
...
func TestDefaultRunner_KubectlScale(t *testing.T) {
	...
	out, err := runner.KubectlScale(ctx, mockConn, resType, resName, replicas, opts)
	assert.NoError(t, err)
	assert.Equal(t, expectedOut, out)
}
...
func TestDefaultRunner_KubectlConfigView(t *testing.T) {
	...
	runner := NewDefaultRunner()
	ctx := context.Background()
	sampleJSON := `{"kind":"Config","current-context":"ctx1"}`
	var expected KubectlConfigInfo
	json.Unmarshal([]byte(sampleJSON), &expected)
	mockConn.EXPECT().Exec(ctx, "kubectl config view -o json", gomock.Any()).Return([]byte(sampleJSON), nil, nil).Times(1)
	cfg, err := runner.KubectlConfigView(ctx, mockConn, KubectlConfigViewOptions{})
	assert.NoError(t, err)
...
func TestDefaultRunner_KubectlConfigGetContexts(t *testing.T) {
	...
	mockConn.EXPECT().Exec(ctx, "kubectl config view -o json", gomock.Any()).Return([]byte(sampleJSON), nil, nil).Times(1)
	ctxs, err := runner.KubectlConfigGetContexts(ctx, mockConn)
	assert.NoError(t, err)
	assert.Len(t, ctxs, 1)
}
...
func TestDefaultRunner_KubectlTopNode(t *testing.T) {
	...
	nodes, err := runner.KubectlTopNodes(ctx, mockConn, KubectlTopOptions{})
	assert.NoError(t, err)
...
func TestDefaultRunner_KubectlTopPods(t *testing.T) {
	...
	pods, err := runner.KubectlTopPods(ctx, mockConn, KubectlTopOptions{Namespace: ns, Containers: true})
	assert.NoError(t, err)
	assert.Len(t, pods, 1)
...
func TestDefaultRunner_KubectlExplain(t *testing.T) {
	...
	out, err := runner.KubectlExplain(ctx, mockConn, resType, KubectlExplainOptions{})
	assert.NoError(t, err)
	assert.Equal(t, expectedOut, out)
}
...
func TestDefaultRunner_KubectlCompletion(t *testing.T) {
	...
	runner := NewDefaultRunner()
	ctx := context.Background()
	shell, expectedOut := "bash", "bash completion script"
	...
	out, err := runner.KubectlCompletion(ctx, mockConn, shell)
	assert.NoError(t, err)
	assert.Equal(t, expectedOut, out)
}
