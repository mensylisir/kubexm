...
func NewDownloadKubeApiServerStepBuilder(ctx runtime.Context, instanceName string) *DownloadKubeApiServerStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Download Kube ApiServer version %s", s.Base.Meta.Name, s.Version)
	s.Base.Sudo = false
...
func (b *DownloadKubeApiServerStepBuilder) WithURL(url string) *DownloadKubeApiServerStepBuilder {
	b.Step.URL = url
	return b
}
...
func (s *DownloadKubeApiServerStep) getBinaryInfo() (*helpers.BinaryInfo, error) {
...
func (s *DownloadKubeApiServerStep) verifyChecksum(filePath, expectedChecksum string) (bool, error) {
...
func (s *DownloadKubeApiServerStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to get kube-apiserver binary info: %w", err)
	}
...
func (s *DownloadKubeApiServerStep) Run(ctx runtime.ExecutionContext) error {
	...
	binaryInfo, err := s.getBinaryInfo()
	if err != nil {
		return fmt.Errorf("failed to get kube-apiserver binary info for run: %w", err)
	}
	...
	logger.Infof("Downloading kube-apiserver from %s ...", binaryInfo.URL)
