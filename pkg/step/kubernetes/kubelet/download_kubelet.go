...
func NewDownloadKubeletStepBuilder(ctx runtime.Context, instanceName string) *DownloadKubeletStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Download Kubelet version %s", s.Base.Meta.Name, s.Version)
	s.Base.Sudo = false
...
func (b *DownloadKubeletStepBuilder) WithURL(url string) *DownloadKubeletStepBuilder {
	b.Step.URL = url
	return b
}
...
func (s *DownloadKubeletStep) getBinaryInfo() (*util.BinaryInfo, error) {
	provider := util.NewBinaryProvider()
	return provider.GetBinaryInfo(
		util.ComponentKubelet,
		s.Version,
		s.Arch,
		s.Zone,
		s.WorkDir,
...
func (s *DownloadKubeletStep) verifyChecksum(filePath, expectedChecksum string) (bool, error) {
...
func (s *DownloadKubeletStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to get kubelet binary info: %w", err)
	}
...
func (s *DownloadKubeletStep) Run(ctx runtime.ExecutionContext) error {
	...
	binaryInfo, err := s.getBinaryInfo()
	if err != nil {
		return fmt.Errorf("failed to get kubelet binary info for run: %w", err)
	}
	...
	logger.Infof("Downloading kubelet from %s ...", binaryInfo.URL)
