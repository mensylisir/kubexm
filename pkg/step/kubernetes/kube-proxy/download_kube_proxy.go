...
func NewDownloadKubeProxyStepBuilder(ctx runtime.Context, instanceName string) *DownloadKubeProxyStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Download Kube Proxy version %s", s.Base.Meta.Name, s.Version)
	s.Base.Sudo = false
...
func (b *DownloadKubeProxyStepBuilder) WithURL(url string) *DownloadKubeProxyStepBuilder {
	b.Step.URL = url
	return b
}
...
func (s *DownloadKubeProxyStep) getBinaryInfo() (*helpers.BinaryInfo, error) {
	provider := helpers.NewBinaryProvider()
	return provider.GetBinaryInfo(
		helpers.ComponentKubeProxy,
		s.Version,
		s.Arch,
		s.Zone,
		s.WorkDir,
...
func (s *DownloadKubeProxyStep) verifyChecksum(filePath, expectedChecksum string) (bool, error) {
...
func (s *DownloadKubeProxyStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to get kube-proxy binary info: %w", err)
	}
...
func (s *DownloadKubeProxyStep) Run(ctx runtime.ExecutionContext) error {
	...
	binaryInfo, err := s.getBinaryInfo()
	if err != nil {
		return fmt.Errorf("failed to get kube-proxy binary info for run: %w", err)
	}
	...
	logger.Infof("Downloading kube-proxy from %s ...", binaryInfo.URL)
