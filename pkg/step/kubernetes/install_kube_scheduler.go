...
func NewInstallKubeSchedulerStepBuilder(ctx runtime.Context, instanceName string) *InstallKubeSchedulerStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Install kube-scheduler binaries for version %s", s.Base.Meta.Name, s.Version)
	s.Base.Sudo = true
...
func (s *InstallKubeSchedulerStep) getExtractedPathOnControlNode() (string, error) {
...
func (s *InstallKubeSchedulerStep) filesToInstall() map[string]struct {
	Target string
	Perms  string
} {
	return map[string]struct {
		...
		"kube-scheduler": {Target: filepath.Join(s.InstallPath, "kube-scheduler"), Perms: s.Permission},
	}
}
...
func (s *InstallKubeSchedulerStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	logger.Info("All required kube-scheduler files already exist on remote host. Step is done.")
	return true, nil
}
