...
func (b *InstallKubeSchedulerStepBuilder) WithInstallPath(path string) *InstallKubeSchedulerStepBuilder {
...
func (s *InstallKubeSchedulerStep) getLocalSourcePath(ctx runtime.ExecutionContext) (string, error) {
	...
	info, err := provider.GetBinaryInfo(helpers.ComponentKubeScheduler, s.Version, arch, "", s.WorkDir, s.ClusterName)
	if err != nil {
		return "", fmt.Errorf("failed to get binary info for kube-scheduler: %w", err)
	}
...
func (s *InstallKubeSchedulerStep) Run(ctx runtime.ExecutionContext) error {
	...
	if _, err := os.Stat(localSourcePath); os.IsNotExist(err) {
		return fmt.Errorf("local source file '%s' not found, ensure the download step ran successfully", localSourcePath)
	}
