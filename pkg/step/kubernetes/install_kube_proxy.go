...
func NewInstallKubeProxyStepBuilder(ctx runtime.Context, instanceName string) *InstallKubeProxyStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Install kube-proxy binaries for version %s", s.Base.Meta.Name, s.Version)
	s.Base.Sudo = true
...
func (s *InstallKubeProxyStep) getExtractedPathOnControlNode() (string, error) {
...
func (s *InstallKubeProxyStep) filesToInstall() map[string]struct {
	Target string
	Perms  string
} {
	return map[string]struct {
		...
		"kube-proxy": {Target: filepath.Join(s.InstallPath, "kube-proxy"), Perms: s.Permission},
	}
}
...
func (s *InstallKubeProxyStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	logger.Info("All required kube-proxy files already exist on remote host. Step is done.")
	return true, nil
}
