...
func NewDownloadKubeControllerManagerStepBuilder(ctx runtime.Context, instanceName string) *DownloadKubeControllerManagerStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Download Kube ControllerManager version %s", s.Base.Meta.Name, s.Version)
	s.Base.Sudo = false
...
func (b *DownloadKubeControllerManagerStepBuilder) WithURL(url string) *DownloadKubeControllerManagerStepBuilder {
	b.Step.URL = url
	return b
}
...
func (s *DownloadKubeControllerManagerStep) getBinaryInfo() (*util.BinaryInfo, error) {
	provider := util.NewBinaryProvider()
	return provider.GetBinaryInfo(
		util.ComponentKubeControllerManager,
		s.Version,
		s.Arch,
		s.Zone,
		s.WorkDir,
...
func (s *DownloadKubeControllerManagerStep) verifyChecksum(filePath, expectedChecksum string) (bool, error) {
...
func (s *DownloadKubeControllerManagerStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to get kube-controller-manager binary info: %w", err)
	}
...
func (s *DownloadKubeControllerManagerStep) Run(ctx runtime.ExecutionContext) error {
	...
	binaryInfo, err := s.getBinaryInfo()
	if err != nil {
		return fmt.Errorf("failed to get kube-controller-manager binary info for run: %w", err)
	}
	...
	logger.Infof("Downloading kube-controller-manager from %s ...", binaryInfo.URL)
