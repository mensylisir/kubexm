...
func NewDownloadKubectlStepBuilder(ctx runtime.Context, instanceName string) *DownloadKubectlStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Download Kubectl version %s", s.Base.Meta.Name, s.Version)
	s.Base.Sudo = false
...
func (b *DownloadKubectlStepBuilder) WithURL(url string) *DownloadKubectlStepBuilder {
	b.Step.URL = url
	return b
}
...
func (s *DownloadKubectlStep) getBinaryInfo() (*helpers.BinaryInfo, error) {
...
func (s *DownloadKubectlStep) verifyChecksum(filePath, expectedChecksum string) (bool, error) {
...
func (s *DownloadKubectlStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to get kubectl binary info: %w", err)
	}
...
func (s *DownloadKubectlStep) Run(ctx runtime.ExecutionContext) error {
	...
	binaryInfo, err := s.getBinaryInfo()
	if err != nil {
		return fmt.Errorf("failed to get kubectl binary info for run: %w", err)
	}
	...
	logger.Infof("Downloading kubectl from %s ...", binaryInfo.URL)
