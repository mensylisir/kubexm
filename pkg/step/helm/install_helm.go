...
func NewInstallHelmStepBuilder(ctx runtime.Context, instanceName string) *InstallHelmStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Install helm binary", s.Base.Meta.Name)
	s.Base.Sudo = false
...
func (s *InstallHelmStep) getLocalExtractedPath(ctx runtime.ExecutionContext) (string, error) {
...
func (s *InstallHelmStep) Run(ctx runtime.ExecutionContext) error {
	...
	arch := ctx.GetHost().GetArch()
	osName := "linux"
	localSourcePath := filepath.Join(localExtractedPath, fmt.Sprintf("%s-%s", osName, arch), "helm")
	...
	logger.Infof("Uploading helm to %s:%s", ctx.GetHost().GetName(), remoteTempPath)
