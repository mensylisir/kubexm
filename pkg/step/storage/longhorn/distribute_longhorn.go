...
// DistributeLonghornArtifactsStep is responsible for distributing the Longhorn artifacts.
...
// DistributeLonghornArtifactsStepBuilder is used to build instances.
...
// NewDistributeLonghornArtifactsStepBuilder is the constructor.
...
func (s *DistributeLonghornArtifactsStep) getLocalValuesPath(ctx runtime.ExecutionContext) (string, error) {
	helmProvider := helm.NewHelmProvider(ctx)
	chart := helmProvider.GetChart("longhorn")
	if chart == nil {
...
func (s *DistributeLonghornArtifactsStep) Run(ctx runtime.ExecutionContext) error {
	...
	if err != nil {
		return fmt.Errorf("failed to read generated values file from agreed path %s: %w. Ensure GenerateLonghornValuesStep ran successfully.", localValuesPath, err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to read offline chart file from %s: %w. Ensure DownloadLonghornChartStep ran successfully.", localChartPath, err)
	}
