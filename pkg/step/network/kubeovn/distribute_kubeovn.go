...
func (s *DistributeKubeovnArtifactsStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if _, err := os.Stat(localValuesPath); os.IsNotExist(err) {
		return false, errors.Wrapf(err, "local source file not found: %s. Ensure GenerateKubeovnValuesStep ran.", localValuesPath)
	}
	if _, err := os.Stat(localChartPath); os.IsNotExist(err) {
		return false, errors.Wrapf(err, "local source file not found: %s. Ensure DownloadKubeovnChartStep ran.", localChartPath)
	}
