...
const (
	CacheKeyRemoteKubeconfigBackupPath = "kubeadm_remote_kubeconfig_backup_path"
)

type KubeadmBackupKubeconfigsStep struct {
	step.Base
	remoteDirToBackup string
	remoteBackupDir   string
}

type KubeadmBackupKubeconfigsStepBuilder struct {
	step.Builder[KubeadmBackupKubeconfigsStepBuilder, *KubeadmBackupKubeconfigsStep]
}

func NewKubeadmBackupKubeconfigsStepBuilder(ctx runtime.Context, instanceName string) *KubeadmBackupKubeconfigsStepBuilder {
	remoteDir := common.KubernetesConfigDir
	backupDir := fmt.Sprintf("%s.%s-%d", remoteDir, "kubexm-backup-kubeconfigs", time.Now().Unix())

	s := &KubeadmBackupKubeconfigsStep{
	...
	b := new(KubeadmBackupKubeconfigsStepBuilder).Init(s)
	return b
}

func (s *KubeadmBackupKubeconfigsStep) Meta() *spec.StepMeta {
	return &s.Base.Meta
}

func (s *KubeadmBackupKubeconfigsStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
...
func (s *KubeadmBackupKubeconfigsStep) Run(ctx runtime.ExecutionContext) error {
	...
	ctx.GetTaskCache().Set(CacheKeyRemoteKubeconfigBackupPath, s.remoteBackupDir)
	logger.Infof("Successfully backed up directory. Backup path '%s' saved to cache.", s.remoteBackupDir)

	return nil
}

func (s *KubeadmBackupKubeconfigsStep) Rollback(ctx runtime.ExecutionContext) error {
...
var _ step.Step = (*KubeadmBackupKubeconfigsStep)(nil)
