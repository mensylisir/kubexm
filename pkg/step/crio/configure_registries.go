...
func (s *ConfigureRegistriesStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if exists {
		...
		if string(remoteContent) == expectedContent {
			logger.Infof("Registries config file '%s' already exists and content matches. Step is done.", s.TargetPath)
			return true, nil
		}

		logger.Infof("Registries config file '%s' exists but content differs. Step needs to run.", s.TargetPath)
		return false, nil
	}

	logger.Infof("Registries config file '%s' does not exist. Configuration is required.", s.TargetPath)
	return false, nil
}
...
func (s *ConfigureRegistriesStep) Run(ctx runtime.ExecutionContext) error {
	...
	logger.Infof("Writing registries config file to %s", s.TargetPath)
