...
func NewDownloadCrioStepBuilder(ctx runtime.Context, instanceName string) *DownloadCrioStepBuilder {
...
func (s *DownloadCrioStep) Run(ctx runtime.ExecutionContext) error {
	...
	for arch := range requiredArchs {
		binaryInfo, err := provider.GetBinary(ComponentCrio, arch)
		if err != nil {
			return fmt.Errorf("failed to get CRI-O info for arch %s: %w", arch, err)
		}
		if binaryInfo == nil {
			continue
		}
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			return fmt.Errorf("failed to download CRI-O for arch %s: %w", arch, err)
		}
	}
