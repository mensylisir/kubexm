...
func (s *DownloadArgoCDChartStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	// TODO: Add a check to see if argocd is enabled in the cluster config.
...
func (s *DownloadArgoCDChartStep) Run(ctx runtime.ExecutionContext) error {
	...
	if chart == nil {
		return fmt.Errorf("could not get argocd chart info for Kubernetes version %s", ctx.GetClusterConfig().Spec.Kubernetes.Version)
	}
	...
	if _, err := os.Stat(destFile); err == nil {
		logger.Infof("Argo CD chart package %s already exists, skipping download.", destFile)
		return nil
	}
...
func (s *DownloadArgoCDChartStep) Rollback(ctx runtime.ExecutionContext) error {
	...
	logger.Warn("Rollback for DownloadArgoCDChartStep is a no-op.")
	return nil
}
