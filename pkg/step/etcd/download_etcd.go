...
func NewDownloadEtcdStepBuilder(ctx runtime.Context, instanceName string) *DownloadEtcdStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Download etcd version %s", s.Base.Meta.Name, s.Version)
	s.Base.Sudo = false
...
func (s *DownloadEtcdStep) verifyChecksum(filePath, expectedChecksum string) (bool, error) {
...
func (s *DownloadEtcdStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to get etcd binary info: %w", err)
	}
...
func (s *DownloadEtcdStep) Run(ctx runtime.ExecutionContext) error {
	...
	binaryInfo, err := s.getBinaryInfo()
	if err != nil {
		return fmt.Errorf("failed to get etcd binary info for run: %w", err)
	}
	...
	logger.Infof("Downloading etcd from %s ...", binaryInfo.URL)
