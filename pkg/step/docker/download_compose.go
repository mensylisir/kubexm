...
func NewDownloadDockerComposeStepBuilder(ctx runtime.Context, instanceName string) *DownloadDockerComposeStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Download docker-compose version %s", s.Base.Meta.Name, s.Version)
	s.Base.Sudo = false
...
func (b *DownloadDockerComposeStepBuilder) WithVersion(version string) *DownloadDockerComposeStepBuilder {
	if version != "" {
		b.Step.Version = version
		b.Step.Base.Meta.Description = fmt.Sprintf("[%s]>>Download docker-compose version %s", b.Step.Base.Meta.Name, b.Step.Version)
	}
	return b
}

func (b *DownloadDockerComposeStepBuilder) WithArch(arch string) *DownloadDockerComposeStepBuilder {
	if arch != "" {
		b.Step.Arch = arch
	}
	return b
...
func (s *DownloadDockerComposeStep) getBinaryInfo(ctx runtime.ExecutionContext) (*binary.Binary, error) {
	...
	if binaryInfo == nil {
		return nil, fmt.Errorf("Docker-Compose are disabled or no compatible version found")
	}
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Download Docker-Compose version %s", s.Base.Meta.Name, binaryInfo.Version)
...
func (s *DownloadDockerComposeStep) verifyChecksum(filePath, expectedChecksum string) (bool, error) {
...
func (s *DownloadDockerComposeStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to get docker-compose binary info: %w", err)
	}
...
func (s *DownloadDockerComposeStep) Run(ctx runtime.ExecutionContext) error {
	...
	binaryInfo, err := s.getBinaryInfo(ctx)
	if err != nil {
		return fmt.Errorf("failed to get docker-compose binary info for run: %w", err)
	}
	...
	logger.Infof("Downloading docker-compose from %s ...", binaryInfo.URL)
