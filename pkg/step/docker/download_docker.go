...
func NewDownloadDockerStepBuilder(ctx runtime.Context, instanceName string) *DownloadDockerStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Download Docker version %s", s.Base.Meta.Name, s.Version)
	s.Base.Sudo = false
...
func (s *DownloadDockerStep) getBinaryInfo() (*helpers.BinaryInfo, error) {
...
func (s *DownloadDockerStep) verifyChecksum(filePath, expectedChecksum string) (bool, error) {
...
func (s *DownloadDockerStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to get docker binary info: %w", err)
	}
...
func (s *DownloadDockerStep) Run(ctx runtime.ExecutionContext) error {
	...
	binaryInfo, err := s.getBinaryInfo()
	if err != nil {
		return fmt.Errorf("failed to get docker binary info for run: %w", err)
	}
	...
	logger.Infof("Downloading docker from %s ...", binaryInfo.URL)
