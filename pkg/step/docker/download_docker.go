...
func NewDownloadDockerStepBuilder(ctx runtime.Context, instanceName string) *DownloadDockerStepBuilder {
...
func (s *DownloadDockerStep) verifyChecksum(filePath string, binaryInfo *binary.Binary) (bool, error) {
...
func (s *DownloadDockerStep) Run(ctx runtime.ExecutionContext) error {
	...
	for arch := range requiredArchs {
		binaryInfo, err := provider.GetBinary(binary.ComponentDocker, arch)
		if err != nil {
			return fmt.Errorf("failed to get Docker info for arch %s: %w", arch, err)
		}
		if binaryInfo == nil {
			continue
		}
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			return fmt.Errorf("failed to download Docker for arch %s: %w", arch, err)
		}
	}
