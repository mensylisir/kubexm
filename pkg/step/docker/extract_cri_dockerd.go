...
func NewExtractCriDockerdStepBuilder(ctx runtime.Context, instanceName string) *ExtractCriDockerdStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Extract cri-dockerd archive for version %s", s.Base.Meta.Name, s.Version)
	s.Base.Sudo = false
...
func (s *ExtractCriDockerdStep) getPaths() (sourcePath, destPath, cacheKey string, err error) {
