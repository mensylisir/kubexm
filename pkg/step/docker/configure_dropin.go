...
const (
	DockerDropInTemplatePath = "docker/kubexm.conf.tmpl"
)

type ConfigureDockerDropInStep struct {
	step.Base
	TargetPath string
	HTTPProxy  string
	HTTPSProxy string
...
type ConfigureDockerDropInStepBuilder struct {
	step.Builder[ConfigureDockerDropInStepBuilder, *ConfigureDockerDropInStep]
}

func NewConfigureDockerDropInStepBuilder(ctx runtime.Context, instanceName string) *ConfigureDockerDropInStepBuilder {
	s := &ConfigureDockerDropInStep{
		TargetPath: common.DockerDefaultDropInFile,
	}

	s.Base.Meta.Name = instanceName
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Configure docker systemd drop-in file", s.Base.Meta.Name)
	s.Base.Sudo = true
	s.Base.IgnoreError = false
	s.Base.Timeout = 1 * time.Minute

	b := new(ConfigureDockerDropInStepBuilder).Init(s)
	return b
}

func (b *ConfigureDockerDropInStepBuilder) WithTargetPath(path string) *ConfigureDockerDropInStepBuilder {
	b.Step.TargetPath = path
	return b
}

func (b *ConfigureDockerDropInStepBuilder) WithHTTPProxy(httpProxy string) *ConfigureDockerDropInStepBuilder {
	b.Step.HTTPProxy = httpProxy
	return b
}

func (b *ConfigureDockerDropInStepBuilder) WithHTTPSProxy(httpsProxy string) *ConfigureDockerDropInStepBuilder {
	b.Step.HTTPSProxy = httpsProxy
	return b
}

func (b *ConfigureDockerDropInStepBuilder) WithNOProxy(noProxy string) *ConfigureDockerDropInStepBuilder {
	b.Step.NOProxy = noProxy
	return b
}
func (s *ConfigureDockerDropInStep) Meta() *spec.StepMeta {
	return &s.Base.Meta
}

func (s *ConfigureDockerDropInStep) renderContent() (string, error) {
	tmplStr, err := templates.Get(DockerDropInTemplatePath)
	if err != nil {
		return "", err
	}
	...
	if err := tmpl.Execute(&buf, s); err != nil {
		return "", fmt.Errorf("failed to render docker drop-in template: %w", err)
	}
...
func (s *ConfigureDockerDropInStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
...
func (s *ConfigureDockerDropInStep) Run(ctx runtime.ExecutionContext) error {
...
func (s *ConfigureDockerDropInStep) Rollback(ctx runtime.ExecutionContext) error {
	runner := ctx.GetRunner()
	conn, err := ctx.GetCurrentHostConnector()
	if err != nil {
		return nil
