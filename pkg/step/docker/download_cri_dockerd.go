...
func (s *DownloadCriDockerdStep) verifyChecksum(filePath string, binaryInfo *binary.Binary) (bool, error) {
...
func (s *DownloadCriDockerdStep) Run(ctx runtime.ExecutionContext) error {
	...
	for arch := range requiredArchs {
		binaryInfo, err := provider.GetBinary(binary.ComponentCriDockerd, arch)
		if err != nil {
			return fmt.Errorf("failed to get cri-dockerd info for arch %s: %w", arch, err)
		}
		if binaryInfo == nil {
			continue
		}
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			return fmt.Errorf("failed to download cri-dockerd for arch %s: %w", arch, err)
		}
	}
