...
type InstallDockerCliStep struct {
	step.Base
	Version     string
	Arch        string
	WorkDir     string
...
type InstallDockerCliStepBuilder struct {
	step.Builder[InstallDockerCliStepBuilder, *InstallDockerCliStep]
}

func NewInstallDockerCliStepBuilder(ctx runtime.Context, instanceName string) *InstallDockerCliStepBuilder {
	s := &InstallDockerCliStep{
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Install Docker CLI binaries for version %s", s.Base.Meta.Name, s.Version)
	s.Base.Sudo = true
	s.Base.IgnoreError = false
	s.Base.Timeout = 5 * time.Minute

	b := new(InstallDockerCliStepBuilder).Init(s)
	return b
}

func (b *InstallDockerCliStepBuilder) WithInstallPath(installPath string) *InstallDockerCliStepBuilder {
	b.Step.InstallPath = installPath
	return b
}

func (b *InstallDockerCliStepBuilder) WithPermission(permission string) *InstallDockerCliStepBuilder {
	b.Step.Permission = permission
	return b
}

func (s *InstallDockerCliStep) Meta() *spec.StepMeta {
	return &s.Base.Meta
}

func (s *InstallDockerCliStep) getExtractedPathOnControlNode() (string, error) {
...
func (s *InstallDockerCliStep) filesToInstall() map[string]struct {
	Target string
	Perms  string
} {
...
func (s *InstallDockerCliStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
...
func (s *InstallDockerCliStep) Run(ctx runtime.ExecutionContext) error {
...
func (s *InstallDockerCliStep) Rollback(ctx runtime.ExecutionContext) error {
