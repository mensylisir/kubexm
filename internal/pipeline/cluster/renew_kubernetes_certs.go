...
// RenewKubernetesLeafPipeline defines the pipeline for renewing only Kubernetes leaf certificates.
type RenewKubernetesLeafPipeline struct {
	*pipeline.Base
	PipelineModules []module.Module
	AssumeYes       bool
}

// NewRenewKubernetesLeafPipeline creates a new RenewKubernetesLeafPipeline.
func NewRenewKubernetesLeafPipeline(assumeYes bool) pipeline.Pipeline {
	modules := []module.Module{
		...
		modulePki.NewPKIModule("kubernetes-certs"),
	}

	return &RenewKubernetesLeafPipeline{
		Base:            pipeline.NewBase("RenewKubernetesLeaf", "Renews only Kubernetes leaf certificates (preserves existing CA) in the cluster"),
		PipelineModules: modules,
		AssumeYes:       assumeYes,
	}
}

func (p *RenewKubernetesLeafPipeline) Name() string {
	return p.Base.Meta.Name
}

func (p *RenewKubernetesLeafPipeline) Description() string {
	return p.Base.Meta.Description
}

func (p *RenewKubernetesLeafPipeline) Modules() []module.Module {
...
func (p *RenewKubernetesLeafPipeline) Plan(ctx runtime2.PipelineContext) (*plan.ExecutionGraph, error) {
	return pipeline.SafePlan(ctx, p.Name(), func() (*plan.ExecutionGraph, error) {
		logger := ctx.GetLogger().With("pipeline", p.Name())
		logger.Info("Planning pipeline for Kubernetes leaf certificate renewal...")
		...
		logger.Info("Kubernetes leaf renewal pipeline planning complete.", "total_nodes", len(finalGraph.Nodes))
		return finalGraph, nil
	})
}

func (p *RenewKubernetesLeafPipeline) Run(ctx runtime2.PipelineContext, graph *plan.ExecutionGraph, dryRun bool) (*plan.GraphExecutionResult, error) {
	logger := ctx.GetLogger().With("pipeline", p.Name())
	logger.Info("Running Kubernetes leaf renewal pipeline...", "dryRun", dryRun)
	...
	logger.Info("Kubernetes leaf renewal pipeline run completed.", "status", result.Status)
	return result, nil
}

var _ pipeline.Pipeline = (*RenewKubernetesLeafPipeline)(nil)
