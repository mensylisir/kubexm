...
// RenewKubernetesCAPipeline defines the pipeline for renewing all Kubernetes CA certificates.
type RenewKubernetesCAPipeline struct {
	*pipeline.Base
	PipelineModules []module.Module
	AssumeYes       bool
}

// NewRenewKubernetesCAPipeline creates a new RenewKubernetesCAPipeline.
func NewRenewKubernetesCAPipeline(assumeYes bool) pipeline.Pipeline {
	modules := []module.Module{
		...
		modulePki.NewPKIModule("kubernetes-ca"),
	}

	return &RenewKubernetesCAPipeline{
		Base:            pipeline.NewBase("RenewKubernetesCA", "Renews all Kubernetes CA certificates (CA + leaf certs) in the cluster"),
		PipelineModules: modules,
		AssumeYes:       assumeYes,
	}
}

func (p *RenewKubernetesCAPipeline) Name() string {
	return p.Base.Meta.Name
}

func (p *RenewKubernetesCAPipeline) Description() string {
	return p.Base.Meta.Description
}

func (p *RenewKubernetesCAPipeline) Modules() []module.Module {
...
func (p *RenewKubernetesCAPipeline) Plan(ctx runtime2.PipelineContext) (*plan.ExecutionGraph, error) {
	return pipeline.SafePlan(ctx, p.Name(), func() (*plan.ExecutionGraph, error) {
		logger := ctx.GetLogger().With("pipeline", p.Name())
		logger.Info("Planning pipeline for Kubernetes CA certificate renewal...")
		...
		logger.Info("Kubernetes CA renewal pipeline planning complete.", "total_nodes", len(finalGraph.Nodes))
		return finalGraph, nil
	})
}

func (p *RenewKubernetesCAPipeline) Run(ctx runtime2.PipelineContext, graph *plan.ExecutionGraph, dryRun bool) (*plan.GraphExecutionResult, error) {
	logger := ctx.GetLogger().With("pipeline", p.Name())
	logger.Info("Running Kubernetes CA renewal pipeline...", "dryRun", dryRun)
	...
	logger.Info("Kubernetes CA renewal pipeline run completed.", "status", result.Status)
	return result, nil
}

var _ pipeline.Pipeline = (*RenewKubernetesCAPipeline)(nil)
