...
// DeleteNodesPipeline defines the pipeline for deleting specific nodes from a Kubernetes cluster.
type DeleteNodesPipeline struct {
	*pipeline.Base
	PipelineModules []module.Module
	AssumeYes       bool
}

// NewDeleteNodesPipeline creates a new DeleteNodesPipeline.
func NewDeleteNodesPipeline(assumeYes bool) pipeline.Pipeline {
	...
	return &DeleteNodesPipeline{
		Base:            pipeline.NewBase("DeleteNodes", "Deletes specific nodes from an existing Kubernetes cluster"),
		PipelineModules: modules,
		AssumeYes:       assumeYes,
	}
}

func (p *DeleteNodesPipeline) Name() string {
	return p.Base.Meta.Name
}

func (p *DeleteNodesPipeline) Description() string {
	return p.Base.Meta.Description
}

func (p *DeleteNodesPipeline) Modules() []module.Module {
...
func (p *DeleteNodesPipeline) Plan(ctx runtime2.PipelineContext) (*plan.ExecutionGraph, error) {
	return pipeline.SafePlan(ctx, p.Name(), func() (*plan.ExecutionGraph, error) {
		logger := ctx.GetLogger().With("pipeline", p.Name())
		logger.Info("Planning pipeline for deleting nodes...")
		...
		logger.Info("Delete nodes pipeline planning complete.", "total_nodes", len(finalGraph.Nodes))
		return finalGraph, nil
	})
}

func (p *DeleteNodesPipeline) Run(ctx runtime2.PipelineContext, graph *plan.ExecutionGraph, dryRun bool) (*plan.GraphExecutionResult, error) {
	logger := ctx.GetLogger().With("pipeline", p.Name())
	logger.Info("Running delete nodes pipeline...", "dryRun", dryRun)
	...
	logger.Info("Delete nodes pipeline run completed.", "status", result.Status)
	return result, nil
}

var _ pipeline.Pipeline = (*DeleteNodesPipeline)(nil)
