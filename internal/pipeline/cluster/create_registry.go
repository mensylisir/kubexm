...
// CreateRegistryPipeline defines the pipeline for creating a registry.
type CreateRegistryPipeline struct {
	*pipeline.Base
	PipelineModules []module.Module
	AssumeYes       bool
}

// NewCreateRegistryPipeline creates a new CreateRegistryPipeline.
func NewCreateRegistryPipeline(assumeYes bool) pipeline.Pipeline {
	modules := []module.Module{
		...
		registry.NewRegistryModule("install"),
	}

	return &CreateRegistryPipeline{
		Base:            pipeline.NewBase("CreateRegistry", "Creates a container registry for the cluster"),
		PipelineModules: modules,
		AssumeYes:       assumeYes,
	}
}

func (p *CreateRegistryPipeline) Name() string {
	return p.Base.Meta.Name
}

func (p *CreateRegistryPipeline) Description() string {
	return p.Base.Meta.Description
}

func (p *CreateRegistryPipeline) Modules() []module.Module {
...
func (p *CreateRegistryPipeline) Plan(ctx runtime2.PipelineContext) (*plan.ExecutionGraph, error) {
	return pipeline.SafePlan(ctx, p.Name(), func() (*plan.ExecutionGraph, error) {
		logger := ctx.GetLogger().With("pipeline", p.Name())
		logger.Info("Planning create registry pipeline...")
		...
		logger.Info("Create registry pipeline planning complete.", "total_nodes", len(finalGraph.Nodes))
		return finalGraph, nil
	})
}

func (p *CreateRegistryPipeline) Run(ctx runtime2.PipelineContext, graph *plan.ExecutionGraph, dryRun bool) (*plan.GraphExecutionResult, error) {
	logger := ctx.GetLogger().With("pipeline", p.Name())
	logger.Info("Running create registry pipeline...", "dryRun", dryRun)
	...
	logger.Info("Create registry pipeline run completed.", "status", result.Status)
	return result, nil
}

var _ pipeline.Pipeline = (*CreateRegistryPipeline)(nil)
