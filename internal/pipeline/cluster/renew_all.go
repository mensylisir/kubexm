...
// RenewAllPipeline defines the pipeline for renewing all certificates in a Kubernetes cluster.
type RenewAllPipeline struct {
	*pipeline.Base
	PipelineModules []module.Module
	AssumeYes       bool
}

// NewRenewAllPipeline creates a new RenewAllPipeline.
func NewRenewAllPipeline(assumeYes bool) pipeline.Pipeline {
	modules := []module.Module{
		...
		modulePki.NewPKIModule("all"),
	}

	return &RenewAllPipeline{
		Base:            pipeline.NewBase("RenewAll", "Renews all certificates (Kubernetes + etcd, CA + leaf) in the cluster"),
		PipelineModules: modules,
		AssumeYes:       assumeYes,
	}
}

func (p *RenewAllPipeline) Name() string {
	return p.Base.Meta.Name
}

func (p *RenewAllPipeline) Description() string {
	return p.Base.Meta.Description
}

func (p *RenewAllPipeline) Modules() []module.Module {
...
func (p *RenewAllPipeline) Plan(ctx runtime2.PipelineContext) (*plan.ExecutionGraph, error) {
	return pipeline.SafePlan(ctx, p.Name(), func() (*plan.ExecutionGraph, error) {
		logger := ctx.GetLogger().With("pipeline", p.Name())
		logger.Info("Planning pipeline for all certificates renewal...")
		...
		logger.Info("All certificates renewal pipeline planning complete.", "total_nodes", len(finalGraph.Nodes))
		return finalGraph, nil
	})
}

func (p *RenewAllPipeline) Run(ctx runtime2.PipelineContext, graph *plan.ExecutionGraph, dryRun bool) (*plan.GraphExecutionResult, error) {
	logger := ctx.GetLogger().With("pipeline", p.Name())
	logger.Info("Running all certificates renewal pipeline...", "dryRun", dryRun)
	...
	logger.Info("All certificates renewal pipeline run completed.", "status", result.Status)
	return result, nil
}

var _ pipeline.Pipeline = (*RenewAllPipeline)(nil)
