...
// RenewEtcdLeafPipeline defines the pipeline for renewing only etcd leaf certificates.
type RenewEtcdLeafPipeline struct {
	*pipeline.Base
	PipelineModules []module.Module
	AssumeYes       bool
}

// NewRenewEtcdLeafPipeline creates a new RenewEtcdLeafPipeline.
func NewRenewEtcdLeafPipeline(assumeYes bool) pipeline.Pipeline {
	modules := []module.Module{
		...
		modulePki.NewPKIModule("etcd-certs"),
	}

	return &RenewEtcdLeafPipeline{
		Base:            pipeline.NewBase("RenewEtcdLeaf", "Renews only etcd leaf certificates (preserves existing CA) in the cluster"),
		PipelineModules: modules,
		AssumeYes:       assumeYes,
	}
}

func (p *RenewEtcdLeafPipeline) Name() string {
	return p.Base.Meta.Name
}

func (p *RenewEtcdLeafPipeline) Description() string {
	return p.Base.Meta.Description
}

func (p *RenewEtcdLeafPipeline) Modules() []module.Module {
...
func (p *RenewEtcdLeafPipeline) Plan(ctx runtime2.PipelineContext) (*plan.ExecutionGraph, error) {
	return pipeline.SafePlan(ctx, p.Name(), func() (*plan.ExecutionGraph, error) {
		logger := ctx.GetLogger().With("pipeline", p.Name())
		logger.Info("Planning pipeline for etcd leaf certificate renewal...")
		...
		logger.Info("Etcd leaf renewal pipeline planning complete.", "total_nodes", len(finalGraph.Nodes))
		return finalGraph, nil
	})
}

func (p *RenewEtcdLeafPipeline) Run(ctx runtime2.PipelineContext, graph *plan.ExecutionGraph, dryRun bool) (*plan.GraphExecutionResult, error) {
	logger := ctx.GetLogger().With("pipeline", p.Name())
	logger.Info("Running etcd leaf renewal pipeline...", "dryRun", dryRun)
	...
	logger.Info("Etcd leaf renewal pipeline run completed.", "status", result.Status)
	return result, nil
}

var _ pipeline.Pipeline = (*RenewEtcdLeafPipeline)(nil)
