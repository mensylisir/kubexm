...
func NewAddEtcdMemberStepBuilder(ctx runtime.ExecutionContext, instanceName string) *AddEtcdMemberStepBuilder {
	s := &AddEtcdMemberStep{
...
func (s *AddEtcdMemberStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "phase", "Run")

	if s.NewNode == nil {
		err := fmt.Errorf("NewNode is not specified for AddEtcdMemberStep")
		result.MarkFailed(err, "NewNode not specified")
		return result, err
	}
	...
	if err != nil {
		if strings.Contains(stderr, "member already exists") {
			...
			result.MarkCompleted("Member already exists")
			return result, nil
		}
		err = fmt.Errorf("failed to add etcd member: %w, stderr: %s", err, stderr)
		result.MarkFailed(err, "Failed to add etcd member")
		return result, err
	}

	logger.Info("New member has been successfully registered in the etcd cluster.", "member", newNodeName)
	result.MarkCompleted("Member added successfully")
	return result, nil
}
