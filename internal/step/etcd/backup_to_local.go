...
func NewEtcdBackupDataStepBuilder(ctx runtime.ExecutionContext, instanceName string) *EtcdBackupDataStepBuilder {
...
func (s *EtcdBackupDataStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if _, err := runner.Run(ctx.GoContext(), conn, snapshotCmd, s.Sudo); err != nil {
		err = fmt.Errorf("failed to create etcd snapshot on host '%s': %w", nodeName, err)
		result.MarkFailed(err, "Failed to create snapshot")
		return result, err
	}
	...
	if err := runner.Fetch(ctx.GoContext(), conn, s.remoteSnapshotPath, s.localSnapshotPath, s.Sudo); err != nil {
		...
		err = fmt.Errorf("failed to fetch etcd snapshot: %w", err)
		result.MarkFailed(err, "Failed to fetch snapshot")
		return result, err
	}
