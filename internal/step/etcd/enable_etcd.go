...
func NewEnableEtcdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *EnableEtcdStepBuilder {
	s := &EnableEtcdStep{
...
func (s *EnableEtcdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	facts, err := ctx.GetHostFacts(ctx.GetHost())
	if err != nil {
		err = fmt.Errorf("failed to get host facts to enable service: %w", err)
	...
	if err := runner.EnableService(ctx.GoContext(), conn, facts, s.ServiceName); err != nil {
		err = fmt.Errorf("failed to enable etcd service %s: %w", s.ServiceName, err)
