...
func NewCheckEtcdHealthStepBuilder(ctx runtime.ExecutionContext, instanceName string) *CheckEtcdHealthStepBuilder {
...
func (s *CheckEtcdHealthStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for {
		...
		case <-timeout:
			if lastErr != nil {
				err := fmt.Errorf("etcd health check timed out after %v: %w", s.Base.Timeout, lastErr)
				result.MarkFailed(err, "Health check timed out")
				return result, err
			}
			err := fmt.Errorf("etcd health check timed out after %v", s.Base.Timeout)
			result.MarkFailed(err, "Health check timed out")
			return result, err
		case <-ticker.C:
