...
func NewGenerateEtcdCAStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateEtcdCAStepBuilder {
...
func (s *GenerateEtcdCAStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		...
		result.MarkFailed(err, "Failed to setup ETCD CA")
		return result, err
	}

	logger.Info("ETCD CA is ready.")
	result.MarkCompleted("CA generated successfully")
	return result, nil
}
