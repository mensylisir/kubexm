...
func NewGenerateEtcdCertsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateEtcdCertsStepBuilder {
...
func (s *GenerateEtcdCertsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for _, node := range s.EtcdNodes {
		...
		if err := helpers.NewSignedCertificate(s.LocalCertsDir, adminCertFile, adminKeyFile, adminCfg, caCert, caKey); err != nil {
			err = fmt.Errorf("failed to generate etcd admin certificate for node %s: %w", nodeName, err)
			result.MarkFailed(err, "Failed to generate admin certificate")
			return result, err
		}
		...
		if err := helpers.NewSignedCertificate(s.LocalCertsDir, nodeCertFile, nodeKeyFile, nodeCfg, caCert, caKey); err != nil {
			err = fmt.Errorf("failed to generate etcd node certificate for node %s: %w", nodeName, err)
			result.MarkFailed(err, "Failed to generate node certificate")
			return result, err
		}
		...
		if err := helpers.NewSignedCertificate(s.LocalCertsDir, memberCertFile, memberKeyFile, memberCfg, caCert, caKey); err != nil {
			err = fmt.Errorf("failed to generate etcd member certificate for node %s: %w", nodeName, err)
			result.MarkFailed(err, "Failed to generate member certificate")
			return result, err
		}
	}
