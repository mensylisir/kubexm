...
func NewInstallEtcdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallEtcdStepBuilder {
	s := &InstallEtcdStep{
...
func (s *InstallEtcdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if _, err := os.Stat(localExtractedPath); os.IsNotExist(err) {
		err = fmt.Errorf("local etcd extracted path '%s' not found, ensure extract step ran successfully", localExtractedPath)
		result.MarkFailed(err, "Extracted path not found")
		return result, err
	}
	...
	for sourceRelPath, remoteTargetPath := range files {
		...
		if err := runner.Upload(ctx.GoContext(), conn, localSourcePath, remoteTempPath, false); err != nil {
			err = fmt.Errorf("failed to upload '%s': %w", localSourcePath, err)
			result.MarkFailed(err, "Failed to upload file")
			return result, err
		}
		...
		if _, err := runner.Run(ctx.GoContext(), conn, moveCmd, s.Sudo); err != nil {
			err = fmt.Errorf("failed to move file to '%s': %w", remoteTargetPath, err)
		...
		if err := runner.Chmod(ctx.GoContext(), conn, remoteTargetPath, s.Permission, s.Sudo); err != nil {
			err = fmt.Errorf("failed to set permission on '%s': %w", remoteTargetPath, err)
	...
	result.MarkCompleted("Installation completed successfully")
	return result, nil
}
