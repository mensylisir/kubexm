...
func NewStopEtcdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *StopEtcdStepBuilder {
	s := &StopEtcdStep{
...
func (s *StopEtcdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "phase", "Run")

	facts, err := ctx.GetHostFacts(ctx.GetHost())
	if err != nil {
		err = fmt.Errorf("failed to get host facts to stop service: %w", err)
	...
	if err := runner.StopService(ctx.GoContext(), conn, facts, s.ServiceName); err != nil {
		err = fmt.Errorf("failed to stop etcd service %s: %w", s.ServiceName, err)
