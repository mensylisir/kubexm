...
func NewConfigureEtcdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ConfigureEtcdStepBuilder {
	s := &ConfigureEtcdStep{
...
func (s *ConfigureEtcdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, s.RemoteConfDir, s.PermissionDir, s.Sudo); err != nil {
		err = fmt.Errorf("failed to create remote config directory %s on node %s: %w", s.RemoteConfDir, ctx.GetHost().GetName(), err)
	...
	if err := helpers.WriteContentToRemote(ctx, conn, string(s.renderedContent), remotePath, s.PermissionFile, s.Sudo); err != nil {
		err = fmt.Errorf("failed to write etcd config to %s on node %s: %w", remotePath, ctx.GetHost().GetName(), err)
		result.MarkFailed(err, "Failed to write etcd config")
		return result, err
	}

	logger.Info("Etcd configuration file for current node has been deployed successfully.")
	result.MarkCompleted("Configuration deployed successfully")
	return result, nil
}
