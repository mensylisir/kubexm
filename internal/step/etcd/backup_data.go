...
func NewEtcdBackupLocalDataStepBuilder(ctx runtime.ExecutionContext, instanceName string) *EtcdBackupLocalDataStepBuilder {
...
func (s *EtcdBackupLocalDataStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if _, err := runner.Run(ctx.GoContext(), conn, backupCmd, s.Sudo); err != nil {
		err = fmt.Errorf("failed to back up remote etcd data directory on host '%s': %w", ctx.GetHost().GetName(), err)
		result.MarkFailed(err, "Failed to backup etcd data")
		return result, err
	}
