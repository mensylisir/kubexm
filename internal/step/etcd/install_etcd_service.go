...
func NewInstallEtcdServiceStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallEtcdServiceStepBuilder {
...
func (s *InstallEtcdServiceStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if s.renderedContent == nil {
		content, err := s.renderServiceFile()
		if err != nil {
			err = fmt.Errorf("failed to render service file: %w", err)
	...
	if err := helpers.WriteContentToRemote(ctx, conn, string(s.renderedContent), remotePath, "0644", s.Sudo); err != nil {
		err = fmt.Errorf("failed to write etcd.service file to %s: %w", remotePath, err)
