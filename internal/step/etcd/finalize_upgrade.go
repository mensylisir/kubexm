...
func NewEtcdFinalizeUpgradeStepBuilder(ctx runtime.ExecutionContext, instanceName string) *EtcdFinalizeUpgradeStepBuilder {
...
func (s *EtcdFinalizeUpgradeStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if _, err := runner.Run(ctx.GoContext(), conn, finalizeCmd, s.Sudo); err != nil {
		err = fmt.Errorf("failed to set etcd cluster version: %w", err)
		result.MarkFailed(err, "Failed to set cluster version")
		return result, err
	}

	logger.Info("Successfully finalized the etcd cluster upgrade.")
	result.MarkCompleted("Upgrade finalized successfully")
	return result, nil
}
