...
func NewExtractEtcdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ExtractEtcdStepBuilder {
...
func (s *ExtractEtcdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(requiredArchs) == 0 {
		...
		result.MarkCompleted("No extraction needed")
		return result, nil
	}

	if err := os.MkdirAll(ctx.GetExtractDir(), 0755); err != nil {
		err = fmt.Errorf("failed to create global extract directory '%s': %w", ctx.GetExtractDir(), err)
		result.MarkFailed(err, "Failed to create extract directory")
		return result, err
	}

	for arch := range requiredArchs {
		if err := s.extractFileForArch(ctx, arch); err != nil {
			result.MarkFailed(err, "Failed to extract for arch "+arch)
			return result, err
		}
	}

	logger.Info("All required etcd archives have been extracted successfully.")
	result.MarkCompleted("Extraction completed successfully")
	return result, nil
}
