...
type DownloadEtcdStepBuilder struct {
	step.Builder[DownloadEtcdStepBuilder, *DownloadEtcdStep]
}

func NewDownloadEtcdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadEtcdStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Download etcd binaries for all required architectures", s.Base.Meta.Name)
	s.Base.Sudo = false
...
func (s *DownloadEtcdStep) Meta() *spec.StepMeta {
	return &s.Base.Meta
}

func (s *DownloadEtcdStep) getRequiredArchs(ctx runtime.ExecutionContext) (map[string]bool, error) {
	archs := make(map[string]bool)
	etcdHosts := ctx.GetHostsByRole("")
	if len(etcdHosts) == 0 {
		return nil, fmt.Errorf("no control-plane hosts found in cluster configuration to determine required etcd architectures")
	}
...
func (s *DownloadEtcdStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if len(requiredArchs) == 0 {
		logger.Info("No hosts require etcd binaries in this cluster configuration. Step is done.")
		return true, nil
	}
	...
	for arch := range requiredArchs {
		...
		if err != nil {
			return false, fmt.Errorf("failed to get etcd info for arch %s: %w", arch, err)
		}
		if binaryInfo == nil {
			continue
		}
		...
		logger.Infof("Checking for etcd (arch: %s) at: %s", arch, destPath)
	...
	if allDone {
		logger.Info("All required etcd binaries for all architectures already exist and are valid.")
		return true, nil
	}
	return false, nil
}

func (s *DownloadEtcdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for arch := range requiredArchs {
		binaryInfo, err := provider.GetBinary(binary.ComponentEtcd, arch)
		if err != nil {
			err = fmt.Errorf("failed to get etcd info for arch %s: %w", arch, err)
			result.MarkFailed(err, "Failed to get etcd info")
			return result, err
		}
		if binaryInfo == nil {
			continue
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			err = fmt.Errorf("failed to download etcd for arch %s: %w", arch, err)
			result.MarkFailed(err, "Failed to download etcd")
			return result, err
		}
	}

	logger.Info("All required etcd binaries have been downloaded successfully.")
	result.MarkCompleted("Download completed successfully")
	return result, nil
}
...
func (s *DownloadEtcdStep) downloadFile(ctx runtime.ExecutionContext, binaryInfo *binary.Binary) error {
	...
	logger.Infof("Downloading etcd (arch: %s, version: %s) from %s ...", binaryInfo.Arch, binaryInfo.Version, binaryInfo.URL())
...
func (s *DownloadEtcdStep) Rollback(ctx runtime.ExecutionContext) error {
