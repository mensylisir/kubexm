...
func NewDistributeEtcdCertsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DistributeEtcdCertsStepBuilder {
...
func (s *DistributeEtcdCertsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for _, fileName := range filesToCopy {
		...
		if err := runner.Upload(ctx.GoContext(), conn, localPath, remotePath, s.Sudo); err != nil {
			err = fmt.Errorf("failed to upload %s to %s on node %s: %w", fileName, remotePath, ctx.GetHost().GetName(), err)
		...
		if err := runner.Chmod(ctx.GoContext(), conn, remotePath, permission, s.Sudo); err != nil {
			err = fmt.Errorf("failed to set permission on %s for node %s: %w", remotePath, ctx.GetHost().GetName(), err)
