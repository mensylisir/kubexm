...
func NewInstallCiliumHelmChartStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallCiliumHelmChartStepBuilder {
...
func (s *InstallCiliumHelmChartStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to install/upgrade Cilium Helm chart")
		return result, err
	}

	logger.Info("Cilium Helm chart installed/upgraded successfully.")
	logger.Debugf("Helm command output:\n%s", output)
	result.MarkCompleted("Cilium Helm chart installed/upgraded successfully")
	return result, nil
}
