...
func NewDownloadCiliumChartStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadCiliumChartStepBuilder {
...
func (s *DownloadCiliumChartStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if output, err := repoAddCmd.CombinedOutput(); err != nil {
		if !strings.Contains(string(output), "already exists") {
			result.MarkFailed(err, fmt.Sprintf("failed to add helm repo '%s'", ciliumChart.RepoName()))
			return result, err
		}
	}
	...
	if _, err := repoUpdateCmd.CombinedOutput(); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to update helm repo %s", ciliumChart.RepoName()))
		return result, err
	}
	...
	if _, err := pullCmd.CombinedOutput(); err != nil {
		result.MarkFailed(err, "failed to pull Cilium chart")
		return result, err
	}

	logger.Info("Cilium Helm chart has been downloaded successfully.")
	result.MarkCompleted("Cilium Helm chart downloaded successfully")
	return result, nil
}
