...
func NewInstallFlannelHelmChartStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallFlannelHelmChartStepBuilder {
...
func (s *InstallFlannelHelmChartStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to install/upgrade Flannel Helm chart")
		return result, err
	}

	logger.Info("Flannel Helm chart installed/upgraded successfully.")
	logger.Debugf("Helm command output:\n%s", output)
	result.MarkCompleted("Flannel Helm chart installed/upgraded successfully")
	return result, nil
}
