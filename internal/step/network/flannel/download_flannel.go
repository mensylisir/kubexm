...
func NewDownloadFlannelChartStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadFlannelChartStepBuilder {
...
func (s *DownloadFlannelChartStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if output, err := repoAddCmd.CombinedOutput(); err != nil {
		if !strings.Contains(string(output), "already exists") {
			result.MarkFailed(err, fmt.Sprintf("failed to add helm repo '%s'", flannelChart.RepoName()))
			return result, err
		}
	}
	...
	if _, err := repoUpdateCmd.CombinedOutput(); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to update helm repo %s", flannelChart.RepoName()))
		return result, err
	}
	...
	if _, err := pullCmd.CombinedOutput(); err != nil {
		result.MarkFailed(err, "failed to pull Flannel chart")
		return result, err
	}

	logger.Info("Flannel Helm chart has been downloaded successfully.")
	result.MarkCompleted("Flannel Helm chart downloaded successfully")
	return result, nil
}
