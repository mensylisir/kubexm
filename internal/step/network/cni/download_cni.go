...
func (s *DownloadCNIPluginsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for arch := range requiredArchs {
		binaryInfo, err := provider.GetBinary(binary.ComponentKubeCNI, arch)
		if err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to get CNI plugins info for arch %s", arch))
			return result, err
		}
		if binaryInfo == nil {
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to download CNI plugins for arch %s", arch))
			return result, err
		}
	}
