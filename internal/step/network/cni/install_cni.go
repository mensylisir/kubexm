...
func (s *InstallCNIPluginsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "disabled for arch") {
			logger.Infof("CNI plugins not required for this host (arch: %s), skipping.", ctx.GetHost().GetArch())
			result.MarkCompleted("CNI plugins not required for this host, skipping")
			return result, nil
		}
	...
	if _, err := os.Stat(localExtractedPath); os.IsNotExist(err) {
		result.MarkFailed(err, fmt.Sprintf("local CNI extracted path '%s' not found", localExtractedPath))
		return result, err
	}

	if err := runner.Mkdirp(ctx.GoContext(), conn, s.RemoteCNIBinDir, "0755", s.Sudo); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to create remote CNI bin directory '%s'", s.RemoteCNIBinDir))
		return result, err
	}
	...
	for _, pluginName := range s.knownCniPlugins {
		...
		if err := runner.Upload(ctx.GoContext(), conn, localPath, remoteTempPath, false); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to upload '%s'", localPath))
			return result, err
		}
		...
		if _, err := runner.Run(ctx.GoContext(), conn, moveCmd, s.Sudo); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to move CNI plugin '%s'", pluginName))
			return result, err
		}
		...
		if err := runner.Chmod(ctx.GoContext(), conn, remoteTargetPath, s.Permission, s.Sudo); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to set permission on CNI plugin '%s'", pluginName))
			return result, err
		}
	}
