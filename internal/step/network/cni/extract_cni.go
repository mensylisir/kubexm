...
func NewExtractCNIPluginsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ExtractCNIPluginsStepBuilder {
...
func (s *ExtractCNIPluginsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(requiredArchs) == 0 {
		...
		result.MarkCompleted("No hosts require CNI plugins, skipping extraction")
		return result, nil
	}

	for arch := range requiredArchs {
		if err := s.extractFileForArch(ctx, arch); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to extract CNI plugins for arch %s", arch))
			return result, err
		}
	}

	logger.Info("All required CNI plugins archives have been extracted successfully.")
	result.MarkCompleted("CNI plugins extracted successfully")
	return result, nil
}
