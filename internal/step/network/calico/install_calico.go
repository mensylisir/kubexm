...
func NewInstallCalicoHelmChartStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallCalicoHelmChartStepBuilder {
...
func (s *InstallCalicoHelmChartStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to install/upgrade Calico Helm chart")
		return result, err
	}

	logger.Info("Calico Helm chart installed/upgraded successfully.")
	logger.Debugf("Helm command output:\n%s", runResult.Stdout)
	result.MarkCompleted("Calico Helm chart installed/upgraded successfully")
	return result, nil
}
