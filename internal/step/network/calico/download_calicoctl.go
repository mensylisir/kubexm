...
func NewDownloadCalicoctlStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadCalicoctlStepBuilder {
...
func (s *DownloadCalicoctlStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(requiredArchs) == 0 {
		...
		result.MarkCompleted("No hosts require calicoctl, skipping download")
		return result, nil
	}
	...
	for arch := range requiredArchs {
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to download calicoctl for arch %s", arch))
			return result, err
		}
	}

	logger.Info("All required calicoctl binaries have been downloaded successfully.")
	result.MarkCompleted("All calicoctl binaries downloaded successfully")
	return result, nil
}
