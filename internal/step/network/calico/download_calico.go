...
func NewDownloadCalicoChartStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadCalicoChartStepBuilder {
...
func (s *DownloadCalicoChartStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if calicoChart == nil {
		...
		result.MarkCompleted("Skipped: chart info is nil")
		return result, nil
	}
	...
	if output, err := repoAddCmd.CombinedOutput(); err != nil {
		if !strings.Contains(string(output), "already exists") {
			result.MarkFailed(err, fmt.Sprintf("failed to add helm repo '%s'", calicoChart.RepoName()))
			return result, err
		}
	}
	...
	if _, err := repoUpdateCmd.CombinedOutput(); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to update helm repo %s", calicoChart.RepoName()))
		return result, err
	}
	...
	if _, err := pullCmd.CombinedOutput(); err != nil {
		result.MarkFailed(err, "failed to pull Calico chart")
		return result, err
	}

	logger.Info("Calico Helm chart has been downloaded successfully.")
	result.MarkCompleted("Calico Helm chart downloaded successfully")
	return result, nil
}
