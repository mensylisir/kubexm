...
func NewGenerateKubeovnValuesStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateKubeovnValuesStepBuilder {
...
func (s *GenerateKubeovnValuesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "phase", "Run")

	valuesTemplateContent, err := templates.Get("cni/kubeovn/values.yaml.tmpl")
	if err != nil {
		result.MarkFailed(err, "failed to get embedded kube-ovn values.yaml.tmpl")
		return result, err
	}

	tmpl, err := template.New("kubeovnValues").Parse(valuesTemplateContent)
	if err != nil {
		result.MarkFailed(err, "failed to parse kube-ovn values.yaml.tmpl")
		return result, err
	}
	var valuesBuffer bytes.Buffer
	if err := tmpl.Execute(&valuesBuffer, s); err != nil {
		result.MarkFailed(err, "failed to render kube-ovn values.yaml.tmpl")
		return result, err
	}
	...
	result.MarkCompleted("Kube-OVN Helm values file generated successfully")
	return result, nil
}
