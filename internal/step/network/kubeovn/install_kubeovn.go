...
func NewInstallKubeOvnHelmChartStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallKubeOvnHelmChartStepBuilder {
	...
	remoteDir := filepath.Join(ctx.GetUploadDir(), kubeovnChart.RepoName(), kubeovnChart.Version)

	valuesFileName := fmt.Sprintf("%s-values.yaml", kubeovnChart.RepoName())
...
func (s *InstallKubeOvnHelmChartStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	logger.Debug("All required Kube-OVN artifacts found on remote host.")
...
func (s *InstallKubeOvnHelmChartStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to install/upgrade Kube-OVN Helm chart")
		return result, err
	}

	logger.Info("Kube-OVN Helm chart installed/upgraded successfully.")
	logger.Debugf("Helm command output:\n%s", runResult.Stdout)
	result.MarkCompleted("Kube-OVN Helm chart installed/upgraded successfully")
	return result, nil
}
...
func (s *InstallKubeOvnHelmChartStep) Rollback(ctx runtime.ExecutionContext) error {
	...
	if _, err := runner.Run(ctx.GoContext(), conn, cmd, s.Sudo); err != nil {
		logger.Errorf("Failed to uninstall Kube-OVN Helm release (this is expected if installation failed): %v", err)
	} else {
