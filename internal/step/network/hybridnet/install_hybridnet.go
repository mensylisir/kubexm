...
func NewInstallHybridnetHelmChartStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallHybridnetHelmChartStepBuilder {
	...
	remoteDir := filepath.Join(ctx.GetUploadDir(), hybridnetChart.RepoName(), hybridnetChart.Version)

	valuesFileName := fmt.Sprintf("%s-values.yaml", hybridnetChart.RepoName())
...
func (s *InstallHybridnetHelmChartStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	logger.Debug("All required Hybridnet artifacts found on remote host.")
...
func (s *InstallHybridnetHelmChartStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to install/upgrade Hybridnet Helm chart")
		return result, err
	}

	logger.Info("Hybridnet Helm chart installed/upgraded successfully.")
	logger.Debugf("Helm command output:\n%s", runResult.Stdout)
	result.MarkCompleted("Hybridnet Helm chart installed/upgraded successfully")
	return result, nil
}
...
func (s *InstallHybridnetHelmChartStep) Rollback(ctx runtime.ExecutionContext) error {
	...
	if _, err := runner.Run(ctx.GoContext(), conn, cmd, s.Sudo); err != nil {
		logger.Errorf("Failed to uninstall Hybridnet Helm release (this is expected if installation failed): %v", err)
	} else {
