...
func NewGenerateHybridnetValuesStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateHybridnetValuesStepBuilder {
...
func (s *GenerateHybridnetValuesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "phase", "Run")

	valuesTemplateContent, err := templates.Get("cni/hybridnet/values.yaml.tmpl")
	if err != nil {
		result.MarkFailed(err, "failed to get embedded hybridnet values.yaml.tmpl")
		return result, err
	}

	tmpl, err := template.New("hybridnetValues").Parse(valuesTemplateContent)
	if err != nil {
		result.MarkFailed(err, "failed to parse hybridnet values.yaml.tmpl")
		return result, err
	}
	var valuesBuffer bytes.Buffer
	if err := tmpl.Execute(&valuesBuffer, s); err != nil {
		result.MarkFailed(err, "failed to render hybridnet values.yaml.tmpl")
		return result, err
	}
	...
	if err := os.WriteFile(localPath, valuesBuffer.Bytes(), 0644); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to write generated values file to %s", localPath))
		return result, err
	}

	logger.Info("Successfully generated Hybridnet Helm values file.")
	result.MarkCompleted("Hybridnet Helm values file generated successfully")
	return result, nil
}
