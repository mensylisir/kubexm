...
func NewGenerateMultusValuesStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateMultusValuesStepBuilder {
...
func (s *GenerateMultusValuesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "phase", "Run")

	valuesTemplateContent, err := templates.Get("cni/multus/values.yaml.tmpl")
	if err != nil {
		result.MarkFailed(err, "failed to get embedded multus values.yaml.tmpl")
		return result, err
	}

	tmpl, err := template.New("multusValues").Parse(valuesTemplateContent)
	if err != nil {
		result.MarkFailed(err, "failed to parse multus values.yaml.tmpl")
		return result, err
	}
	var valuesBuffer bytes.Buffer
	if err := tmpl.Execute(&valuesBuffer, s); err != nil {
		result.MarkFailed(err, "failed to render multus values.yaml.tmpl")
		return result, err
	}
	...
	result.MarkCompleted("Multus Helm values file generated successfully")
	return result, nil
}
