...
func NewInstallMultusHelmChartStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallMultusHelmChartStepBuilder {
...
func (s *InstallMultusHelmChartStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to install/upgrade Multus CNI Helm chart")
		return result, err
	}

	logger.Info("Multus CNI Helm chart installed/upgraded successfully.")
	logger.Debugf("Helm command output:\n%s", output)
	result.MarkCompleted("Multus CNI Helm chart installed/upgraded successfully")
	return result, nil
}
