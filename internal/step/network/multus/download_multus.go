...
func NewDownloadMultusChartStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadMultusChartStepBuilder {
...
func (s *DownloadMultusChartStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		...
		result.MarkCompleted("Skipped - chart info unavailable")
		return result, nil
	}
	...
	if output, err := repoAddCmd.CombinedOutput(); err != nil {
		if !strings.Contains(string(output), "already exists") {
			result.MarkFailed(err, fmt.Sprintf("failed to add helm repo '%s'", multusChart.RepoName()))
			return result, err
		}
	}
	...
	if _, err := repoUpdateCmd.CombinedOutput(); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to update helm repo %s", multusChart.RepoName()))
		return result, err
	}
	...
	if _, err := pullCmd.CombinedOutput(); err != nil {
		result.MarkFailed(err, "failed to pull Multus chart")
		return result, err
	}

	logger.Info("Multus Helm chart has been downloaded successfully.")
	result.MarkCompleted("Multus Helm chart downloaded successfully")
	return result, nil
}
