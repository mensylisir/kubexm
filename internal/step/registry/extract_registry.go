...
func NewExtractRegistryStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ExtractRegistryStepBuilder {
...
func (s *ExtractRegistryStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(requiredArchs) == 0 {
		...
		result.MarkCompleted("no extraction required")
		return result, nil
	}

	if err := os.MkdirAll(ctx.GetExtractDir(), 0755); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to create global extract directory '%s'", ctx.GetExtractDir()))
		return result, err
	}

	for arch := range requiredArchs {
		if err := s.extractFileForArch(ctx, arch); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to extract archive for arch %s", arch))
			return result, err
		}
	}

	logger.Info("All required registry archives have been extracted successfully.")
	result.MarkCompleted("registry archives extracted successfully")
	return result, nil
}
