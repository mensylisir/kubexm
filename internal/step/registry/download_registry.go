...
func NewDownloadRegistryStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadRegistryStepBuilder {
...
func (s *DownloadRegistryStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(requiredArchs) == 0 {
		result.MarkCompleted("no registry binaries required")
		return result, nil
	}
	provider := binary.NewBinaryProvider(ctx)
	for arch := range requiredArchs {
		...
		if _, err := os.Stat(destPath); err == nil {
			if match, _ := helpers.VerifyLocalFileChecksum(destPath, binaryInfo.Checksum()); match {
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to download registry for arch %s", arch))
			return result, err
		}
	}
	logger.Info("All required registry binaries have been downloaded successfully.")
	result.MarkCompleted("registry binaries downloaded successfully")
	return result, nil
}
