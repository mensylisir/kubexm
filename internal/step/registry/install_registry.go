...
func NewInstallRegistryStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallRegistryStepBuilder {
...
func (s *InstallRegistryStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "disabled for arch") {
			logger.Infof("Registry not required for this host (arch: %s), skipping.", ctx.GetHost().GetArch())
			result.MarkCompleted("registry not required for this host")
			return result, nil
		}
	...
	if _, err := os.Stat(localSourcePath); os.IsNotExist(err) {
		result.MarkFailed(err, fmt.Sprintf("local source file '%s' not found", localSourcePath))
		return result, err
	}
	...
	result.MarkCompleted("registry installed successfully")
	return result, nil
}
