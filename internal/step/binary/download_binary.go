...
func NewDownloadBinariesStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadBinariesStepBuilder {
...
func (s *DownloadBinariesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for arch := range requiredArchs {
		...
		if err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to get binaries list for arch %s", arch))
			return result, err
		}
	...
	if len(binariesToDownload) == 0 {
		...
		result.MarkCompleted("no binaries needed")
		return result, nil
	}
	...
	if len(allErrors) > 0 {
		failErr := fmt.Errorf("failed to download some binaries:\n- %s", strings.Join(allErrors, "\n- "))
		result.MarkFailed(failErr, "some binaries failed to download")
		return result, failErr
	}

	logger.Info("All required binaries have been downloaded successfully.")
	result.MarkCompleted("all binaries downloaded successfully")
	return result, nil
}
