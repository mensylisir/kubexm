...
func NewInstallDockerStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallDockerStepBuilder {
...
func (s *InstallDockerStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "disabled for arch") {
			logger.Infof("Docker not required for this host (arch: %s), skipping.", ctx.GetHost().GetArch())
			return true, nil
		}
		return false, err
	}
	...
	if allDone {
		logger.Info("All required Docker files already exist and are up-to-date. Step is done.")
	}

	return allDone, nil
}

func (s *InstallDockerStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if _, err := os.Stat(localSourceDir); os.IsNotExist(err) {
		result.MarkFailed(err, "local Docker extracted path not found")
		return result, fmt.Errorf("local Docker extracted path '%s' not found, ensure extract step ran successfully", localSourceDir)
	}
	...
	for _, fileName := range files {
		...
		if err := runner.Upload(ctx.GoContext(), conn, localSourcePath, remoteTempPath, false); err != nil {
			result.MarkFailed(err, "failed to upload Docker binary")
			return result, fmt.Errorf("failed to upload '%s': %w", localSourcePath, err)
		}
		...
		if _, err := runner.Run(ctx.GoContext(), conn, moveCmd, s.Sudo); err != nil {
			result.MarkFailed(err, "failed to move Docker binary")
			return result, fmt.Errorf("failed to move file to '%s': %w", remoteTargetPath, err)
		}
		...
		if err := runner.Chmod(ctx.GoContext(), conn, remoteTargetPath, s.Permission, s.Sudo); err != nil {
			...
			return result, fmt.Errorf("failed to set permission on '%s': %w", remoteTargetPath, err)
		}
	}

	logger.Info("Successfully installed all Docker binaries.")
	result.MarkCompleted("Docker binaries installed successfully")
	return result, nil
}
