...
func NewSetupDockerServiceStepBuilder(ctx runtime.ExecutionContext, instanceName string) *SetupDockerServiceStepBuilder {
...
func (s *SetupDockerServiceStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to write docker.service")
		return result, fmt.Errorf("failed to write docker.service to %s: %w", s.ServiceFilePath, err)
	}

	logger.Info("Successfully wrote Docker service file. Running 'systemctl daemon-reload' is required next.")
	result.MarkCompleted("Docker service configured successfully")
	return result, nil
}
