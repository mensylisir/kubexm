...
func NewSetupDockerDropInStepBuilder(ctx runtime.ExecutionContext, instanceName string) *SetupDockerDropInStepBuilder {
...
func (s *SetupDockerDropInStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, dropInDir, "0755", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to create systemd drop-in directory")
		return result, fmt.Errorf("failed to create systemd drop-in directory %s: %w", dropInDir, err)
	}
	...
	if err != nil {
		result.MarkFailed(err, "failed to write Docker drop-in file")
		return result, fmt.Errorf("failed to write Docker drop-in file to %s: %w", s.DropInFilePath, err)
	}

	logger.Info("Successfully wrote Docker drop-in file. Running 'systemctl daemon-reload' is required next.")
	result.MarkCompleted("Docker drop-in file configured successfully")
	return result, nil
}
