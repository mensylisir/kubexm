...
func NewRemoveCriDockerdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *RemoveCriDockerdStepBuilder {
...
func (s *RemoveCriDockerdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(removalErrors) > 0 {
		result.MarkFailed(err, "encountered errors during cri-dockerd cleanup")
		return result, fmt.Errorf("encountered %d error(s) during cri-dockerd cleanup, manual review may be required", len(removalErrors))
	}

	result.MarkCompleted("cri-dockerd cleanup completed successfully")
	return result, nil
}
