...
func NewCleanDockerStepBuilder(ctx runtime.ExecutionContext, instanceName string) *CleanDockerStepBuilder {
	s := &CleanDockerStep{
...
func (s *CleanDockerStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(removalErrors) > 0 {
		result.MarkFailed(err, "encountered errors during Docker cleanup")
		return result, fmt.Errorf("encountered %d error(s) during Docker cleanup, manual review may be required", len(removalErrors))
	}

	logger.Info("Docker and containerd cleanup completed successfully.")
	result.MarkCompleted("Docker cleanup completed successfully")
	return result, nil
}
