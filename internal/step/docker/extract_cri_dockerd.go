...
func NewExtractCriDockerdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ExtractCriDockerdStepBuilder {
...
func (s *ExtractCriDockerdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(requiredArchs) == 0 {
		...
		result.MarkCompleted("skipping - no cri-dockerd required")
		return result, nil
	}

	for arch := range requiredArchs {
		if err := s.extractFileForArch(ctx, arch); err != nil {
			result.MarkFailed(err, "failed to extract cri-dockerd archive")
			return result, err
		}
	}

	logger.Info("All required cri-dockerd archives have been extracted successfully.")
	result.MarkCompleted("cri-dockerd archives extracted successfully")
	return result, nil
}
