...
func NewInstallCriDockerdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallCriDockerdStepBuilder {
...
func (s *InstallCriDockerdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Upload(ctx.GoContext(), conn, localSourcePath, remoteTempPath, false); err != nil {
		result.MarkFailed(err, "failed to upload cri-dockerd")
	...
	if _, err := runner.Run(ctx.GoContext(), conn, moveCmd, s.Sudo); err != nil {
		result.MarkFailed(err, "failed to move cri-dockerd")
	...
	result.MarkCompleted("cri-dockerd installed successfully")
	return result, nil
}
