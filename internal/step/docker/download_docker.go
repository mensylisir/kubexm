...
type DownloadDockerStepBuilder struct {
	step.Builder[DownloadDockerStepBuilder, *DownloadDockerStep]
}

func NewDownloadDockerStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadDockerStepBuilder {
	...
	s := &DownloadDockerStep{}
	s.Base.Meta.Name = instanceName
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Download Docker binaries for all required architectures", s.Base.Meta.Name)
	s.Base.Sudo = false
...
func (s *DownloadDockerStep) Meta() *spec.StepMeta {
	return &s.Base.Meta
}

func (s *DownloadDockerStep) getRequiredArchs(ctx runtime.ExecutionContext) (map[string]bool, error) {
...
func (s *DownloadDockerStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if len(requiredArchs) == 0 {
		logger.Info("No hosts require Docker in this cluster configuration. Step is done.")
		return true, nil
	}
	...
	for arch := range requiredArchs {
		...
		if err != nil {
			return false, fmt.Errorf("failed to get Docker info for arch %s: %w", arch, err)
		}
		if binaryInfo == nil {
			continue
		}
		...
		logger.Infof("Checking for Docker (arch: %s) at: %s", arch, destPath)
	...
	if allDone {
		logger.Info("All required Docker binaries for all architectures already exist and are valid.")
		return true, nil
	}
	return false, nil
}

func (s *DownloadDockerStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(requiredArchs) == 0 {
		logger.Info("No hosts require Docker in this cluster configuration. Skipping download.")
		result.MarkCompleted("skipping - no hosts require Docker")
		return result, nil
	}
	...
	for arch := range requiredArchs {
		binaryInfo, err := provider.GetBinary(binary.ComponentDocker, arch)
		if err != nil {
			result.MarkFailed(err, "failed to get Docker info")
			return result, fmt.Errorf("failed to get Docker info for arch %s: %w", arch, err)
		}
		if binaryInfo == nil {
			continue
		}
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			result.MarkFailed(err, "failed to download Docker")
			return result, fmt.Errorf("failed to download Docker for arch %s: %w", arch, err)
		}
	}

	logger.Info("All required Docker binaries have been downloaded successfully.")
	result.MarkCompleted("Docker downloaded successfully")
	return result, nil
}

func (s *DownloadDockerStep) downloadFile(ctx runtime.ExecutionContext, binaryInfo *binary.Binary) error {
	...
	logger.Infof("Downloading Docker (arch: %s, version: %s) from %s ...", binaryInfo.Arch, binaryInfo.Version, binaryInfo.URL())
...
func (s *DownloadDockerStep) Rollback(ctx runtime.ExecutionContext) error {
...
var _ step.Step = (*DownloadDockerStep)(nil)
