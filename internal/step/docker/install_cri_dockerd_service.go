...
func NewSetupCriDockerdServiceStepBuilder(ctx runtime.ExecutionContext, instanceName string) *SetupCriDockerdServiceStepBuilder {
...
func (s *SetupCriDockerdServiceStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := helpers.WriteContentToRemote(ctx, conn, serviceContent, s.ServiceFilePath, "0644", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to write cri-dockerd.service")
		return result, fmt.Errorf("failed to write cri-dockerd.service: %w", err)
	}

	logger.Info("Successfully wrote cri-dockerd.service file. 'systemctl daemon-reload' is required to apply changes.")
	result.MarkCompleted("cri-dockerd.service configured successfully")
	return result, nil
}
