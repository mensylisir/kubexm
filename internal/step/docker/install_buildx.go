...
func NewInstallBuildxStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallBuildxStepBuilder {
	s := &InstallBuildxStep{
...
func (s *InstallBuildxStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Upload(ctx.GoContext(), conn, localSourcePath, remoteTempPath, false); err != nil {
		result.MarkFailed(err, "failed to upload buildx")
	...
	if _, err := runner.Run(ctx.GoContext(), conn, moveCmd, s.Sudo); err != nil {
		result.MarkFailed(err, "failed to move buildx")
		return result, fmt.Errorf("failed to move file to '%s': %w", s.InstallPath, err)
	}
	...
	if err := runner.Chmod(ctx.GoContext(), conn, s.InstallPath, s.Permission, s.Sudo); err != nil {
		...
		return result, fmt.Errorf("failed to set permission on '%s': %w", s.InstallPath, err)
	}

	logger.Infof("Successfully installed docker-buildx to %s", s.InstallPath)
	result.MarkCompleted("docker-buildx installed successfully")
	return result, nil
}
