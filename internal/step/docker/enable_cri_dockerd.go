...
func NewEnableCriDockerdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *EnableCriDockerdStepBuilder {
...
func (s *EnableCriDockerdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.EnableService(ctx.GoContext(), conn, facts, CriDockerdServiceName); err != nil {
		result.MarkFailed(err, "failed to enable cri-dockerd service")
		return result, fmt.Errorf("failed to enable cri-dockerd service: %w", err)
	}

	logger.Info("CriDockerd service enabled successfully.")
	result.MarkCompleted("cri-dockerd service enabled successfully")
	return result, nil
}
