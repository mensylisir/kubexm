...
func NewSetupCriDockerdSocketStepBuilder(ctx runtime.ExecutionContext, instanceName string) *SetupCriDockerdSocketStepBuilder {
...
func (s *SetupCriDockerdSocketStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to render socket file")
		return result, err
	}
	...
	if err := helpers.WriteContentToRemote(ctx, conn, socketContent, s.SocketFilePath, "0644", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to write cri-dockerd.socket")
		return result, fmt.Errorf("failed to write cri-dockerd.socket: %w", err)
	}

	logger.Info("Successfully wrote cri-dockerd.socket file. 'systemctl daemon-reload' is required to apply changes.")
	result.MarkCompleted("cri-dockerd.socket configured successfully")
	return result, nil
}
