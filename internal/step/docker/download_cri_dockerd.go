...
func NewDownloadCriDockerdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadCriDockerdStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Download cri-dockerd binaries for all required architectures", s.Base.Meta.Name)
	s.Base.Sudo = false
...
func (s *DownloadCriDockerdStep) getRequiredArchs(ctx runtime.ExecutionContext) (map[string]bool, error) {
...
func (s *DownloadCriDockerdStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if len(requiredArchs) == 0 {
		logger.Info("No hosts require cri-dockerd in this cluster configuration. Step is done.")
		return true, nil
	}
	...
	for arch := range requiredArchs {
		...
		if err != nil {
			return false, fmt.Errorf("failed to get cri-dockerd info for arch %s: %w", arch, err)
		}
		if binaryInfo == nil {
			continue
		}
		...
		logger.Infof("Checking for cri-dockerd (arch: %s) at: %s", arch, destPath)
	...
	if allDone {
		logger.Info("All required cri-dockerd binaries for all architectures already exist and are valid.")
		return true, nil
	}
	return false, nil
}

func (s *DownloadCriDockerdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(requiredArchs) == 0 {
		logger.Info("No hosts require cri-dockerd in this cluster configuration. Skipping download.")
		result.MarkCompleted("skipping - no hosts require cri-dockerd")
		return result, nil
	}
	...
	for arch := range requiredArchs {
		binaryInfo, err := provider.GetBinary(binary.ComponentCriDockerd, arch)
		if err != nil {
			result.MarkFailed(err, "failed to get cri-dockerd info")
			return result, fmt.Errorf("failed to get cri-dockerd info for arch %s: %w", arch, err)
		}
		if binaryInfo == nil {
			continue
		}
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			result.MarkFailed(err, "failed to download cri-dockerd")
			return result, fmt.Errorf("failed to download cri-dockerd for arch %s: %w", arch, err)
		}
	}

	logger.Info("All required cri-dockerd binaries have been downloaded successfully.")
	result.MarkCompleted("cri-dockerd downloaded successfully")
	return result, nil
}

func (s *DownloadCriDockerdStep) downloadFile(ctx runtime.ExecutionContext, binaryInfo *binary.Binary) error {
	...
	logger.Infof("Downloading cri-dockerd (arch: %s, version: %s) from %s ...", binaryInfo.Arch, binaryInfo.Version, binaryInfo.URL())
...
var _ step.Step = (*DownloadCriDockerdStep)(nil)
