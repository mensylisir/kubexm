...
func NewRemoveDockerStepBuilder(ctx runtime.ExecutionContext, instanceName string) *RemoveDockerStepBuilder {
	s := &RemoveDockerStep{
		FilesToRemove: []string{
			"containerd",
...
func (s *RemoveDockerStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(removalErrors) > 0 {
		...
		return result, fmt.Errorf("encountered %d error(s) during Docker cleanup", len(removalErrors))
	}
