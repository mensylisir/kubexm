...
func NewExtractDockerStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ExtractDockerStepBuilder {
...
func (s *ExtractDockerStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(requiredArchs) == 0 {
		...
		result.MarkCompleted("skipping - no Docker required")
		return result, nil
	}

	for arch := range requiredArchs {
		if err := s.extractFileForArch(ctx, arch); err != nil {
			result.MarkFailed(err, "failed to extract Docker archive")
			return result, err
		}
	}

	logger.Info("All required Docker archives have been extracted successfully.")
	result.MarkCompleted("Docker archives extracted successfully")
	return result, nil
}
