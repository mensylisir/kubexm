...
func NewInstallCriCtlStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallCriCtlStepBuilder {
...
func (s *InstallCriCtlStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "disabled for arch") {
			logger.Infof("crictl not required for this host (arch: %s), skipping.", ctx.GetHost().GetArch())
			result.MarkCompleted("skipping - crictl not required for this arch")
			return result, nil
		}
	...
	if _, err := os.Stat(localSourcePath); os.IsNotExist(err) {
		...
		return result, fmt.Errorf("local source file '%s' not found, ensure extract step ran successfully", localSourcePath)
	}
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, common.DefaultBinDir, "0755", s.Sudo); err != nil {
		...
		return result, fmt.Errorf("failed to create remote install directory '%s': %w", common.DefaultBinDir, err)
	}
	...
	if err := runner.Upload(ctx.GoContext(), conn, localSourcePath, remoteTempPath, false); err != nil {
		result.MarkFailed(err, "failed to upload crictl")
	...
	if _, err := runner.Run(ctx.GoContext(), conn, moveCmd, s.Sudo); err != nil {
		result.MarkFailed(err, "failed to move crictl")
		return result, fmt.Errorf("failed to move file to '%s': %w", s.RemoteCriCtlTargetPath, err)
	}
	...
	if err := runner.Chmod(ctx.GoContext(), conn, s.RemoteCriCtlTargetPath, s.CrictlPermissions, s.Sudo); err != nil {
		...
		return result, fmt.Errorf("failed to set permission on '%s': %w", s.RemoteCriCtlTargetPath, err)
	}

	logger.Info("crictl binary has been installed successfully.")
	result.MarkCompleted("crictl installed successfully")
	return result, nil
}
