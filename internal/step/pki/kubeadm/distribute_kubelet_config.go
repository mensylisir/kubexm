...
func NewKubeadmDistributeKubeletConfigStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmDistributeKubeletConfigStepBuilder {
...
func (s *KubeadmDistributeKubeletConfigStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Upload(ctx.GoContext(), conn, localCaPath, remoteCaPath, s.Sudo); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to upload ca.crt for node %s", nodeName))
		return result, err
	}
	...
	if err := runner.Upload(ctx.GoContext(), conn, localKubeletConfPath, remoteKubeletConfPath, s.Sudo); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to upload kubelet.conf for node %s", nodeName))
		return result, err
	}

	logger.Info("Successfully distributed new ca.crt and kubelet.conf.")
	result.MarkCompleted("kubelet config distributed successfully")
	return result, nil
}
