...
func NewKubeadmBackupKubeconfigsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmBackupKubeconfigsStepBuilder {
...
func (s *KubeadmBackupKubeconfigsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if _, err := runner.Run(ctx.GoContext(), conn, backupCmd, s.Sudo); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to back up remote directory on host '%s'", ctx.GetHost().GetName()))
		return result, err
	}
	...
	result.MarkCompleted("kubeconfigs backed up successfully")
	return result, nil
}
