...
func NewKubeadmDistributeStackedEtcdLeafCertsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmDistributeStackedEtcdLeafCertsStepBuilder {
...
func (s *KubeadmDistributeStackedEtcdLeafCertsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for localFileName, remoteFileName := range filesToDistribute {
		...
		if err := runner.Upload(ctx.GoContext(), conn, localPath, remotePath, s.Sudo); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to upload '%s'", localFileName))
			return result, err
		}
	}

	logger.Info("Successfully distributed renewed stacked Etcd leaf certificates.")
	result.MarkCompleted("stacked etcd leaf certs distributed successfully")
	return result, nil
}
