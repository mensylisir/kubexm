...
type KubeadmRestartKubeletStep struct {
	step.Base
	ServiceName    string
	postWaitSettle time.Duration
}

type KubeadmRestartKubeletStepBuilder struct {
	step.Builder[KubeadmRestartKubeletStepBuilder, *KubeadmRestartKubeletStep]
}

func NewKubeadmRestartKubeletStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmRestartKubeletStepBuilder {
	s := &KubeadmRestartKubeletStep{
	...
	b := new(KubeadmRestartKubeletStepBuilder).Init(s)
	return b
}

func (s *KubeadmRestartKubeletStep) Meta() *spec.StepMeta {
	return &s.Base.Meta
}

func (s *KubeadmRestartKubeletStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
...
func (s *KubeadmRestartKubeletStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.RestartService(ctx.GoContext(), conn, facts, s.ServiceName); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to restart service '%s' on host %s", s.ServiceName, ctx.GetHost().GetName()))
		return result, err
	}
	...
	result.MarkCompleted("kubelet service restarted successfully")
	return result, nil
}

func (s *KubeadmRestartKubeletStep) Rollback(ctx runtime.ExecutionContext) error {
...
var _ step.Step = (*KubeadmRestartKubeletStep)(nil)
