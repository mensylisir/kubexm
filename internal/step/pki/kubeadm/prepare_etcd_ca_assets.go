...
func NewKubeadmPrepareStackedEtcdCAAssetsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmPrepareStackedEtcdCAAssetsStepBuilder {
...
func (s *KubeadmPrepareStackedEtcdCAAssetsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := os.MkdirAll(filepath.Dir(destOldCert), 0755); err != nil {
		result.MarkFailed(err, "failed to create 'certs-old/etcd' subdirectory")
		return result, err
	}
	if err := os.MkdirAll(filepath.Dir(destNewKey), 0755); err != nil {
		result.MarkFailed(err, "failed to create 'certs-new/etcd' subdirectory")
		return result, err
	}
	...
	if err := helpers.CopyFile(sourceCert, destOldCert); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to copy '%s' to backup directory", s.etcdCaAsset.CertFile))
		return result, err
	}
	if err := helpers.CopyFile(sourceKey, destOldKey); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to copy '%s' to backup directory", s.etcdCaAsset.KeyFile))
		return result, err
	}

	log.Info("Staging stacked Etcd CA private key for renewal step...")
	if err := helpers.CopyFile(sourceKey, destNewKey); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to stage key '%s' to new assets directory", s.etcdCaAsset.KeyFile))
		return result, err
	}

	logger.Info("Successfully prepared stacked Etcd CA assets.")
	result.MarkCompleted("stacked etcd CA assets prepared successfully")
	return result, nil
}
