...
type KubeadmCheckLeafCertsExpirationStep struct {
	step.Base
...
type KubeadmCheckLeafCertsExpirationStepBuilder struct {
	step.Builder[KubeadmCheckLeafCertsExpirationStepBuilder, *KubeadmCheckLeafCertsExpirationStep]
}

func NewKubeadmCheckLeafCertsExpirationStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmCheckLeafCertsExpirationStepBuilder {
	certsToCheck := []string{
		"apiserver.crt",
	...
	s := &KubeadmCheckLeafCertsExpirationStep{
	...
	b := new(KubeadmCheckLeafCertsExpirationStepBuilder).Init(s)
	return b
}

func (s *KubeadmCheckLeafCertsExpirationStep) Meta() *spec.StepMeta {
	return &s.Base.Meta
}

func (s *KubeadmCheckLeafCertsExpirationStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "host", ctx.GetHost().GetName(), "phase", "Precheck")
	logger.Info("Starting precheck for Kubernetes leaf certificate expiration on remote node.")
	return false, nil
}

func (s *KubeadmCheckLeafCertsExpirationStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for _, certFile := range s.leafCerts {
		...
		if err != nil {
			...
			result.MarkFailed(err, fmt.Sprintf("failed to read critical leaf certificate '%s'", remotePath))
			return result, err
		}
		...
		if pemBlock == nil {
			...
			result.MarkFailed(fmt.Errorf("failed to decode PEM block"), fmt.Sprintf("failed to decode PEM block from '%s'", remotePath))
			return result, fmt.Errorf("failed to decode PEM block from '%s'", remotePath)
		}

		cert, err := x509.ParseCertificate(pemBlock.Bytes)
		if err != nil {
			log.Errorf("Failed to parse critical leaf certificate. Error: %v", err)
			result.MarkFailed(err, fmt.Sprintf("failed to parse critical leaf certificate from '%s'", remotePath))
			return result, err
		}
	...
	result.MarkCompleted("leaf certificate expiration check completed")
	return result, nil
}

func (s *KubeadmCheckLeafCertsExpirationStep) Rollback(ctx runtime.ExecutionContext) error {
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "host", ctx.GetHost().GetName(), "phase", "Rollback")
	logger.Info("Rollback is not applicable for a check-only step. Nothing to do.")
	return nil
}

var _ step.Step = (*KubeadmCheckLeafCertsExpirationStep)(nil)
