...
func NewKubeadmRestartControlPlaneStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmRestartControlPlaneStepBuilder {
...
func (s *KubeadmRestartControlPlaneStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for _, component := range s.components {
		...
		if _, err := runner.Run(ctx.GoContext(), conn, moveOutCmd, s.Sudo); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to move manifest out for '%s' on host '%s'", component, ctx.GetHost().GetName()))
			return result, err
		}
		...
		if _, err := runner.Run(ctx.GoContext(), conn, moveInCmd, s.Sudo); err != nil {
			...
			result.MarkFailed(err, fmt.Sprintf("failed to restore manifest for '%s' on host '%s'", component, ctx.GetHost().GetName()))
			return result, err
		}
	...
	result.MarkCompleted("control plane restarted successfully")
	return result, nil
}
