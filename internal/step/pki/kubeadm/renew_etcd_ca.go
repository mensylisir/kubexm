...
func NewKubeadmRenewStackedEtcdCAStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmRenewStackedEtcdCAStepBuilder {
...
func (s *KubeadmRenewStackedEtcdCAStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := generateNewCACert(keyPath, certPath, subject, s.validity); err != nil {
		...
		result.MarkFailed(err, "failed to generate new Etcd CA certificate")
		return result, err
	}

	logger.Info("Successfully generated new stacked Etcd CA certificate.")
	result.MarkCompleted("Etcd CA certificate renewed successfully")
	return result, nil
}
