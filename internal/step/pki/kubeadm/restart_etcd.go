...
func NewKubeadmRestartEtcdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmRestartEtcdStepBuilder {
...
func (s *KubeadmRestartEtcdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if _, err := runner.Run(ctx.GoContext(), conn, moveOutCmd, s.Sudo); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to move etcd manifest out on host '%s'", ctx.GetHost().GetName()))
		return result, err
	}
	...
	if _, err := runner.Run(ctx.GoContext(), conn, moveInCmd, s.Sudo); err != nil {
		...
		result.MarkFailed(err, fmt.Sprintf("failed to restore manifest for etcd on host '%s'", ctx.GetHost().GetName()))
		return result, err
	}
