...
func NewKubeadmDistributeKubeconfigsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmDistributeKubeconfigsStepBuilder {
...
func (s *KubeadmDistributeKubeconfigsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for _, confFile := range s.configsToDistribute {
		...
		if err := runner.Upload(ctx.GoContext(), conn, localPath, remotePath, s.Sudo); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to upload kubeconfig '%s'", confFile))
			return result, err
		}
	}
