...
func NewKubeadmCheckEtcdCAExpirationStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmCheckEtcdCAExpirationStepBuilder {
...
func (s *KubeadmCheckEtcdCAExpirationStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		...
		result.MarkFailed(err, fmt.Sprintf("failed to read stacked Etcd CA certificate file '%s'", fullPath))
		return result, err
	}
	...
	if pemBlock == nil {
		...
		result.MarkFailed(fmt.Errorf("failed to decode PEM block"), fmt.Sprintf("failed to decode PEM block from Etcd CA certificate file '%s'", fullPath))
		return result, fmt.Errorf("failed to decode PEM block from Etcd CA certificate file '%s'", fullPath)
	}

	cert, err := x509.ParseCertificate(pemBlock.Bytes)
	if err != nil {
		log.Errorf("Failed to parse Etcd CA certificate. Error: %v", err)
		result.MarkFailed(err, fmt.Sprintf("failed to parse Etcd CA certificate from '%s'", fullPath))
		return result, err
	}
	...
	if rawValue, ok := ctx.GetModuleCache().Get(cacheKey); ok {
		if val, isBool := rawValue.(bool); isBool {
			...
			result.MarkFailed(fmt.Errorf("cache corruption"), fmt.Sprintf("value for key '%s' is not a boolean", cacheKey))
			return result, fmt.Errorf("cache corruption: value for key '%s' is not a boolean", cacheKey)
		}
	}
	...
	result.MarkCompleted("Etcd CA expiration check completed")
	return result, nil
}
