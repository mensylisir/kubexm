...
import (
	...
	"github.com/mensylisir/kubexm/internal/step/helpers" // USING YOUR HELPERS
...
func NewKubeadmCreateKubeconfigsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmCreateKubeconfigsStepBuilder {
...
func (s *KubeadmCreateKubeconfigsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to load main CA")
		return result, err
	}

	serverURL := fmt.Sprintf("https://%s:%d", clusterSpec.ControlPlaneEndpoint.Domain, common.DefaultAPIServerPort)

	for _, cfg := range kubeconfigs {
		if err := s.generateKubeconfig(logger, cfg.fileName, cfg.baseName, cfg.config, serverURL, caCert, caKey); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to generate kubeconfig for '%s'", cfg.baseName))
			return result, err
		}
	}
	...
	for _, node := range ctx.GetHostsByRole("") {
		...
		if err := s.generateKubeconfig(logger, kubeletFileName, kubeletBaseName, kubeletConfig, serverURL, caCert, caKey); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to generate kubeconfig for '%s'", kubeletBaseName))
			return result, err
		}
	}

	logger.Info("All kubeconfig files created successfully.")
	result.MarkCompleted("kubeconfig files created successfully")
	return result, nil
}
