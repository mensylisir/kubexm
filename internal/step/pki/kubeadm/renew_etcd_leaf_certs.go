...
func NewKubeadmRenewStackedEtcdLeafCertsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmRenewStackedEtcdLeafCertsStepBuilder {
...
func (s *KubeadmRenewStackedEtcdLeafCertsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := helpers.NewSignedCertificate(s.outputDir, serverCertFile, serverKeyFile, serverCfg, caCert, caKey); err != nil {
		result.MarkFailed(err, "failed to generate etcd server certificate")
		return result, err
	}
	...
	if err := helpers.NewSignedCertificate(s.outputDir, peerCertFile, peerKeyFile, peerCfg, caCert, caKey); err != nil {
		result.MarkFailed(err, "failed to generate etcd peer certificate")
		return result, err
	}
	...
	if err := helpers.NewSignedCertificate(s.outputDir, healthCheckClientCertFile, healthCheckClientKeyFile, healthCheckClientCfg, caCert, caKey); err != nil {
		result.MarkFailed(err, "failed to generate etcd healthcheck-client certificate")
		return result, err
	}

	logger.Info("All stacked Etcd leaf certificates generated successfully.")
	result.MarkCompleted("Etcd leaf certificates generated successfully")
	return result, nil
}
