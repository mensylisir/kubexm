...
func NewKubeadmCheckStackedEtcdLeafCertsExpirationStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmCheckStackedEtcdLeafCertsExpirationStepBuilder {
...
func (s *KubeadmCheckStackedEtcdLeafCertsExpirationStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for _, certFile := range s.etcdLeafCerts {
		...
		if err != nil {
			...
			result.MarkFailed(err, fmt.Sprintf("failed to read critical stacked etcd leaf certificate '%s'", remotePath))
			return result, err
		}
		...
		if err != nil {
			...
			result.MarkFailed(err, fmt.Sprintf("failed to parse critical stacked etcd leaf certificate from '%s'", remotePath))
			return result, err
		}
	...
	result.MarkCompleted("Etcd leaf certs expiration check completed")
	return result, nil
}
