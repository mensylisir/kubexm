...
func (s *BackupRemoteEtcdCertsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if !exists || errors.IsNotFound(err) {
		...
		if err := ctx.GetRunner().Mkdirp(ctx.GoContext(), conn, s.BackupDir, "0755", s.Sudo); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to create backup directory '%s'", s.BackupDir))
			return result, err
		}
	}
	...
	for remoteFile := range s.remoteFilesMap {
		...
		if err := ctx.GetRunner().CopyFile(ctx.GoContext(), conn, remoteFile, backupFile, false, s.Sudo); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to scp file '%s' from remote node", remoteFile))
			return result, err
		}
	}
	...
	for remoteFile, remoteSum := range s.remoteFilesMap {
		...
		if err != nil || backupFileSum != remoteSum {
			result.MarkFailed(err, fmt.Sprintf("checksum mismatch for file '%s'. Remote: %s, Local: %s", filepath.Base(remoteFile), remoteSum, backupFileSum))
			return result, fmt.Errorf("checksum mismatch for file '%s'. Remote: %s, Local: %s", filepath.Base(remoteFile), remoteSum, backupFileSum)
		}
	}
	logger.Infof("Backup completed successfully. %d files are stored in '%s'.", len(s.remoteFilesMap), s.BackupDir)
	result.MarkCompleted("certificates backed up successfully")
	return result, nil
}
