...
func NewPrepareAssetsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *PrepareAssetsStepBuilder {
...
func (s *PrepareAssetsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := helpers.CopyFile(s.localCAPath, s.localOldCAPath); err != nil {
		result.MarkFailed(err, "failed to copy ca.pem to old assets directory")
		return result, err
	}
	if err := helpers.CopyFile(s.localCAKeyPath, s.localOldCAKeyPath); err != nil {
		result.MarkFailed(err, "failed to copy ca-key.pem to old assets directory")
		return result, err
	}
	...
	if err := helpers.CopyFile(s.localCAKeyPath, s.localNewCAKeyPath); err != nil {
		result.MarkFailed(err, "failed to copy ca-key.pem to new assets directory")
		return result, err
	}

	logger.Info("Successfully prepared old assets.")
	result.MarkCompleted("assets prepared successfully")
	return result, nil
}
