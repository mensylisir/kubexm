...
func NewResignCAStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ResignCAStepBuilder {
...
func (s *ResignCAStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to re-sign CA certificate")
		return result, err
	}

	logger.Info("Successfully re-signed CA certificate and saved to 'certs-new' directory.")
	result.MarkCompleted("CA certificate re-signed successfully")
	return result, nil
}
