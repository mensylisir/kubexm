...
func NewPrepareCATransitionStepBuilder(ctx runtime.ExecutionContext, instanceName string) *PrepareCATransitionStepBuilder {
...
func (s *PrepareCATransitionStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "host", ctx.GetHost().GetName(), "phase", "Run")
	oldCAData, err := os.ReadFile(s.localOldCAPath)
	if err != nil {
		result.MarkFailed(err, "failed to read ca-old.pem")
		return result, err
	}
	newCAData, err := os.ReadFile(s.localNewCAPath)
	if err != nil {
		result.MarkFailed(err, "failed to read ca-new.pem")
		return result, err
	}
	...
	if err := os.WriteFile(s.localBundlePath, bundleData, 0644); err != nil {
		result.MarkFailed(err, "failed to write ca-bundle.pem")
		return result, err
	}
	...
	if err := os.WriteFile(s.localCAPath, bundleData, 0644); err != nil {
		result.MarkFailed(err, "failed to write bundle content to ca.pem")
		return result, err
	}

	logger.Info("CA bundle created and activated successfully.")
	result.MarkCompleted("CA bundle created and activated successfully")
	return result, nil
}
