...
func NewGenerateNewLeafCertsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateNewLeafCertsStepBuilder {
...
func (s *GenerateNewLeafCertsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for _, node := range s.etcdNodes {
		...
		if err := helpers.NewSignedCertificate(s.outputDir, adminCertFile, adminKeyFile, adminCfg, caCert, caKey); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to generate etcd admin certificate for node %s", nodeName))
			return result, err
		}
		...
		if err := helpers.NewSignedCertificate(s.outputDir, nodeCertFile, nodeKeyFile, nodeCfg, caCert, caKey); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to generate etcd node certificate for node %s", nodeName))
			return result, err
		}
		...
		if err := helpers.NewSignedCertificate(s.outputDir, memberCertFile, memberKeyFile, memberCfg, caCert, caKey); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to generate etcd member certificate for node %s", nodeName))
			return result, err
		}
	}
