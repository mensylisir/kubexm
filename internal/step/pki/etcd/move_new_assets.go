...
func NewMoveNewAssetsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *MoveNewAssetsStepBuilder {
...
func (s *MoveNewAssetsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to list files in certs-new directory")
		return result, err
	}
	...
	for _, srcPath := range files {
		...
		if err := os.Rename(srcPath, dstPath); err != nil {
			...
			result.MarkFailed(err, fmt.Sprintf("failed to move file '%s' to '%s'", srcPath, dstPath))
			return result, err
		}
