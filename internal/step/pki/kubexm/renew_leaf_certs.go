...
func NewBinaryRenewAllLeafCertsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *BinaryRenewAllLeafCertsStepBuilder {
...
func (s *BinaryRenewAllLeafCertsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	defs, err := s.getCertDefinitions(ctx)
	if err != nil {
		result.MarkFailed(err, "failed to get cert definitions")
		return result, err
	}
	...
	for name, def := range defs {
		...
		if !ok {
			result.MarkFailed(fmt.Errorf("unknown CA"), fmt.Sprintf("unknown CA name '%s' for certificate '%s'", def.caName, name))
