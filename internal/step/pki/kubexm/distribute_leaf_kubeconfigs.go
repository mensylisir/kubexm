...
func NewDistributeWorkerKubeconfigsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DistributeWorkerKubeconfigsStepBuilder {
...
func (s *DistributeWorkerKubeconfigsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, s.remotePkiDir, "0755", s.Sudo); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to create remote PKI directory '%s'", s.remotePkiDir))
		return result, err
	}
	if err := runner.Mkdirp(ctx.GoContext(), conn, s.remoteKubeconfigDir, "0755", s.Sudo); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to create remote kubeconfig directory '%s'", s.remoteKubeconfigDir))
		return result, err
	}
	...
	kubeletConfFileName := common.KubeletKubeconfigFileName
	...
	if err := runner.Upload(ctx.GoContext(), conn, localKubeletConfPath, remoteKubeletConfPath, s.Sudo); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to upload kubelet.conf for node %s", currentNodeName))
		return result, err
	}
	...
	if err := runner.Upload(ctx.GoContext(), conn, localKubeProxyConfPath, remoteKubeProxyConfPath, s.Sudo); err != nil {
		result.MarkFailed(err, "failed to upload kube-proxy.conf")
		return result, err
	}

	logger.Info("Successfully distributed all required certificates and kubeconfigs to the worker node.")
	result.MarkCompleted("worker kubeconfigs distributed successfully")
	return result, nil
}
