...
type LocalCleanupStepBuilder struct {
	step.Builder[LocalCleanupStepBuilder, *KubeadmLocalCleanupStep]
}

func NewLocalCleanupStepBuilder(ctx runtime.ExecutionContext, instanceName string) *LocalCleanupStepBuilder {
	...
	b := new(LocalCleanupStepBuilder).Init(s)
	return b
}
...
func (s *KubeadmLocalCleanupStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	result.MarkCompleted("workspace cleanup completed")
	return result, nil
}
