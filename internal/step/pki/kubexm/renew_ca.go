...
type KubexmRenewK8sCAStep struct {
	step.Base
...
type KubexmRenewK8sCAStepBuilder struct {
	step.Builder[KubexmRenewK8sCAStepBuilder, *KubexmRenewK8sCAStep]
}

func NewKubexmRenewK8sCAStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubexmRenewK8sCAStepBuilder {
	...
	b := new(KubexmRenewK8sCAStepBuilder).Init(s)
	return b
}

func (s *KubexmRenewK8sCAStep) Meta() *spec.StepMeta {
	return &s.Base.Meta
}

func (s *KubexmRenewK8sCAStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
...
func (s *KubexmRenewK8sCAStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for _, asset := range s.casToRenew {
		...
		if err != nil {
			result.MarkFailed(err, "failed to get CA subject")
			return result, err
		}
		...
		if err := generateNewCACert(keyPath, certPath, subject, s.validity); err != nil {
			...
			result.MarkFailed(err, fmt.Sprintf("failed to generate new CA '%s'", asset.CertFile))
			return result, err
		}
	...
	result.MarkCompleted("CA certificates renewed successfully")
	return result, nil
}

func (s *KubexmRenewK8sCAStep) Rollback(ctx runtime.ExecutionContext) error {
...
var _ step.Step = (*KubexmRenewK8sCAStep)(nil)
