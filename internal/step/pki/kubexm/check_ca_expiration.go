...
type KubexmCheckK8sCAExpirationStep struct {
	step.Base
...
type KubexmCheckK8sCAExpirationStepBuilder struct {
	step.Builder[KubexmCheckK8sCAExpirationStepBuilder, *KubexmCheckK8sCAExpirationStep]
}

func NewKubexmCheckK8sCAExpirationStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubexmCheckK8sCAExpirationStepBuilder {
	s := &KubexmCheckK8sCAExpirationStep{
	...
	b := new(KubexmCheckK8sCAExpirationStepBuilder).Init(s)
	return b
}

func (s *KubexmCheckK8sCAExpirationStep) Meta() *spec.StepMeta {
	return &s.Base.Meta
}

func (s *KubexmCheckK8sCAExpirationStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
...
func (s *KubexmCheckK8sCAExpirationStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for _, certFile := range s.k8sCaCerts {
		...
		if err != nil {
			...
			result.MarkFailed(err, fmt.Sprintf("failed to read critical CA certificate file '%s'", fullPath))
			return result, err
		}
		...
		if pemBlock == nil {
			...
			result.MarkFailed(fmt.Errorf("failed to decode PEM block"), fmt.Sprintf("failed to decode PEM block from critical CA certificate file '%s'", fullPath))
			return result, fmt.Errorf("failed to decode PEM block from critical CA certificate file '%s'", fullPath)
		}

		cert, err := x509.ParseCertificate(pemBlock.Bytes)
		if err != nil {
			log.Errorf("Failed to parse critical CA certificate. Error: %v", err)
			result.MarkFailed(err, fmt.Sprintf("failed to parse critical CA certificate from '%s'", fullPath))
			return result, err
		}
...
func (s *KubexmCheckK8sCAExpirationStep) Rollback(ctx runtime.ExecutionContext) error {
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "host", ctx.GetHost().GetName(), "phase", "Rollback")
	logger.Info("Rollback is not applicable for a check-only step. Nothing to do.")
	return nil
}

var _ step.Step = (*KubexmCheckK8sCAExpirationStep)(nil)
