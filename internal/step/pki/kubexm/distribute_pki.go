...
type KubexmDistributeK8sPKIStep struct {
	step.Base
...
type KubexmDistributeK8sPKIStepBuilder struct {
	step.Builder[KubexmDistributeK8sPKIStepBuilder, *KubexmDistributeK8sPKIStep]
}

func NewKubexmDistributeK8sPKIStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubexmDistributeK8sPKIStepBuilder {
	...
	b := new(KubexmDistributeK8sPKIStepBuilder).Init(s)
	return b
}

func (s *KubexmDistributeK8sPKIStep) Meta() *spec.StepMeta {
	return &s.Base.Meta
}
...
func (s *KubexmDistributeK8sPKIStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for _, asset := range s.pkiFilesToSync {
		...
		if err := runner.Upload(ctx.GoContext(), conn, localCertPath, remoteCertPath, s.Sudo); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to upload '%s'", asset.CertFile))
			return result, err
		}
		...
		if err := runner.Upload(ctx.GoContext(), conn, localKeyPath, remoteKeyPath, s.Sudo); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to upload '%s'", asset.KeyFile))
			return result, err
		}
	}

	logger.Info("Successfully distributed renewed K8s PKI files to the node.")
	result.MarkCompleted("K8s PKI distributed successfully")
	return result, nil
}
...
var _ step.Step = (*KubexmDistributeK8sPKIStep)(nil)
