...
func NewConfigureContainerdDropInStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ConfigureContainerdDropInStepBuilder {
...
func (s *ConfigureContainerdDropInStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if content == "" {
		...
		if err := runner.Remove(ctx.GoContext(), conn, s.TargetPath, s.Sudo, false); err != nil {
			if !strings.Contains(err.Error(), "no such file or directory") {
				result.MarkFailed(err, "failed to remove drop-in file")
				return result, fmt.Errorf("failed to remove drop-in file: %w", err)
			}
		}
	} else {
		...
		if err := runner.Mkdirp(ctx.GoContext(), conn, targetDir, "0755", s.Sudo); err != nil {
			...
			return result, fmt.Errorf("failed to create drop-in directory '%s': %w", targetDir, err)
		}
		...
		if err := helpers.WriteContentToRemote(ctx, conn, content, s.TargetPath, "0644", s.Sudo); err != nil {
			...
			return result, fmt.Errorf("failed to write drop-in file: %w", err)
		}
	}
	...
	result.MarkCompleted("containerd drop-in configured successfully")
	return result, nil
}
