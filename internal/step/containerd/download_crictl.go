...
func NewDownloadCriCtlStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadCriCtlStepBuilder {
...
func (s *DownloadCriCtlStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(requiredArchs) == 0 {
		...
		result.MarkCompleted("skipping - no hosts require crictl")
		return result, nil
	}
	...
	for arch := range requiredArchs {
		binaryInfo, err := provider.GetBinary(binary.ComponentCriCtl, arch)
		if err != nil {
			result.MarkFailed(err, "failed to get crictl info")
			return result, fmt.Errorf("failed to get crictl info for arch %s: %w", arch, err)
		}
		if binaryInfo == nil {
			continue
		}
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			result.MarkFailed(err, "failed to download crictl")
			return result, fmt.Errorf("failed to download crictl for arch %s: %w", arch, err)
		}
	}

	logger.Info("All required crictl binaries have been downloaded successfully.")
	result.MarkCompleted("crictl downloaded successfully")
	return result, nil
}
