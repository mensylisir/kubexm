...
func (s *ExtractCNIPluginsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(requiredArchs) == 0 {
		...
		result.MarkCompleted("skipping - no hosts require CNI plugins")
		return result, nil
	}

	for arch := range requiredArchs {
		if err := s.extractFileForArch(ctx, arch); err != nil {
			result.MarkFailed(err, "failed to extract archive for arch "+arch)
			return result, err
		}
	}

	logger.Info("All required CNI plugins archives have been extracted successfully.")
	result.MarkCompleted("all CNI plugins archives extracted successfully")
	return result, nil
}
