...
func NewDownloadCNIPluginsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadCNIPluginsStepBuilder {
...
func (s *DownloadCNIPluginsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for arch := range requiredArchs {
		binaryInfo, err := provider.GetBinary(binary.ComponentKubeCNI, arch)
		if err != nil {
			result.MarkFailed(err, "failed to get CNI plugins info")
			return result, fmt.Errorf("failed to get CNI plugins info for arch %s: %w", arch, err)
		}
		if binaryInfo == nil {
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			result.MarkFailed(err, "failed to download CNI plugins")
			return result, fmt.Errorf("failed to download CNI plugins for arch %s: %w", arch, err)
		}
	}

	logger.Info("All required CNI plugins have been downloaded successfully.")
	result.MarkCompleted("CNI plugins downloaded successfully")
	return result, nil
}
