...
func NewEnableContainerdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *EnableContainerdStepBuilder {
...
func (s *EnableContainerdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.EnableService(ctx.GoContext(), conn, facts, containerdServiceName); err != nil {
		result.MarkFailed(err, "failed to enable containerd service")
		return result, fmt.Errorf("failed to enable containerd service: %w", err)
	}

	logger.Info("Containerd service enabled successfully.")
	result.MarkCompleted("containerd enabled successfully")
	return result, nil
}
