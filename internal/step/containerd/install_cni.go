...
func NewInstallCNIPluginsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallCNIPluginsStepBuilder {
...
func (s *InstallCNIPluginsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "disabled for arch") {
			logger.Info("CNI plugins not required for this host, skipping.", "arch", ctx.GetHost().GetArch())
			result.MarkCompleted("skipping - CNI plugins not required for this host")
			return result, nil
		}
		result.MarkFailed(err, "failed to get local extracted path")
		return result, err
	}
	if _, err := os.Stat(localExtractedPath); os.IsNotExist(err) {
		result.MarkFailed(err, "local CNI extracted path not found")
		return result, fmt.Errorf("local CNI extracted path '%s' not found, ensure extract step ran successfully", localExtractedPath)
	}

	if err := runner.Mkdirp(ctx.GoContext(), conn, s.RemoteCNIBinDir, "0755", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to create remote CNI bin directory")
		return result, fmt.Errorf("failed to create remote CNI bin directory '%s': %w", s.RemoteCNIBinDir, err)
	}
	...
	for _, pluginName := range s.knownCniPlugins {
		...
		if err := runner.Upload(ctx.GoContext(), conn, localPath, remoteTempPath, false); err != nil {
			result.MarkFailed(err, "failed to upload CNI plugin")
			return result, fmt.Errorf("failed to upload '%s': %w", localPath, err)
		}
		...
		if _, err := runner.Run(ctx.GoContext(), conn, moveCmd, s.Sudo); err != nil {
			result.MarkFailed(err, "failed to move CNI plugin")
			return result, fmt.Errorf("failed to move CNI plugin '%s': %w", pluginName, err)
		}
		...
		if err := runner.Chmod(ctx.GoContext(), conn, remoteTargetPath, s.Permission, s.Sudo); err != nil {
			result.MarkFailed(err, "failed to set permission on CNI plugin")
			return result, fmt.Errorf("failed to set permission on CNI plugin '%s': %w", pluginName, err)
		}
	}

	logger.Info("All required CNI plugins have been installed successfully.")
	result.MarkCompleted("CNI plugins installed successfully")
	return result, nil
}
