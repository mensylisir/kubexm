...
func NewInstallContainerdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallContainerdStepBuilder {
...
func (s *InstallContainerdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "disabled for arch") {
			logger.Info("Containerd not required for this host, skipping.", "arch", ctx.GetHost().GetArch())
			result.MarkCompleted("skipping - containerd not required for this host")
			return result, nil
		}
	...
	if _, err := os.Stat(localExtractedPath); os.IsNotExist(err) {
		result.MarkFailed(err, "local containerd extracted path not found")
		return result, fmt.Errorf("local containerd extracted path '%s' not found, ensure extract step ran successfully", localExtractedPath)
	}
	...
	for sourceRelPath, remoteTargetPath := range files {
		...
		if err := runner.Upload(ctx.GoContext(), conn, localSourcePath, remoteTempPath, false); err != nil {
			result.MarkFailed(err, "failed to upload containerd")
		...
		if _, err := runner.Run(ctx.GoContext(), conn, moveCmd, s.Sudo); err != nil {
			result.MarkFailed(err, "failed to move containerd file")
	...
	result.MarkCompleted("containerd installed successfully")
	return result, nil
}
