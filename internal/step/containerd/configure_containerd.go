...
func NewConfigureContainerdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ConfigureContainerdStepBuilder {
...
func (s *ConfigureContainerdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, targetDir, "0755", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to create containerd config directory")
		return result, fmt.Errorf("failed to create containerd config directory '%s': %w", targetDir, err)
	}
	...
	if err := helpers.WriteContentToRemote(ctx, conn, content, s.TargetPath, "0644", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to write containerd config")
		return result, err
	}
	result.MarkCompleted("containerd configured successfully")
	return result, nil
}
