...
func NewDownloadContainerdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadContainerdStepBuilder {
...
func (s *DownloadContainerdStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	for arch := range requiredArchs {
		...
		logger.Debug("Checking for containerd.", "arch", arch, "path", destPath)
...
func (s *DownloadContainerdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(requiredArchs) == 0 {
		...
		result.MarkCompleted("skipping - no hosts require containerd")
		return result, nil
	}
	...
	for arch := range requiredArchs {
		binaryInfo, err := provider.GetBinary(binary.ComponentContainerd, arch)
		if err != nil {
			result.MarkFailed(err, "failed to get containerd info")
			return result, fmt.Errorf("failed to get containerd info for arch %s: %w", arch, err)
		}
		if binaryInfo == nil {
			continue
		}
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			result.MarkFailed(err, "failed to download containerd")
			return result, fmt.Errorf("failed to download containerd for arch %s: %w", arch, err)
		}
	}

	logger.Info("All required containerd binaries have been downloaded successfully.")
	result.MarkCompleted("containerd downloaded successfully")
	return result, nil
}
...
func (s *DownloadContainerdStep) downloadFile(ctx runtime.ExecutionContext, binaryInfo *binary.Binary) error {
	...
	logger.Info("Downloading containerd.", "arch", binaryInfo.Arch, "version", binaryInfo.Version, "url", binaryInfo.URL())
