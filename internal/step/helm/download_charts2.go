...
func NewDownloadHelmCharts2StepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadHelmCharts2StepBuilder {
...
func (s *DownloadHelmCharts2Step) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	helmPath, err := s.resolveHelmBinary(ctx)
	if err != nil {
		return false, err
	}
...
func (s *DownloadHelmCharts2Step) resolveHelmBinary(ctx runtime.ExecutionContext) (string, error) {
	...
	if err != nil {
		return "", errors.Wrap(err, "failed to resolve helm binary metadata")
	}
	if binaryInfo == nil {
		return "", fmt.Errorf("helm binary is not configured for arch %s", arch)
	}

	sourcePath := binaryInfo.FilePath()
	if _, err := os.Stat(sourcePath); err != nil {
		return "", errors.Wrapf(err, "helm binary archive not found at %s; ensure DownloadBinaries ran", sourcePath)
	}

	if !binaryInfo.IsArchive() {
		return sourcePath, nil
	}

	extractDir := filepath.Join(ctx.GetGlobalWorkDir(), "tools", "helm", binaryInfo.Version, arch)
	helmPath := filepath.Join(extractDir, fmt.Sprintf("linux-%s", arch), "helm")
	if _, err := os.Stat(helmPath); err == nil {
		return helmPath, nil
	}

	if err := helpers.ExtractTarGz(sourcePath, extractDir); err != nil {
		return "", errors.Wrapf(err, "failed to extract helm archive %s", sourcePath)
	}
	if _, err := os.Stat(helmPath); err != nil {
		return "", errors.Wrapf(err, "helm binary not found after extraction at %s", helmPath)
	}
	return helmPath, nil
}

func (s *DownloadHelmCharts2Step) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for _, chart := range chartsToDownload {
		if !addedRepos[chart.RepoName()] {
			...
			if output, err := repoAddCmd.CombinedOutput(); err != nil && !strings.Contains(string(output), "already exists") {
				err := fmt.Errorf("failed to add helm repo '%s': %w\nOutput: %s", chart.RepoName(), err, string(output))
	...
	if len(addedRepos) > 0 {
		...
		if output, err := repoUpdateCmd.CombinedOutput(); err != nil {
			err := fmt.Errorf("failed to update helm repos: %w\nOutput: %s", err, string(output))
