...
import (
	...
	"github.com/mensylisir/kubexm/internal/step/helpers" // 引入 helpers 包
...
func NewDownloadHelmStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadHelmStepBuilder {
...
func (s *DownloadHelmStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for arch := range requiredArchs {
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			err := fmt.Errorf("failed to download helm for arch %s: %w", arch, err)
			result.MarkFailed(err, "failed to download helm binary")
			return result, err
		}
	}

	logger.Info("All required helm binaries have been downloaded successfully.")
	result.MarkCompleted("helm binaries downloaded successfully")
	return result, nil
}
