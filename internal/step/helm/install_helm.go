...
func NewInstallHelmStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallHelmStepBuilder {
...
func (s *InstallHelmStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, s.InstallPath, "0755", s.Sudo); err != nil {
		err := errors.Wrapf(err, "failed to create remote install directory '%s'", s.InstallPath)
		result.MarkFailed(err, "failed to create install directory")
		return result, err
	}
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, remoteUploadTmpDir, "0755", false); err != nil { // No sudo for tmp dir
		err := errors.Wrapf(err, "failed to create remote upload directory '%s'", remoteUploadTmpDir)
		result.MarkFailed(err, "failed to create upload directory")
		return result, err
	}
	defer func() {
	...
	if err := runner.Upload(ctx.GoContext(), conn, localSourcePath, remoteTempPath, false); err != nil {
		err := errors.Wrapf(err, "failed to upload '%s' to '%s'", localSourcePath, remoteTempPath)
		result.MarkFailed(err, "failed to upload helm binary")
		return result, err
	}
	...
	if _, err := runner.Run(ctx.GoContext(), conn, moveCmd, s.Sudo); err != nil {
		err := errors.Wrapf(err, "failed to move file to '%s'", targetPath)
		result.MarkFailed(err, "failed to move helm binary")
		return result, err
	}
	...
	if err := runner.Chmod(ctx.GoContext(), conn, targetPath, s.Permission, s.Sudo); err != nil {
		err := errors.Wrapf(err, "failed to set permission on '%s'", targetPath)
		result.MarkFailed(err, "failed to set permissions")
		return result, err
	}

	logger.Infof("Successfully installed helm to %s", targetPath)
	result.MarkCompleted("helm installed successfully")
	return result, nil
}
