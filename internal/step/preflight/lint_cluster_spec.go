...
func NewLintClusterSpecStepBuilder(ctx runtime.ExecutionContext, instanceName string) *LintClusterSpecStepBuilder {
...
func (s *LintClusterSpecStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(validationErrors) > 0 {
		...
		result.MarkFailed(err, "Cluster configuration validation failed")
		return result, err
	}

	logger.Info("Cluster configuration linting passed successfully.")
	result.MarkCompleted("Cluster configuration linting passed")
	return result, nil
}
