...
func NewCheckHostConnectivityStepBuilder(ctx runtime.ExecutionContext, instanceName string) *CheckHostConnectivityStepBuilder {
...
func (s *CheckHostConnectivityStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(allErrors) > 0 {
		err = fmt.Errorf("one or more connectivity checks failed: %s", strings.Join(allErrors, "; "))
		result.MarkFailed(err, "Connectivity check failed")
		return result, err
	}

	logger.Info("All connectivity checks to other nodes passed.")
	result.MarkCompleted("All connectivity checks passed")
	return result, nil
}
