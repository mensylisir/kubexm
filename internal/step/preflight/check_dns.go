...
func NewCheckDNSConfigStepBuilder(ctx runtime.ExecutionContext, instanceName string) *CheckDNSConfigStepBuilder {
...
func (s *CheckDNSConfigStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to read '%s' on host %s", s.resolvConfPath, ctx.GetHost().GetName()))
		return result, err
	}

	contentStr := string(fileContent)
	if strings.TrimSpace(contentStr) == "" {
		err = fmt.Errorf("'%s' is empty or contains only whitespace", s.resolvConfPath)
		result.MarkFailed(err, "DNS configuration is empty")
		return result, err
	}
	...
	for scanner.Scan() {
		...
		if len(fields) >= 2 && fields[0] == "nameserver" {
			...
			if ip == nil {
				err = fmt.Errorf("found an invalid IP address for a nameserver in '%s': %s", s.resolvConfPath, ipStr)
				result.MarkFailed(err, "Invalid nameserver IP")
				return result, err
			}
	...
	if !nameserverFound {
		err = fmt.Errorf("no valid 'nameserver' entry found in '%s'", s.resolvConfPath)
		result.MarkFailed(err, "No valid nameserver found")
		return result, err
	}

	logger.Infof("DNS configuration in '%s' is valid.", s.resolvConfPath)
	result.MarkCompleted("DNS configuration is valid")
	return result, nil
}
