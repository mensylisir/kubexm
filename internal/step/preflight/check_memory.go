...
func NewCheckMemoryStepBuilder(ctx runtime.ExecutionContext, instanceName string) *CheckMemoryStepBuilder {
	s := &CheckMemoryStep{
...
func (s *CheckMemoryStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	if err := s.checkRequirement(ctx); err != nil {
		result.MarkFailed(err, "Memory check failed")
		return result, err
	}
	result.MarkCompleted("Memory check passed")
	return result, nil
}
