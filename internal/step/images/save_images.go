...
func NewSaveImagesStepBuilder(ctx runtime.ExecutionContext, instanceName string) *SaveImagesStepBuilder {
	s := &SaveImagesStep{
...
func (s *SaveImagesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(imagesToSave) == 0 {
		...
		result.MarkCompleted("No images need to be saved for this configuration")
		return result, nil
	}
	...
	if err := os.MkdirAll(s.ImagesDir, 0755); err != nil {
		failErr := fmt.Errorf("failed to create local images directory '%s': %w", s.ImagesDir, err)
		result.MarkFailed(failErr, "failed to create local images directory")
	...
	if len(allErrors) > 0 {
		failErr := fmt.Errorf("failed to save some images:\n- %s", strings.Join(allErrors, "\n- "))
		result.MarkFailed(failErr, "failed to save some images")
		return result, failErr
	}

	logger.Info("All required container images have been saved successfully.")
	result.MarkCompleted("All required container images have been saved successfully")
	return result, nil
}
