...
func (s *PushImagesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		failErr := fmt.Errorf("failed to read local index.json: %w", err)
		result.MarkFailed(failErr, "failed to read local index.json")
		return result, failErr
	}
	var ociIndex v1.Index
	if err := json.Unmarshal(indexJsonContent, &ociIndex); err != nil {
		failErr := fmt.Errorf("failed to unmarshal OCI index: %w", err)
		result.MarkFailed(failErr, "failed to unmarshal OCI index")
		return result, failErr
	}
	...
	if len(allErrors) > 0 {
		failErr := fmt.Errorf("failed to push some images:\n- %s", strings.Join(allErrors, "\n- "))
		result.MarkFailed(failErr, "failed to push some images")
		return result, failErr
	}
	...
	result.MarkCompleted("All saved images have been pushed to the private registry successfully")
	return result, nil
}
