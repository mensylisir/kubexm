...
type CleanNFSProvisionerStep struct {
	step.Base
	ReleaseName         string
	Namespace           string
...
type CleanNFSProvisionerStepBuilder struct {
	step.Builder[CleanNFSProvisionerStepBuilder, *CleanNFSProvisionerStep]
}

func NewCleanNFSProvisionerStepBuilder(ctx runtime.ExecutionContext, instanceName string) *CleanNFSProvisionerStepBuilder {
	cfg := ctx.GetClusterConfig()
	if cfg.Spec.Storage == nil || cfg.Spec.Storage.NFS == nil {
		return nil
	}
	...
	s := &CleanNFSProvisionerStep{}
	s.Base.Meta.Name = instanceName
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Uninstall NFS Provisioner Helm release", s.Base.Meta.Name)
	s.Base.Sudo = false
	s.Base.IgnoreError = false
	s.Base.Timeout = 10 * time.Minute

	s.ReleaseName = NfsChartName
	s.Namespace = "nfs-provisioner"
	s.AdminKubeconfigPath = filepath.Join(common.KubernetesConfigDir, common.AdminKubeconfigFileName)

	b := new(CleanNFSProvisionerStepBuilder).Init(s)
	return b
}

func (s *CleanNFSProvisionerStep) Meta() *spec.StepMeta {
	return &s.Base.Meta
}

func (s *CleanNFSProvisionerStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
...
func (s *CleanNFSProvisionerStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	result.MarkCompleted("NFS Provisioner Helm chart uninstalled successfully")
	return result, nil
}

func (s *CleanNFSProvisionerStep) Rollback(ctx runtime.ExecutionContext) error {
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name)
	logger.Info("Rollback is not applicable for a cleanup step. No action taken.")
	return nil
}

var _ step.Step = (*CleanNFSProvisionerStep)(nil)
