...
func NewGenerateNFSProvisionerValuesStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateNFSProvisionerValuesStepBuilder {
...
func (s *GenerateNFSProvisionerValuesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "phase", "Run")

	if s.NfsServer == "" || s.NfsPath == "" {
		result.MarkFailed(fmt.Errorf("NFS Server or Path is not configured"), "cannot generate values file")
		return result, fmt.Errorf("NFS Server or Path is not configured, cannot generate values file")
	}

	valuesTemplateContent, err := templates.Get("storage/nfs/values.yaml.tmpl")
	if err != nil {
		result.MarkFailed(err, "failed to get embedded nfs values.yaml.tmpl")
		return result, err
	}

	tmpl, err := template.New("nfsValues").Parse(valuesTemplateContent)
	if err != nil {
		result.MarkFailed(err, "failed to parse nfs values.yaml.tmpl")
		return result, err
	}
	var valuesBuffer bytes.Buffer
	if err := tmpl.Execute(&valuesBuffer, s); err != nil {
		result.MarkFailed(err, "failed to render nfs values.yaml.tmpl")
		return result, err
	}
	...
	result.MarkCompleted("NFS Provisioner values file generated successfully")
	return result, nil
}
