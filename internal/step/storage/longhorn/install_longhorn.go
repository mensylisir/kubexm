...
func NewInstallLonghornHelmChartStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallLonghornHelmChartStepBuilder {
...
func (s *InstallLonghornHelmChartStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to install/upgrade Longhorn Helm chart")
		return result, err
	}

	logger.Info("Longhorn Helm chart installed/upgraded successfully.")
	logger.Debugf("Helm command output:\n%s", output)
	result.MarkCompleted("Longhorn Helm chart installed successfully")
	return result, nil
}
