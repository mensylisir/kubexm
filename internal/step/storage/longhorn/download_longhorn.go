...
func NewDownloadLonghornChartStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadLonghornChartStepBuilder {
...
func (s *DownloadLonghornChartStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if _, err := pullCmd.CombinedOutput(); err != nil {
		result.MarkFailed(err, "failed to pull Longhorn chart")
		return result, err
	}

	logger.Info("Longhorn Helm chart has been downloaded successfully to the local machine.")
	result.MarkCompleted("Longhorn chart downloaded successfully")
	return result, nil
}
