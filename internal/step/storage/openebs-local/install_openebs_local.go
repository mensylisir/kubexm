...
func NewInstallOpenEBSHelmChartStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallOpenEBSHelmChartStepBuilder {
...
func (s *InstallOpenEBSHelmChartStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	logger.Debug("All required OpenEBS artifacts found on remote host.")
...
func (s *InstallOpenEBSHelmChartStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to install/upgrade OpenEBS Helm chart")
		return result, err
	}

	logger.Info("OpenEBS Helm chart installed/upgraded successfully.")
	logger.Debugf("Helm command output:\n%s", output)
	result.MarkCompleted("OpenEBS Helm chart installed successfully")
	return result, nil
}
