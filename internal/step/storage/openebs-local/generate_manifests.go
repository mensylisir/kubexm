...
func NewGenerateOpenEBSValuesStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateOpenEBSValuesStepBuilder {
...
func (s *GenerateOpenEBSValuesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "phase", "Run")

	valuesTemplateContent, err := templates.Get("storage/openebs-local/values.yaml.tmpl")
	if err != nil {
		result.MarkFailed(err, "failed to get embedded openebs values.yaml.tmpl")
		return result, err
	}

	tmpl, err := template.New("openebsValues").Parse(valuesTemplateContent)
	if err != nil {
		result.MarkFailed(err, "failed to parse openebs values.yaml.tmpl")
		return result, err
	}
	var valuesBuffer bytes.Buffer
	if err := tmpl.Execute(&valuesBuffer, s); err != nil {
		result.MarkFailed(err, "failed to render openebs values.yaml.tmpl")
		return result, err
	}
	...
	result.MarkCompleted("OpenEBS values file generated successfully")
	return result, nil
}
