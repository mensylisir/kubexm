...
func NewGenerateKubeletCertsForAllNodesStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateKubeletCertsForAllNodesStepBuilder {
...
func (s *GenerateKubeletCertsForAllNodesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to load certificate authority")
		return result, err
	}
	...
	for _, node := range allHosts {
		...
		if err := helpers.NewSignedCertificate(s.CertsDir, certFile, keyFile, kubeletClientCfg, mainCA, mainKey); err != nil {
			err = fmt.Errorf("failed to generate kubelet client cert for %s: %w", nodeName, err)
