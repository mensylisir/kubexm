...
func NewDistributeKubeletConfigForAllNodesStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DistributeKubeletConfigForAllNodesStepBuilder {
...
func (s *DistributeKubeletConfigForAllNodesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, s.RemotePKIDir, "0755", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to create PKI directory")
		return result, err
	}
	if err := runner.Mkdirp(ctx.GoContext(), conn, s.RemoteConfigDir, "0755", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to create config directory")
		return result, err
	}

	for _, file := range s.getFilesToDistribute(ctx) {
		if _, err := os.Stat(file.localSrcFile); os.IsNotExist(err) {
			err = fmt.Errorf("local source file '%s' not found, ensure previous generation steps ran successfully", file.localSrcFile)
			result.MarkFailed(err, "local file not found")
			return result, err
		}
		...
		if err := runner.Upload(ctx.GoContext(), conn, file.localSrcFile, file.remoteDstFile, s.Sudo); err != nil {
			err = fmt.Errorf("failed to upload '%s': %w", filepath.Base(file.localSrcFile), err)
		...
		if err := runner.Chmod(ctx.GoContext(), conn, file.remoteDstFile, file.permissions, s.Sudo); err != nil {
			err = fmt.Errorf("failed to set permission on '%s': %w", file.remoteDstFile, err)
	...
	result.MarkCompleted("credentials distributed successfully")
	return result, nil
}
