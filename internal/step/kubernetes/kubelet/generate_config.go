...
func NewCreateKubeletConfigYAMLStepBuilder(ctx runtime.ExecutionContext, instanceName string) *CreateKubeletConfigYAMLStepBuilder {
...
func (s *CreateKubeletConfigYAMLStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, filepath.Dir(s.RemoteConfigYAMLFile), "0755", s.Sudo); err != nil {
		err = fmt.Errorf("failed to create directory for kubelet config: %w", err)
	...
	if err := helpers.WriteContentToRemote(ctx, conn, content, s.RemoteConfigYAMLFile, "0644", s.Sudo); err != nil {
		...
		result.MarkFailed(err, "failed to write config")
		return result, err
	}
	result.MarkCompleted("config written successfully")
	return result, nil
}
