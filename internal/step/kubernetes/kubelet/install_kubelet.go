...
func NewInstallKubeletStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallKubeletStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Install kubelet binary", s.Base.Meta.Name)
	s.Base.Sudo = false
...
func (s *InstallKubeletStep) getLocalSourcePath(ctx runtime.ExecutionContext) (string, error) {
	provider := binary.NewBinaryProvider(ctx)
	arch := ctx.GetHost().GetArch()
	binaryInfo, err := provider.GetBinary(binary.ComponentKubelet, arch)
	if err != nil {
		return "", fmt.Errorf("failed to get kubelet binary info: %w", err)
	}
	if binaryInfo == nil {
		return "", fmt.Errorf("kubelet is unexpectedly disabled for arch %s", arch)
	}

	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Install kubelet binary (version %s)", s.Base.Meta.Name, binaryInfo.Version)

	return binaryInfo.FilePath(), nil
}

func (s *InstallKubeletStep) getRemoteTargetPath() string {
	return filepath.Join(s.InstallPath, "kubelet")
}
...
func (s *InstallKubeletStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "disabled for arch") {
			logger.Infof("kubelet not required for this host (arch: %s), skipping.", ctx.GetHost().GetArch())
			return true, nil
		}
		return false, err
	}
...
func (s *InstallKubeletStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	remoteTempPath := filepath.Join(remoteUploadTmpDir, "kubelet")
	logger.Infof("Uploading kubelet to %s:%s", ctx.GetHost().GetName(), remoteTempPath)
	...
	if err := runner.Upload(ctx.GoContext(), conn, localSourcePath, remoteTempPath, false); err != nil {
		...
		result.MarkFailed(err, "failed to upload kubelet")
		return result, err
	}
	...
	logger.Infof("Successfully installed kubelet to %s", targetPath)
	result.MarkCompleted("kubelet installed successfully")
	return result, nil
}
...
var _ step.Step = (*InstallKubeletStep)(nil)
