...
func NewDownloadKubeletStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadKubeletStepBuilder {
...
func (s *DownloadKubeletStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for arch := range requiredArchs {
		binaryInfo, err := provider.GetBinary(binary.ComponentKubelet, arch)
		if err != nil {
			err = fmt.Errorf("failed to get kubelet info for arch %s: %w", arch, err)
			result.MarkFailed(err, "failed to get binary info")
			return result, err
		}
		if binaryInfo == nil {
			err = fmt.Errorf("kubelet binary info not found for arch %s in BOM", arch)
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			err = fmt.Errorf("failed to download kubelet for arch %s: %w", arch, err)
