...
func NewVerifyKubeProxyHealthStepBuilder(ctx runtime.ExecutionContext, instanceName string) *VerifyKubeProxyHealthStepBuilder {
...
func (s *VerifyKubeProxyHealthStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for i := 0; i < s.retries; i++ {
		...
		if err == nil {
			...
			result.MarkCompleted("kube-proxy is healthy")
			return result, nil
		}
	...
	err := fmt.Errorf("kube-proxy health verification failed: %w", lastErr)
	result.MarkFailed(err, "kube-proxy health verification failed")
	return result, err
}
