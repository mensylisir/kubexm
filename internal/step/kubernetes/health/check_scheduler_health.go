...
func NewVerifySchedulerHealthStepBuilder(ctx runtime.ExecutionContext, instanceName string) *VerifySchedulerHealthStepBuilder {
...
func (s *VerifySchedulerHealthStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for i := 0; i < s.retries; i++ {
		...
		if err == nil {
			...
			result.MarkCompleted("kube-scheduler is healthy")
			return result, nil
		}
	...
	err := fmt.Errorf("kube-scheduler health verification failed: %w", lastErr)
	result.MarkFailed(err, "kube-scheduler health verification failed")
	return result, err
}
