...
type CheckClusterHealthStep struct {
	step.Base
	retryDelay time.Duration
}

type CheckClusterHealthStepBuilder struct {
	step.Builder[CheckClusterHealthStepBuilder, *CheckClusterHealthStep]
}

func NewCheckClusterHealthStepBuilder(ctx runtime.ExecutionContext, instanceName string) *CheckClusterHealthStepBuilder {
	s := &CheckClusterHealthStep{
	...
	b := new(CheckClusterHealthStepBuilder).Init(s)
	return b
}

func (s *CheckClusterHealthStep) Meta() *spec.StepMeta {
	return &s.Base.Meta
}

func (s *CheckClusterHealthStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
...
func (s *CheckClusterHealthStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for {
		...
		case <-timeout:
			if lastErr != nil {
				err := fmt.Errorf("cluster health verification timed out after %v: %w", s.Base.Timeout, lastErr)
				result.MarkFailed(err, "cluster health verification timed out")
				return result, err
			}
			err := fmt.Errorf("cluster health verification timed out after %v", s.Base.Timeout)
			result.MarkFailed(err, "cluster health verification timed out")
			return result, err
		case <-ticker.C:
			...
			if err == nil {
				...
				result.MarkCompleted("cluster is healthy")
				return result, nil
			}
...
func (s *CheckClusterHealthStep) Rollback(ctx runtime.ExecutionContext) error {
...
var _ step.Step = (*CheckClusterHealthStep)(nil)
