...
func NewVerifyKubeletHealthStepBuilder(ctx runtime.ExecutionContext, instanceName string) *VerifyKubeletHealthStepBuilder {
...
func (s *VerifyKubeletHealthStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for {
		...
		case <-timeout:
			if lastErr != nil {
				err := fmt.Errorf("kubelet health verification timed out after %v: %w", s.Base.Timeout, lastErr)
				result.MarkFailed(err, "kubelet health verification timed out")
				return result, err
			}
			err := fmt.Errorf("kubelet health verification timed out after %v", s.Base.Timeout)
			result.MarkFailed(err, "kubelet health verification timed out")
			return result, err
		case <-ticker.C:
			...
			if err == nil {
				...
				result.MarkCompleted("kubelet is healthy")
				return result, nil
			}
