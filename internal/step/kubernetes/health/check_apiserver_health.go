...
func NewVerifyAPIServerHealthStepBuilder(ctx runtime.ExecutionContext, instanceName string) *VerifyAPIServerHealthStepBuilder {
...
func (s *VerifyAPIServerHealthStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for {
		...
		case <-timeout:
			if lastErr != nil {
				err := fmt.Errorf("kube-apiserver health verification timed out after %v: %w", s.Base.Timeout, lastErr)
				result.MarkFailed(err, "kube-apiserver health verification timed out")
				return result, err
			}
			err := fmt.Errorf("kube-apiserver health verification timed out after %v", s.Base.Timeout)
			result.MarkFailed(err, "kube-apiserver health verification timed out")
			return result, err
		case <-ticker.C:
			...
			if err == nil {
				...
				result.MarkCompleted("kube-apiserver is healthy")
				return result, nil
			}
