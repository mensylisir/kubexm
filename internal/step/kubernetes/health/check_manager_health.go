...
func NewVerifyControllerManagerHealthStepBuilder(ctx runtime.ExecutionContext, instanceName string) *VerifyControllerManagerHealthStepBuilder {
...
func (s *VerifyControllerManagerHealthStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for i := 0; i < s.retries; i++ {
		...
		if err == nil {
			...
			result.MarkCompleted("kube-controller-manager is healthy")
			return result, nil
		}
	...
	err := fmt.Errorf("kube-controller-manager health verification failed: %w", lastErr)
	result.MarkFailed(err, "kube-controller-manager health verification failed")
	return result, err
}
