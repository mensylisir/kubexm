...
func NewBackupRemoteConfigsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *BackupRemoteConfigsStepBuilder {
...
func (s *BackupRemoteConfigsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if _, err := runner.Run(ctx.GoContext(), conn, backupCmd, s.Sudo); err != nil {
		err = fmt.Errorf("failed to back up remote config directory on host '%s': %w", ctx.GetHost().GetName(), err)
		result.MarkFailed(err, "failed to backup config directory")
		return result, err
	}
