...
func NewDrainNodeStepBuilder(ctx runtime.ExecutionContext, instanceName string, targetNodeName string) *DrainNodeStepBuilder {
	s := &DrainNodeStep{
...
func (s *DrainNodeStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if _, err := runner.Run(ctx.GoContext(), conn, drainCmd, s.Sudo); err != nil {
		err = fmt.Errorf("failed to drain node '%s': %w", s.TargetNodeName, err)
		result.MarkFailed(err, "failed to drain node")
		return result, err
	}

	logger.Infof("Node '%s' drained successfully.", s.TargetNodeName)
	result.MarkCompleted("node drained successfully")
	return result, nil
}
