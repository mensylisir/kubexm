...
func NewKubeadmLabelWorkerNodeStepBuilder(ctx runtime.ExecutionContext, instanceName string, targetWorkerNodeName string) *KubeadmLabelWorkerNodeStepBuilder {
...
func (s *KubeadmLabelWorkerNodeStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if _, err := runner.Run(ctx.GoContext(), conn, labelCmd, s.Sudo); err != nil {
		err = fmt.Errorf("failed to apply label '%s' to worker node '%s': %w", workerLabel, s.TargetWorkerNodeName, err)
		result.MarkFailed(err, "failed to apply worker label")
		return result, err
	}

	log.Info("Worker role label applied successfully.")
	result.MarkCompleted("worker label applied successfully")
	return result, nil
}
