...
func NewKubeadmLabelNodesStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmLabelNodesStepBuilder {
...
func (s *KubeadmLabelNodesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(s.Labels) == 0 {
		...
		result.MarkCompleted("no labels to apply")
		return result, nil
	}
	...
	if _, err := runner.Run(ctx.GoContext(), conn, labelCmd, s.Sudo); err != nil {
		err = fmt.Errorf("failed to apply labels to node '%s': %w", nodeName, err)
		result.MarkFailed(err, "failed to apply labels")
		return result, err
	}

	logger.Info("Labels applied successfully.")
	result.MarkCompleted("labels applied successfully")
	return result, nil
}
