...
func NewCheckAPIServerHealthStepBuilder(ctx runtime.ExecutionContext, instanceName string) *CheckAPIServerHealthStepBuilder {
...
func (s *CheckAPIServerHealthStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for {
		...
		case <-timeout:
			if lastErr != nil {
				err := fmt.Errorf("kube-apiserver health check timed out after %v: %w", s.Base.Timeout, lastErr)
				result.MarkFailed(err, "health check timed out")
				return result, err
			}
			err := fmt.Errorf("kube-apiserver health check timed out after %v", s.Base.Timeout)
			result.MarkFailed(err, "health check timed out")
			return result, err
		case <-ticker.C:
			healthy, err := s.checkHealth(ctx)
			if healthy {
				...
				result.MarkCompleted("apiserver is healthy")
				return result, nil
			}
			if err != nil {
				lastErr = err
