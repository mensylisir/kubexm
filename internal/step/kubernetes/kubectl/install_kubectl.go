...
func NewInstallKubectlStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallKubectlStepBuilder {
	...
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Install kubectl binary", s.Base.Meta.Name)
	s.Base.Sudo = false
...
func (s *InstallKubectlStep) getLocalSourcePath(ctx runtime.ExecutionContext) (string, error) {
	provider := binary.NewBinaryProvider(ctx)
	arch := ctx.GetHost().GetArch()
	binaryInfo, err := provider.GetBinary(binary.ComponentKubectl, arch)
	if err != nil {
		return "", fmt.Errorf("failed to get kubectl binary info: %w", err)
	}
	if binaryInfo == nil {
		return "", fmt.Errorf("kubectl is unexpectedly disabled for arch %s", arch)
	}

	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Install kubectl binary (version %s)", s.Base.Meta.Name, binaryInfo.Version)

	return binaryInfo.FilePath(), nil
}

func (s *InstallKubectlStep) getRemoteTargetPath() string {
	return filepath.Join(s.InstallPath, "kubectl")
}
...
func (s *InstallKubectlStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "disabled for arch") {
			logger.Infof("kubectl not required for this host (arch: %s), skipping.", ctx.GetHost().GetArch())
			return true, nil
		}
		return false, err
	}
...
func (s *InstallKubectlStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	remoteUploadTmpDir := filepath.Join(ctx.GetUploadDir(), fmt.Sprintf("kubectl-%d", time.Now().UnixNano()))
	...
	remoteTempPath := filepath.Join(remoteUploadTmpDir, "kubectl")
	logger.Infof("Uploading kubectl to %s:%s", ctx.GetHost().GetName(), remoteTempPath)
	...
	if err := runner.Upload(ctx.GoContext(), conn, localSourcePath, remoteTempPath, false); err != nil {
		...
		result.MarkFailed(err, "failed to upload kubectl")
		return result, err
	}
	...
	logger.Infof("Successfully installed kubectl to %s", targetPath)
	result.MarkCompleted("kubectl installed successfully")
	return result, nil
}
...
var _ step.Step = (*InstallKubectlStep)(nil)
