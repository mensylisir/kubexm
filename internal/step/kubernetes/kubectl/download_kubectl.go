...
func NewDownloadKubectlStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadKubectlStepBuilder {
...
func (s *DownloadKubectlStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for arch := range requiredArchs {
		binaryInfo, err := provider.GetBinary(binary.ComponentKubectl, arch)
		if err != nil {
			err = fmt.Errorf("failed to get kubectl info for arch %s: %w", arch, err)
			result.MarkFailed(err, "failed to get binary info")
			return result, err
		}
		if binaryInfo == nil {
			err = fmt.Errorf("kubectl binary info not found for arch %s in BOM", arch)
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			err = fmt.Errorf("failed to download kubectl for arch %s: %w", arch, err)
