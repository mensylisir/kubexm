...
func NewConfigureKubectlStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ConfigureKubectlStepBuilder {
...
func (s *ConfigureKubectlStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, kubeDir, "0700", s.Sudo); err != nil {
		err = fmt.Errorf("failed to create directory %s: %w", kubeDir, err)
	...
	if _, _, err := runner.OriginRun(ctx.GoContext(), conn, copyCmd, s.Sudo); err != nil {
		err = fmt.Errorf("failed to copy kubeconfig file: %w", err)
	...
	if err := runner.Chown(ctx.GoContext(), conn, s.RemoteKubeconfigDest, "root", "root", false); err != nil {
		err = fmt.Errorf("failed to set ownership on %s: %w", s.RemoteKubeconfigDest, err)
	...
	if s.EnableCompletion {
		...
		if err != nil {
			err = fmt.Errorf("failed to get kubectl completion script template: %w", err)
			result.MarkFailed(err, "failed to get completion template")
			return result, err
		}
		...
		if err := helpers.WriteContentToRemote(ctx, conn, completionScriptContent, remoteScriptPath, "0755", false); err != nil {
			err = fmt.Errorf("failed to write completion script: %w", err)
			result.MarkFailed(err, "failed to write completion script")
			return result, err
		}
	...
	result.MarkCompleted("kubectl configured successfully")
	return result, nil
}
