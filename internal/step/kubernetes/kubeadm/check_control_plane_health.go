...
func NewKubeadmVerifyControlPlaneHealthStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmVerifyControlPlaneHealthStepBuilder {
...
func (s *KubeadmVerifyControlPlaneHealthStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for {
		...
		case <-timeout:
			if lastErr != nil {
				err := fmt.Errorf("control plane health verification timed out after %v: %w", s.Base.Timeout, lastErr)
				result.MarkFailed(err, "control plane health verification timed out")
				return result, err
			}
			err := fmt.Errorf("control plane health verification timed out after %v", s.Base.Timeout)
			result.MarkFailed(err, "control plane health verification timed out")
			return result, err
		case <-ticker.C:
			...
			if err == nil {
				...
				result.MarkCompleted("control plane is healthy")
				return result, nil
			}
