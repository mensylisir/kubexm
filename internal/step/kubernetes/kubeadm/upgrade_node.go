...
func NewKubeadmUpgradeNodeStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmUpgradeNodeStepBuilder {
...
func (s *KubeadmUpgradeNodeStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if !strings.Contains(output, "[upgrade/successful] SUCCESS!") {
		err := fmt.Errorf("could not find success message in 'kubeadm upgrade node' output")
		result.MarkFailed(err, "upgrade node failed")
		return result, err
	}

	logger.Info("Node configuration upgraded successfully.")
	result.MarkCompleted("node upgraded successfully")
	return result, nil
}
