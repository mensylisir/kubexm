...
func NewKubeadmBackupEtcdStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmBackupEtcdStepBuilder {
...
func (s *KubeadmBackupEtcdStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := os.MkdirAll(filepath.Dir(s.localSnapshotPath), 0755); err != nil {
		err = fmt.Errorf("failed to create local backup directory: %w", err)
		result.MarkFailed(err, "failed to create local backup directory")
		return result, err
	}
	...
	if _, err := runner.Run(ctx.GoContext(), conn, snapshotCmd, s.Sudo); err != nil {
		err = fmt.Errorf("failed to create etcd snapshot inside pod on host '%s': %w", nodeName, err)
		result.MarkFailed(err, "failed to create etcd snapshot")
		return result, err
	}
	...
	if _, err := runner.Run(ctx.GoContext(), conn, cpCmd, s.Sudo); err != nil {
		err = fmt.Errorf("failed to copy snapshot from pod to host: %w", err)
		result.MarkFailed(err, "failed to copy snapshot from pod to host")
		return result, err
	}
	...
	if err := runner.Fetch(ctx.GoContext(), conn, s.remoteSnapshotPath, s.localSnapshotPath, s.Sudo); err != nil {
		err = fmt.Errorf("failed to fetch etcd snapshot from host: %w", err)
		result.MarkFailed(err, "failed to fetch etcd snapshot")
		return result, err
	}
	...
	result.MarkCompleted("etcd backup completed successfully")
	return result, nil
}
