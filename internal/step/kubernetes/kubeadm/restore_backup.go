...
func NewRestoreKubeadmBackupStepBuilder(ctx runtime.ExecutionContext, instanceName string) *RestoreKubeadmBackupStepBuilder {
...
func (s *RestoreKubeadmBackupStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if _, err := runner.Run(ctx.GoContext(), conn, restoreCmd, s.Sudo); err != nil {
		err = fmt.Errorf("failed to move backup directory on host '%s': %w", ctx.GetHost().GetName(), err)
		result.MarkFailed(err, "failed to restore backup")
		return result, err
	}
