...
func NewKubeadmVerifyWorkerHealthStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmVerifyWorkerHealthStepBuilder {
...
func (s *KubeadmVerifyWorkerHealthStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for {
		...
		case <-timeout:
			if lastErr != nil {
				err := fmt.Errorf("worker health verification timed out after %v: %w", s.Base.Timeout, lastErr)
				result.MarkFailed(err, "worker health verification timed out")
				return result, err
			}
			err := fmt.Errorf("worker health verification timed out after %v", s.Base.Timeout)
			result.MarkFailed(err, "worker health verification timed out")
			return result, err
		case <-ticker.C:
			...
			if err == nil {
				...
				result.MarkCompleted("worker components are healthy")
				return result, nil
			}
