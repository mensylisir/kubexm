...
func NewKubeadmFinalizeUpgradeStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmFinalizeUpgradeStepBuilder {
...
func (s *KubeadmFinalizeUpgradeStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if !ok {
		err := fmt.Errorf("could not retrieve target version from cache, 'plan' step must run first")
		result.MarkFailed(err, "could not retrieve target version from cache")
		return result, err
	}
	...
	result.MarkCompleted("finalization completed successfully")
	return result, nil
}
