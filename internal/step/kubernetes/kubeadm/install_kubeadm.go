...
type InstallKubeadmStep struct {
	step.Base
	InstallPath string
	Permission  string
}

type InstallKubeadmStepBuilder struct {
	step.Builder[InstallKubeadmStepBuilder, *InstallKubeadmStep]
}

func NewInstallKubeadmStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallKubeadmStepBuilder {
	s := &InstallKubeadmStep{
		InstallPath: common.DefaultBinDir,
		Permission:  "0755",
	}

	s.Base.Meta.Name = instanceName
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Install kubeadm binary", s.Base.Meta.Name)
	s.Base.Sudo = false
	s.Base.IgnoreError = false
	s.Base.Timeout = 5 * time.Minute

	b := new(InstallKubeadmStepBuilder).Init(s)
	return b
}

func (b *InstallKubeadmStepBuilder) WithInstallPath(installPath string) *InstallKubeadmStepBuilder {
	if installPath != "" {
...
func (b *InstallKubeadmStepBuilder) WithPermission(permission string) *InstallKubeadmStepBuilder {
	if permission != "" {
...
func (s *InstallKubeadmStep) Meta() *spec.StepMeta {
	return &s.Base.Meta
}

func (s *InstallKubeadmStep) getLocalSourcePath(ctx runtime.ExecutionContext) (string, error) {
	provider := binary.NewBinaryProvider(ctx)
	arch := ctx.GetHost().GetArch()
	binaryInfo, err := provider.GetBinary(binary.ComponentKubeadm, arch)
	if err != nil {
		return "", fmt.Errorf("failed to get kubeadm binary info: %w", err)
	}
	if binaryInfo == nil {
		return "", fmt.Errorf("kubeadm is unexpectedly disabled for arch %s", arch)
	}

	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Install kubeadm binary (version %s)", s.Base.Meta.Name, binaryInfo.Version)

	return binaryInfo.FilePath(), nil
}

func (s *InstallKubeadmStep) getRemoteTargetPath() string {
	return filepath.Join(s.InstallPath, "kubeadm")
}
...
func (s *InstallKubeadmStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	remoteUploadTmpDir := filepath.Join(ctx.GetUploadDir(), fmt.Sprintf("kubeadm-%d", time.Now().UnixNano()))
	...
	remoteTempPath := filepath.Join(remoteUploadTmpDir, "kubeadm")
	logger.Infof("Uploading kubeadm to %s:%s", ctx.GetHost().GetName(), remoteTempPath)
	...
	if err := runner.Upload(ctx.GoContext(), conn, localSourcePath, remoteTempPath, false); err != nil {
		...
		result.MarkFailed(err, "failed to upload kubeadm")
		return result, err
	}
	...
	logger.Infof("Successfully installed kubeadm to %s", targetPath)
	result.MarkCompleted("kubeadm installed successfully")
	return result, nil
}

func (s *InstallKubeadmStep) Rollback(ctx runtime.ExecutionContext) error {
...
var _ step.Step = (*InstallKubeadmStep)(nil)
