...
func NewGenerateJoinMasterConfigStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateJoinMasterConfigStepBuilder {
...
func (s *GenerateJoinMasterConfigStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to render join master config")
		return result, err
	}
	...
	result.MarkCompleted("kubeadm join-master config generated and uploaded successfully")
	return result, nil
}
