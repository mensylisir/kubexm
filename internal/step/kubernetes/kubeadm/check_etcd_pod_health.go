...
func NewKubeadmVerifyEtcdPodHealthStepBuilder(ctx runtime.ExecutionContext, instanceName string) *KubeadmVerifyEtcdPodHealthStepBuilder {
...
func (s *KubeadmVerifyEtcdPodHealthStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for {
		...
		case <-timeout:
			if lastErr != nil {
				err := fmt.Errorf("etcd pod health verification timed out after %v: %w", s.Base.Timeout, lastErr)
				result.MarkFailed(err, "etcd pod health verification timed out")
				return result, err
			}
			err := fmt.Errorf("etcd pod health verification timed out after %v", s.Base.Timeout)
			result.MarkFailed(err, "etcd pod health verification timed out")
			return result, err
		case <-ticker.C:
			...
			if err == nil {
				...
				result.MarkCompleted("etcd pod is healthy")
				return result, nil
			}
