...
func NewInstallKubeApiServerStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallKubeApiServerStepBuilder {
...
func (s *InstallKubeApiServerStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "disabled for arch") {
			logger.Infof("kube-apiserver not required for this host (arch: %s), skipping.", ctx.GetHost().GetArch())
			return true, nil
		}
		return false, err
	}
...
func (s *InstallKubeApiServerStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	remoteUploadTmpDir := filepath.Join(ctx.GetUploadDir(), fmt.Sprintf("kube-apiserver-%d", time.Now().UnixNano()))
	...
	if err := runner.Upload(ctx.GoContext(), conn, localSourcePath, remoteTempPath, false); err != nil {
		...
		result.MarkFailed(err, "failed to upload kube-apiserver")
		return result, err
	}
	...
	result.MarkCompleted("kube-apiserver installed successfully")
	return result, nil
}
