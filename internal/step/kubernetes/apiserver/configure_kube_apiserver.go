...
func NewConfigureKubeAPIServerStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ConfigureKubeAPIServerStepBuilder {
	...
	if len(etcdNodes) == 0 {
		ctx.GetLogger().Warn("no etcd nodes found in cluster configuration, etcd servers will be empty")
	} else {
...
func (s *ConfigureKubeAPIServerStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if s.Audit != nil && s.Audit.Enabled != nil && *s.Audit.Enabled {
		...
		if err != nil {
			err = fmt.Errorf("failed to get default audit policy template: %w", err)
			result.MarkFailed(err, "failed to get audit policy template")
			return result, err
		}
		err = helpers.WriteContentToRemote(ctx, conn, policyContent, s.RemoteAuditPolicyFile, "0644", s.Sudo)
		if err != nil {
			err = fmt.Errorf("failed to write audit policy file: %w", err)
			result.MarkFailed(err, "failed to write audit policy file")
			return result, err
		}
	}
	...
	if err != nil {
		err = fmt.Errorf("failed to render kube-apiserver config: %w", err)
		result.MarkFailed(err, "failed to render config")
		return result, err
	}
	err = helpers.WriteContentToRemote(ctx, conn, configContent, s.RemoteConfigFile, "0644", s.Sudo)
	if err != nil {
		err = fmt.Errorf("failed to write kube-apiserver config file: %w", err)
		result.MarkFailed(err, "failed to write config file")
		return result, err
	}

	logger.Info("kube-apiserver configuration files have been created successfully.")
	result.MarkCompleted("configuration created successfully")
	return result, nil
}
