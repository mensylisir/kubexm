...
var _ step.Step = (*RestartKubeApiServerStep)(nil)
...
func (s *RestartKubeApiServerStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to determine kube-apiserver service status: %w", err)
	}

	if !active {
		logger.Infof("Kube-apiserver service is not active. Nothing to restart, skipping.")
		return true, nil
	}

	logger.Info("Kube-apiserver service is active. Step needs to run to restart it.")
	return false, nil
}
