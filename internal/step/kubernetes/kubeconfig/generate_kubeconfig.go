...
func NewGenerateAdminKubeconfigStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateAdminKubeconfigStepBuilder {
...
func (s *GenerateAdminKubeconfigStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		err = fmt.Errorf("failed to render admin.conf: %w", err)
		result.MarkFailed(err, "failed to render admin.conf")
		return result, err
	}

	if err := helpers.WriteContentToRemote(ctx, conn, kubeconfigContent, s.RemoteKubeconfigFile, "0644", s.Sudo); err != nil {
		err = fmt.Errorf("failed to write admin.conf file: %w", err)
		result.MarkFailed(err, "failed to write admin.conf file")
		return result, err
	}

	logger.Info("admin.conf has been created successfully.")
	result.MarkCompleted("admin.conf created successfully")
	return result, nil
}
