...
func NewCopyKubeconfigStepBuilder(ctx runtime.ExecutionContext, instanceName string) *CopyKubeconfigStepBuilder {
...
func (s *CopyKubeconfigStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if user == "" {
		...
		result.MarkCompleted("skipped - no user specified")
		return result, nil
	}
	...
	if _, err := runner.Run(ctx.GoContext(), conn, mkdirCmd, s.Sudo); err != nil {
		err = fmt.Errorf("failed to create .kube directory: %w", err)
		result.MarkFailed(err, "failed to create .kube directory")
		return result, err
	}
	...
	if _, err := runner.Run(ctx.GoContext(), conn, cpCmd, s.Sudo); err != nil {
		err = fmt.Errorf("failed to copy admin.conf: %w", err)
		result.MarkFailed(err, "failed to copy admin.conf")
		return result, err
	}
	...
	if _, err := runner.Run(ctx.GoContext(), conn, chownCmd, s.Sudo); err != nil {
		err = fmt.Errorf("failed to change ownership of kubeconfig: %w", err)
		result.MarkFailed(err, "failed to change ownership")
		return result, err
	}

	logger.Info("Successfully copied admin.conf to user's .kube directory.")
	result.MarkCompleted("kubeconfig copied successfully")
	return result, nil
}
