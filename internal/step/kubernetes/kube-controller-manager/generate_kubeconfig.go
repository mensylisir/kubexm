...
func NewCreateControllerManagerKubeconfigStepBuilder(ctx runtime.ExecutionContext, instanceName string) *CreateControllerManagerKubeconfigStepBuilder {
...
func (s *CreateControllerManagerKubeconfigStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		err = fmt.Errorf("failed to render kube-controller-manager.kubeconfig: %w", err)
	...
	if err := helpers.WriteContentToRemote(ctx, conn, kubeconfigContent, s.RemoteKubeconfigFile, "0644", s.Sudo); err != nil {
		err = fmt.Errorf("failed to write kube-controller-manager.kubeconfig file: %w", err)
