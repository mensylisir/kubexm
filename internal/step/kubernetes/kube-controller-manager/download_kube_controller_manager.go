...
func NewDownloadKubeControllerManagerStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadKubeControllerManagerStepBuilder {
...
func (s *DownloadKubeControllerManagerStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for arch := range requiredArchs {
		binaryInfo, err := provider.GetBinary(binary.ComponentKubeControllerManager, arch)
		if err != nil {
			err = fmt.Errorf("failed to get kube-controller-manager info for arch %s: %w", arch, err)
			result.MarkFailed(err, "failed to get binary info")
			return result, err
		}
		if binaryInfo == nil {
			err = fmt.Errorf("kube-controller-manager binary info not found for arch %s in BOM", arch)
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			err = fmt.Errorf("failed to download kube-controller-manager for arch %s: %w", arch, err)
