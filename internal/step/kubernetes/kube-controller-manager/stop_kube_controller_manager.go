...
func NewStopKubeControllerManagerStepBuilder(ctx runtime.ExecutionContext, instanceName string) *StopKubeControllerManagerStepBuilder {
...
func (s *StopKubeControllerManagerStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.StopService(ctx.GoContext(), conn, facts, s.ServiceName); err != nil {
		err = fmt.Errorf("failed to stop service '%s' on host %s: %w", s.ServiceName, ctx.GetHost().GetName(), err)
