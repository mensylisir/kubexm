...
func NewConfigureKubeControllerManagerStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ConfigureKubeControllerManagerStepBuilder {
...
func (s *ConfigureKubeControllerManagerStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		err = fmt.Errorf("failed to render kube-controller-manager config: %w", err)
	...
	if err := helpers.WriteContentToRemote(ctx, conn, configContent, s.RemoteConfigFile, "0644", s.Sudo); err != nil {
		err = fmt.Errorf("failed to write kube-controller-manager config file: %w", err)
