...
func (s *RestartKubeControllerManagerStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to determine kube-controller-manager service status: %w", err)
	}

	if !active {
		logger.Infof("Kube-controller-manager service is not active. Nothing to restart, skipping.")
		return true, nil
	}

	logger.Info("Kube-controller-manager service is active. Step needs to run to restart it.")
	return false, nil
}
