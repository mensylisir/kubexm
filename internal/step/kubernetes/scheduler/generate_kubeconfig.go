...
func NewCreateSchedulerKubeconfigStepBuilder(ctx runtime.ExecutionContext, instanceName string) *CreateSchedulerKubeconfigStepBuilder {
...
func (s *CreateSchedulerKubeconfigStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		err = fmt.Errorf("failed to render kube-scheduler.kubeconfig: %w", err)
	...
	if err := helpers.WriteContentToRemote(ctx, conn, kubeconfigContent, s.RemoteKubeconfigFile, "0644", s.Sudo); err != nil {
		err = fmt.Errorf("failed to write kube-scheduler.kubeconfig file: %w", err)
