...
func NewConfigureKubeSchedulerStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ConfigureKubeSchedulerStepBuilder {
...
func (s *ConfigureKubeSchedulerStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		err = fmt.Errorf("failed to render kube-scheduler config: %w", err)
	...
	if err := helpers.WriteContentToRemote(ctx, conn, configContent, s.RemoteConfigFile, "0644", s.Sudo); err != nil {
		err = fmt.Errorf("failed to write kube-scheduler config file: %w", err)
