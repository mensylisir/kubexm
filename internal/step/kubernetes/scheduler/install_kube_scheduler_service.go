...
func NewInstallKubeSchedulerServiceStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallKubeSchedulerServiceStepBuilder {
	k8sSpec := &struct {
		Scheduler *struct {
			ExtraArgs    map[string]string
	...
	if cfg := ctx.GetClusterConfig(); cfg != nil && cfg.Spec.Kubernetes != nil {
		if sched := cfg.Spec.Kubernetes.Scheduler; sched != nil {
			k8sSpec.Scheduler = &struct {
				ExtraArgs    map[string]string
				FeatureGates map[string]bool
			}{
				ExtraArgs:    sched.ExtraArgs,
				FeatureGates: sched.FeatureGates,
			}
		}
	}
...
func (s *InstallKubeSchedulerServiceStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		err = fmt.Errorf("failed to render kube-scheduler service file: %w", err)
