...
func NewGenerateKubeProxyCertsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateKubeProxyCertsStepBuilder {
...
func (s *GenerateKubeProxyCertsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		err = fmt.Errorf("failed to load main kubernetes CA: %w", err)
	...
	if err := helpers.NewSignedCertificate(s.CertsDir, common.KubeProxyClientCertFileName, common.KubeProxyClientKeyFileName, kubeProxyClientCfg, mainCA, mainKey); err != nil {
		err = fmt.Errorf("failed to generate kube-proxy client certificate: %w", err)
