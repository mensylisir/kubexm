...
type StartKubeProxyStep struct {
	step.Base
	ServiceName string
}

type StartKubeProxyStepBuilder struct {
	step.Builder[StartKubeProxyStepBuilder, *StartKubeProxyStep]
}

func NewStartKubeProxyStepBuilder(ctx runtime.ExecutionContext, instanceName string) *StartKubeProxyStepBuilder {
	s := &StartKubeProxyStep{
		ServiceName: "kube-proxy.service",
	}

	s.Base.Meta.Name = instanceName
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Start kube-proxy service", s.Base.Meta.Name)
	s.Base.Sudo = false
	s.Base.IgnoreError = false
	s.Base.Timeout = 3 * time.Minute

	b := new(StartKubeProxyStepBuilder).Init(s)
	return b
}

func (s *StartKubeProxyStep) Meta() *spec.StepMeta {
	return &s.Base.Meta
}

func (s *StartKubeProxyStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
...
func (s *StartKubeProxyStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
...
func (s *StartKubeProxyStep) Rollback(ctx runtime.ExecutionContext) error {
...
var _ step.Step = (*StartKubeProxyStep)(nil)
