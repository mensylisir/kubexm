...
func NewCreateKubeProxyConfigYAMLStepBuilder(ctx runtime.ExecutionContext, instanceName string) *CreateKubeProxyConfigYAMLStepBuilder {
...
func (s *CreateKubeProxyConfigYAMLStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, remoteDir, "0755", s.Sudo); err != nil {
		err = fmt.Errorf("failed to create directory for kube-proxy config: %w", err)
	...
	if err := helpers.WriteContentToRemote(ctx, conn, content, s.RemoteConfigYAMLFile, "0644", s.Sudo); err != nil {
		err = fmt.Errorf("failed to write config file: %w", err)
		result.MarkFailed(err, "failed to write config file")
		return result, err
	}
	result.MarkCompleted("config file created successfully")
	return result, nil
}
