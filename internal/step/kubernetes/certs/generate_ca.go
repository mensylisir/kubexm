...
func NewGenerateKubeCAStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateKubeCAStepBuilder {
...
func (s *GenerateKubeCAStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := os.MkdirAll(s.LocalCertsDir, common.DefaultCertificateDirPermission); err != nil {
		...
		result.MarkFailed(err, "failed to create certs directory")
		return result, err
	}
	...
	if err != nil {
		err = fmt.Errorf("failed to setup main Kubernetes CA: %w", err)
		result.MarkFailed(err, "failed to setup main CA")
		return result, err
	}
	...
	if err != nil {
		err = fmt.Errorf("failed to setup front-proxy CA: %w", err)
		result.MarkFailed(err, "failed to setup front-proxy CA")
		return result, err
	}
	...
	if err := helpers.NewServiceAccountKeyPair(s.LocalCertsDir, common.ServiceAccountPublicKeyFileName, common.ServiceAccountPrivateKeyFileName); err != nil {
		err = fmt.Errorf("failed to generate service account key pair: %w", err)
		result.MarkFailed(err, "failed to generate service account key pair")
		return result, err
	}

	logger.Info("All Kubernetes root CAs and service account keys are ready.")
	result.MarkCompleted("CAs and keys ready")
	return result, nil
}
