...
func NewDistributeKubeCertsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DistributeKubeCertsStepBuilder {
...
func (s *DistributeKubeCertsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, s.RemoteCertsDir, s.PermissionDir, s.Sudo); err != nil {
		err = fmt.Errorf("failed to create remote PKI directory '%s' on host %s: %w", s.RemoteCertsDir, ctx.GetHost().GetName(), err)
		result.MarkFailed(err, "failed to create remote PKI directory")
		return result, err
	}
	...
	for _, fileName := range s.filesToDistribute() {
		...
		if _, err := os.Stat(localPath); os.IsNotExist(err) {
			err = fmt.Errorf("local source certificate file '%s' not found, ensure previous generation steps ran successfully", localPath)
			result.MarkFailed(err, "local source certificate not found")
			return result, err
		}
		...
		if err := runner.Upload(ctx.GoContext(), conn, localPath, remotePath, s.Sudo); err != nil {
			err = fmt.Errorf("failed to upload '%s' to host %s: %w", fileName, ctx.GetHost().GetName(), err)
			result.MarkFailed(err, "failed to upload certificate")
			return result, err
		}
		...
		if err := runner.Chmod(ctx.GoContext(), conn, remotePath, permission, s.Sudo); err != nil {
			err = fmt.Errorf("failed to set permission '%s' on file '%s' on host %s: %w", permission, remotePath, ctx.GetHost().GetName(), err)
	...
	result.MarkCompleted("certificates distributed successfully")
	return result, nil
}
