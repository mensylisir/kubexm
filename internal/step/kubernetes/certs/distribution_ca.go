...
func NewDistributeKubeCACertsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DistributeKubeCACertsStepBuilder {
...
func (s *DistributeKubeCACertsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for _, fileName := range s.filesToDistribute() {
		...
		if _, err := os.Stat(localPath); os.IsNotExist(err) {
			err = fmt.Errorf("local source CA file '%s' not found, ensure CA generation step ran successfully", localPath)
			result.MarkFailed(err, "local source CA file not found")
			return result, err
		}
		...
		if err := runner.Upload(ctx.GoContext(), conn, localPath, remotePath, s.Sudo); err != nil {
			err = fmt.Errorf("failed to upload CA file '%s' to host %s: %w", fileName, ctx.GetHost().GetName(), err)
			result.MarkFailed(err, "failed to upload CA file")
			return result, err
		}
		...
		if err := runner.Chmod(ctx.GoContext(), conn, remotePath, "0644", s.Sudo); err != nil {
			err = fmt.Errorf("failed to set permission on CA file '%s' on host %s: %w", remotePath, ctx.GetHost().GetName(), err)
	...
	result.MarkCompleted("CA certificates distributed successfully")
	return result, nil
}
