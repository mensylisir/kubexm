...
func NewGenerateKubeconfigsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateKubeconfigsStepBuilder {
...
func (s *GenerateKubeconfigsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "phase", "Run")

	if err := os.MkdirAll(s.OutputDir, 0755); err != nil {
		err = fmt.Errorf("failed to create output directory for kubeconfigs: %w", err)
		result.MarkFailed(err, "failed to create output directory")
		return result, err
	}
	caCertPath := filepath.Join(s.CertsDir, common.CACertFileName)
	caCertData, err := os.ReadFile(caCertPath)
	if err != nil {
		err = fmt.Errorf("failed to read CA certificate from %s: %w", caCertPath, err)
		result.MarkFailed(err, "failed to read CA certificate")
		return result, err
	}
	...
	for _, def := range s.getKubeconfigDefinitions() {
		...
		if err := clientcmd.WriteToFile(*config, outputPath); err != nil {
			err = fmt.Errorf("failed to write kubeconfig file %s: %w", outputPath, err)
			result.MarkFailed(err, "failed to write kubeconfig file")
			return result, err
		}
	}
