...
func NewGenerateKubeCertsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateKubeCertsStepBuilder {
...
func (s *GenerateKubeCertsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		err = fmt.Errorf("failed to load main kubernetes CA from %s: %w. Ensure GenerateKubeCAStep ran successfully", s.KubeCertsDir, err)
	...
	if err != nil {
		err = fmt.Errorf("failed to load front-proxy CA from %s: %w. Ensure GenerateKubeCAStep ran successfully", s.KubeCertsDir, err)
	...
	defs, err := s.getCertDefinitions(ctx)
	if err != nil {
		err = fmt.Errorf("failed to get certificate definitions: %w", err)
	...
	for name, def := range defs {
		...
		if !ok {
			err = fmt.Errorf("unknown CA name '%s' for certificate '%s'", def.caName, name)
			result.MarkFailed(err, "unknown CA name")
			return result, err
		}

		err := helpers.NewSignedCertificate(s.KubeCertsDir, def.certFile, def.keyFile, def.config, ca.Certificate, ca.PrivateKey)
		if err != nil {
			err = fmt.Errorf("failed to generate certificate for %s: %w", name, err)
			result.MarkFailed(err, "failed to generate certificate")
			return result, err
		}
	}

	logger.Info("All Kubernetes component certificates generated successfully.")
	result.MarkCompleted("certificates generated successfully")
	return result, nil
}
