...
func NewRenderRepoNginxConfigStepBuilder(ctx runtime.ExecutionContext, instanceName string) *RenderRepoNginxConfigStepBuilder {
...
func (s *RenderRepoNginxConfigStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to render nginx config content")
		return result, err
	}
	...
	if err := helpers.WriteContentToRemote(ctx, conn, string(renderedConfig), s.ConfigPath, "0644", s.Sudo); err != nil {
		err = errors.Wrap(err, "failed to upload repository nginx config")
		result.MarkFailed(err, "failed to upload nginx config")
		return result, err
	}

	result.MarkCompleted("NGINX config rendered and uploaded successfully")
	return result, nil
}
