...
func NewDeployRepoNginxPodStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DeployRepoNginxPodStepBuilder {
...
func (s *DeployRepoNginxPodStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to render nginx static pod content")
		return result, err
	}

	staticPodDir := common.KubernetesManifestsDir
	if err := runner.Mkdirp(ctx.GoContext(), conn, staticPodDir, "0755", true); err != nil {
		err = fmt.Errorf("failed to create static pod directory '%s': %w", staticPodDir, err)
	...
	if err := helpers.WriteContentToRemote(ctx, conn, string(renderedConfig), remoteManifestPath, "0644", s.Sudo); err != nil {
		err = fmt.Errorf("failed to upload repository NGINX static pod manifest: %w", err)
		result.MarkFailed(err, "failed to upload nginx static pod manifest")
		return result, err
	}

	result.MarkCompleted("NGINX static pod deployed successfully")
	return result, nil
}
