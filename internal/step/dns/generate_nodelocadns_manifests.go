...
func NewGenerateNodeLocalDNSArtifactsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateNodeLocalDNSArtifactsStepBuilder {
...
func (s *GenerateNodeLocalDNSArtifactsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to get embedded nodelocaldns.Corefile.tmpl")
		return result, err
	}
	...
	if err != nil {
		result.MarkFailed(err, "failed to parse nodelocaldns.Corefile.tmpl")
		return result, err
	}

	var corefileBuffer bytes.Buffer
	if err := corefileTmpl.Execute(&corefileBuffer, s); err != nil {
		result.MarkFailed(err, "failed to render nodelocaldns.Corefile.tmpl")
		return result, err
	}
	...
	if err != nil {
		result.MarkFailed(err, "failed to get embedded nodelocaldns.yaml.tmpl")
		return result, err
	}
	...
	if err != nil {
		result.MarkFailed(err, "failed to parse nodelocaldns.yaml.tmpl")
		return result, err
	}

	var finalManifestBuffer bytes.Buffer
	if err := mainTmpl.Execute(&finalManifestBuffer, s); err != nil {
		result.MarkFailed(err, "failed to render nodelocaldns.yaml.tmpl")
		return result, err
	}
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, remoteDir, "0755", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to create remote dir")
		return result, err
	}
	...
	if err := helpers.WriteContentToRemote(ctx, conn, finalManifestBuffer.String(), s.RemoteManifestPath, "0644", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to upload NodeLocalDNS manifest")
		return result, err
	}

	logger.Info("NodeLocalDNS manifest generated and uploaded successfully.")
	result.MarkCompleted("NodeLocalDNS manifest generated and uploaded successfully")
	return result, nil
}
