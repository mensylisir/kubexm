...
func NewGenerateCoreDNSArtifactsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateCoreDNSArtifactsStepBuilder {
...
func (s *GenerateCoreDNSArtifactsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to get embedded Corefile.tmpl")
		return result, err
	}
	...
	if err != nil {
		result.MarkFailed(err, "failed to parse Corefile.tmpl")
		return result, err
	}

	var corefileBuffer bytes.Buffer
	if err := corefileTmpl.Execute(&corefileBuffer, s); err != nil {
		result.MarkFailed(err, "failed to render Corefile.tmpl")
		return result, err
	}
	...
	if err != nil {
		result.MarkFailed(err, "failed to get embedded coredns.yaml.tmpl")
		return result, err
	}
	...
	if err != nil {
		result.MarkFailed(err, "failed to parse coredns.yaml.tmpl")
		return result, err
	}

	var finalManifestBuffer bytes.Buffer
	if err := mainTmpl.Execute(&finalManifestBuffer, s); err != nil {
		result.MarkFailed(err, "failed to render coredns.yaml.tmpl")
		return result, err
	}
	...
	if err := helpers.WriteContentToRemote(ctx, conn, finalManifestBuffer.String(), s.RemoteManifestPath, "0644", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to upload CoreDNS manifest")
		return result, err
	}

	logger.Info("CoreDNS manifest generated and uploaded successfully.")
	result.MarkCompleted("CoreDNS manifest generated and uploaded successfully")
	return result, nil
}
