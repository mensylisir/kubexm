...
type Step interface {
	...
	// Run 执行Step的核心逻辑
	// 返回 (*types.StepResult, error)
	// result.Status == types.StepStatusCompleted 表示成功
	// result.Status == types.StepStatusSkipped 表示跳过
	// error != nil 表示执行失败
	...
	// 用于在执行过程中查询状态
	GetStatus(ctx runtime.ExecutionContext) (types.StepStatus, error)
