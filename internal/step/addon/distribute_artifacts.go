...
func NewDistributeAddonArtifactsStepBuilder(ctx runtime.ExecutionContext, addonName string) *DistributeAddonArtifactsStepBuilder {
...
func (s *DistributeAddonArtifactsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "host", ctx.GetHost().GetName(), "phase", "Run")

	artifacts, err := s.getArtifactsToDistribute(ctx)
	if err != nil {
		result.MarkFailed(err, "failed to get artifacts to distribute")
		return result, err
	}
	...
	for _, artifact := range artifacts {
		...
		if err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to read local artifact %s", artifact.LocalPath))
			return result, err
		}
		...
		if err := helpers.WriteContentToRemote(ctx, conn, string(content), artifact.RemotePath, "0644", s.Sudo); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to upload artifact to %s", artifact.RemotePath))
			return result, err
		}
	}

	logger.Info("Successfully distributed all artifacts for addon.")
	result.MarkCompleted("all artifacts distributed successfully")
	return result, nil
}
