...
func NewInstallIngressNginxHelmChartStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallIngressNginxHelmChartStepBuilder {
...
func (s *InstallIngressNginxHelmChartStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	logger.Debug("All required Ingress-Nginx artifacts found on remote host.")
...
func (s *InstallIngressNginxHelmChartStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to install/upgrade Ingress-Nginx Helm chart")
		return result, err
	}

	logger.Info("Ingress-Nginx Helm chart installed/upgraded successfully.")
	logger.Debugf("Helm command output:\n%s", runResult.Stdout)
	result.MarkCompleted("Ingress-Nginx Helm chart installed/upgraded successfully")
	return result, nil
}
