...
func (s *LoadKernelModulesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to check for existence of '%s'"); return result, err
	}
	s.configFileCreated = !fileExists
	if fileExists {
		contentBytes, err := runner.ReadFile(ctx.GoContext(), conn, filePath)
		if err != nil {
			result.MarkFailed(err, "failed to read existing file '%s' for rollback state"); return result, err
		}
	...
	if len(availableModules) > 0 {
		...
		if err != nil {
			result.MarkFailed(err, "failed to write modules config file '%s'"); return result, err
		}
	} else {
