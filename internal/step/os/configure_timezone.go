...
func (s *ConfigureTimezoneStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if _, err := runner.Run(ctx.GoContext(), conn, setTimezoneCmd, s.Sudo); err != nil {
		err := fmt.Errorf("failed to set timezone to '%s': %w", s.Timezone, err)
		result.MarkFailed(err, "failed to set timezone")
		return result, err
	}
