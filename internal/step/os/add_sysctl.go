...
func (s *ConfigureSysctlStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to write sysctl config file '%s'"); return result, err
	}

	logger.Info("Applying new sysctl settings...")
	if _, err := runner.Run(ctx.GoContext(), conn, "sysctl --system", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to apply sysctl settings with 'sysctl --system'"); return result, err
	}
