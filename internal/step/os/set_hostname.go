...
func (s *SetHostnameStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		err := fmt.Errorf("failed to set hostname to '%s': %w", s.Hostname, err)
		result.MarkFailed(err, "failed to set hostname")
		return result, err
	}
