...
func NewConfigureRegistriesStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ConfigureRegistriesStepBuilder {
...
func (s *ConfigureRegistriesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, targetDir, "0755", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to create registries config directory")
		return result, fmt.Errorf("failed to create registries config directory '%s': %w", targetDir, err)
	}
	...
	if err := helpers.WriteContentToRemote(ctx, conn, content, s.TargetPath, "0644", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to write registries config")
		return result, err
	}
	result.MarkCompleted("registries configured successfully")
	return result, nil
}
