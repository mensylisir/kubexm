...
func NewInstallCrioEnvStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallCrioEnvStepBuilder {
...
func (s *InstallCrioEnvStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to get target env path")
		return result, err
	}
	...
	if err != nil {
		result.MarkFailed(err, "failed to write content to remote")
		return result, err
	}
	logger.Infof("CRI-O environment file installed successfully to %s.", targetFile)
	result.MarkCompleted("CRI-O environment file installed successfully")
	return result, nil
}
