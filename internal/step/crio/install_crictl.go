...
func (s *InstallCrictlStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, "failed to get extracted source dir")
		return result, err
	}
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, targetDir, "0755", s.Sudo); err != nil {
		...
		return result, fmt.Errorf("failed to create target directory '%s': %w", targetDir, err)
	}
	...
	if err := runner.Upload(ctx.GoContext(), conn, sourceFile, remoteTempPath, false); err != nil {
		...
		return result, fmt.Errorf("failed to upload '%s': %w", sourceFile, err)
	}
	...
	if _, _, err := runner.OriginRun(ctx.GoContext(), conn, moveCmd, s.Sudo); err != nil {
		...
		return result, fmt.Errorf("failed to move file to '%s': %w", targetFile, err)
	}
	...
	if err := runner.Chmod(ctx.GoContext(), conn, targetFile, "0755", s.Sudo); err != nil {
		...
		return result, fmt.Errorf("failed to set permission on '%s': %w", targetFile, err)
	}
