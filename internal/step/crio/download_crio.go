...
const ComponentCrio = "crio"

type DownloadCrioStep struct {
	step.Base
}

type DownloadCrioStepBuilder struct {
	step.Builder[DownloadCrioStepBuilder, *DownloadCrioStep]
}

func NewDownloadCrioStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadCrioStepBuilder {
	provider := binary.NewBinaryProvider(ctx)
	const representativeArch = "amd64"
	binaryInfo, err := provider.GetBinary(ComponentCrio, representativeArch)

	if err != nil || binaryInfo == nil {
		ctx.GetLogger().Debugf("CRI-O component not found in BOM or not enabled, skipping creation of DownloadCrioStep.")
		return nil
	}

	s := &DownloadCrioStep{}
	s.Base.Meta.Name = instanceName
	s.Base.Meta.Description = fmt.Sprintf("[%s]>>Download CRI-O binaries for all required architectures", s.Base.Meta.Name)
	s.Base.Sudo = false
	s.Base.IgnoreError = false
	s.Base.Timeout = 10 * time.Minute

	b := new(DownloadCrioStepBuilder).Init(s)
	return b
}

func (s *DownloadCrioStep) Meta() *spec.StepMeta {
	return &s.Base.Meta
}

func (s *DownloadCrioStep) getRequiredArchs(ctx runtime.ExecutionContext) (map[string]bool, error) {
...
func (s *DownloadCrioStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if len(requiredArchs) == 0 {
		logger.Info("No hosts require CRI-O in this cluster configuration. Step is done.")
		return true, nil
	}
	...
	for arch := range requiredArchs {
		...
		if err != nil {
			return false, fmt.Errorf("failed to get CRI-O info for arch %s: %w", arch, err)
		}
		if binaryInfo == nil {
			continue
		}
		...
		logger.Infof("Checking for CRI-O (arch: %s) at: %s", arch, destPath)
	...
	if allDone {
		logger.Info("All required CRI-O binaries for all architectures already exist and are valid.")
		return true, nil
	}
	return false, nil
}

func (s *DownloadCrioStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(requiredArchs) == 0 {
		logger.Info("No hosts require CRI-O in this cluster configuration. Skipping download.")
		result.MarkCompleted("skipping - no hosts require CRI-O")
		return result, nil
	}
	...
	for arch := range requiredArchs {
		binaryInfo, err := provider.GetBinary(ComponentCrio, arch)
		if err != nil {
			result.MarkFailed(err, "failed to get CRI-O info")
			return result, fmt.Errorf("failed to get CRI-O info for arch %s: %w", arch, err)
		}
		if binaryInfo == nil {
			continue
		}
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			result.MarkFailed(err, "failed to download CRI-O")
			return result, fmt.Errorf("failed to download CRI-O for arch %s: %w", arch, err)
		}
	}

	logger.Info("All required CRI-O binaries have been downloaded successfully.")
	result.MarkCompleted("CRI-O downloaded successfully")
	return result, nil
}

func (s *DownloadCrioStep) downloadFile(ctx runtime.ExecutionContext, binaryInfo *binary.Binary) error {
	...
	logger.Infof("Downloading CRI-O (arch: %s, version: %s) from %s ...", binaryInfo.Arch, binaryInfo.Version, binaryInfo.URL())
...
func (s *DownloadCrioStep) Rollback(ctx runtime.ExecutionContext) error {
...
var _ step.Step = (*DownloadCrioStep)(nil)
