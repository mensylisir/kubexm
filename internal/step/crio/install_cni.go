...
func NewInstallCniStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallCniStepBuilder {
...
func (s *InstallCniStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Upload(ctx.GoContext(), conn, sourceDir, remoteUploadTmpDir, false); err != nil {
		result.MarkFailed(err, "failed to upload CNI plugins directory")
		return result, fmt.Errorf("failed to upload CNI plugins directory: %w", err)
	}
	defer func() {
	...
	if _, _, err := runner.OriginRun(ctx.GoContext(), conn, moveAndChmodCmd, s.Sudo); err != nil {
		result.MarkFailed(err, "failed to copy and set permissions for CNI plugins")
		return result, fmt.Errorf("failed to copy and set permissions for CNI plugins: %w", err)
	}
