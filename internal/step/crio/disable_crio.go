...
func NewDisableCrioStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DisableCrioStepBuilder {
	s := &DisableCrioStep{}
...
func (s *DisableCrioStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		...
		return result, fmt.Errorf("failed to gather facts to disable service: %w", err)
	}

	logger.Infof("Disabling CRI-O service...")
	if err := runner.DisableService(ctx.GoContext(), conn, facts, crioServiceName); err != nil {
		result.MarkFailed(err, "failed to disable CRI-O service")
		return result, fmt.Errorf("failed to disable CRI-O service: %w", err)
	}

	logger.Info("CRI-O service disabled successfully.")
	result.MarkCompleted("CRI-O disabled successfully")
	return result, nil
}
