...
func NewInstallCrioStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallCrioStepBuilder {
...
func (s *InstallCrioStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for dir := range targetDirs {
		if err := runner.Mkdirp(ctx.GoContext(), conn, dir, "0755", s.Sudo); err != nil {
			...
			return result, fmt.Errorf("failed to create target directory '%s': %w", dir, err)
		}
	}
	...
	for file, targetDir := range filesToInstall {
		...
		if err := runner.Upload(ctx.GoContext(), conn, sourceFile, remoteTempPath, false); err != nil {
			result.MarkFailed(err, "failed to upload CRI-O binary")
		...
		if _, err := runner.Run(ctx.GoContext(), conn, moveCmd, s.Sudo); err != nil {
			result.MarkFailed(err, "failed to move CRI-O binary")
	...
	result.MarkCompleted("CRI-O installed successfully")
	return result, nil
}
