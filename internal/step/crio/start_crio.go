...
func NewStartCrioStepBuilder(ctx runtime.ExecutionContext, instanceName string) *StartCrioStepBuilder {
	s := &StartCrioStep{}
...
func (s *StartCrioStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.StartService(ctx.GoContext(), conn, facts, crioServiceName); err != nil {
		result.MarkFailed(err, "failed to start CRI-O service")
		return result, fmt.Errorf("failed to start CRI-O service: %w", err)
	}

	logger.Info("CRI-O service started successfully.")
	result.MarkCompleted("CRI-O service started successfully")
	return result, nil
}
