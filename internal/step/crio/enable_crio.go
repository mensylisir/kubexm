...
func NewEnableCrioStepBuilder(ctx runtime.ExecutionContext, instanceName string) *EnableCrioStepBuilder {
	s := &EnableCrioStep{}
...
func (s *EnableCrioStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.EnableService(ctx.GoContext(), conn, facts, crioServiceName); err != nil {
		result.MarkFailed(err, "failed to enable cri-o service")
		return result, fmt.Errorf("failed to enable cri-o service: %w", err)
	}

	logger.Info("CRI-O service enabled successfully.")
	result.MarkCompleted("CRI-O enabled successfully")
	return result, nil
}
