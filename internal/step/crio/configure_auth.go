...
func NewConfigureAuthStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ConfigureAuthStepBuilder {
...
func (s *ConfigureAuthStep) Precheck(ctx runtime.ExecutionContext) (isDone bool, err error) {
	...
	if exists {
		...
		if err != nil {
			logger.Warn(err, "Auth file exists but failed to read, will overwrite.", "path", s.TargetPath)
			return false, nil
		}

		if string(remoteContent) == expectedContent {
			logger.Info("Auth file already exists and content matches. Step is done.", "path", s.TargetPath)
			return true, nil
		}

		logger.Info("Auth file exists but content differs. Step needs to run.", "path", s.TargetPath)
		return false, nil
	}

	logger.Info("Auth file does not exist. Configuration is required.", "path", s.TargetPath)
	return false, nil
}

func (s *ConfigureAuthStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if content == "" {
		...
		result.MarkCompleted("skipping - no valid authentications")
		return result, nil
	}

	targetDir := filepath.Dir(s.TargetPath)
	if err := runner.Mkdirp(ctx.GoContext(), conn, targetDir, "0755", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to create auth config directory")
		return result, fmt.Errorf("failed to create auth config directory '%s': %w", targetDir, err)
	}

	logger.Info("Writing authentication config file.", "path", s.TargetPath)
	if err := helpers.WriteContentToRemote(ctx, conn, content, s.TargetPath, "0600", s.Sudo); err != nil {
		result.MarkFailed(err, "failed to write authentication config")
		return result, err
	}
	result.MarkCompleted("authentication configured successfully")
	return result, nil
}
