...
func NewInstallOfflinePackagesStepBuilder(ctx runtime.ExecutionContext, instanceName string) *InstallOfflinePackagesStepBuilder {
...
func (s *InstallOfflinePackagesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	facts, err := ctx.GetHostFacts(ctx.GetHost())
	if err != nil {
		result.MarkFailed(err, "failed to gather facts to determine offline package")
		return result, err
	}
	...
	if facts.PackageManager == nil || facts.PackageManager.Type == runner.PackageManagerUnknown {
		result.MarkFailed(fmt.Errorf("cannot perform offline install without a known package manager"), "unsupported package manager")
		return result, fmt.Errorf("unsupported package manager for offline installation: %s", facts.PackageManager.Type)
	}
	...
	default:
		result.MarkFailed(fmt.Errorf("unsupported package manager for offline installation: %s", facts.PackageManager.Type), "unsupported package manager")
		return result, fmt.Errorf("unsupported package manager for offline installation: %s", facts.PackageManager.Type)
	}
	...
	if _, err := runnerSvc.Run(ctx.GoContext(), conn, installCmd, s.Sudo); err != nil {
		result.MarkFailed(err, "failed to install offline packages")
		return result, err
	}
	...
	result.MarkCompleted("offline packages installed successfully")
	return result, nil
}
