...
type ExtendedStep interface {
	...
	Retry(ctx runtime.ExecutionContext) (*types.StepResult, error)                   // Execute retry logic
...
func (s *StepWithRetry) Retry(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	return s.Run(ctx)
}
...
func (s *StepWithRetry) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	var lastErr error
	delay := s.RetryPolicy.RetryDelay

	for attempt := 0; attempt <= s.RetryPolicy.MaxRetries; attempt++ {
		result, err := s.Step.Run(ctx)
		if err == nil && (result == nil || result.Status != types.StepStatusFailed) {
			return result, nil
		}
		...
		if !retryable && (result != nil && result.Status != types.StepStatusFailed) {
			return result, nil
		}
