...
func NewEnableChronyStepBuilder(ctx runtime.ExecutionContext, instanceName string) *EnableChronyStepBuilder {
	s := &EnableChronyStep{
...
func (s *EnableChronyStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.EnableService(ctx.GoContext(), conn, facts, s.ServiceName); err != nil {
		result.MarkFailed(fmt.Errorf("failed to enable service '%s': %w", s.ServiceName, err), "Failed to enable service")
		return result, err
	}
