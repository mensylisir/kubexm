...
func NewVerifyTimeSyncStepBuilder(ctx runtime.ExecutionContext, instanceName string) *VerifyTimeSyncStepBuilder {
...
func (s *VerifyTimeSyncStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for {
		...
		case <-timeout:
			if lastErr != nil {
				err := fmt.Errorf("time synchronization verification timed out after %v: %w", s.Base.Timeout, lastErr)
				result.MarkFailed(err, "Time synchronization timed out with last error")
				return result, err
			}
			err := fmt.Errorf("time synchronization verification timed out after %v", s.Base.Timeout)
			result.MarkFailed(err, "Time synchronization timed out")
			return result, err
		case <-ticker.C:
			synced, err := s.checkSyncStatus(ctx)
			if synced {
				logger.Info("Time synchronization successful!")
				result.MarkCompleted("Time synchronization successful")
				return result, nil
			}
			if err != nil {
				lastErr = err
