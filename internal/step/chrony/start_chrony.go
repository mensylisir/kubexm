...
func NewStartChronyStepBuilder(ctx runtime.ExecutionContext, instanceName string) *StartChronyStepBuilder {
	s := &StartChronyStep{
...
func (s *StartChronyStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.StartService(ctx.GoContext(), conn, facts, s.ServiceName); err != nil {
		...
		result.MarkFailed(fmt.Errorf("failed to start service '%s': %w", s.ServiceName, err), "Failed to start service")
		return result, err
	}

	active, err := runner.IsServiceActive(ctx.GoContext(), conn, facts, s.ServiceName)
	if err != nil {
		result.MarkFailed(fmt.Errorf("failed to verify service status for %s after starting: %w", s.ServiceName, err), "Failed to verify service status")
		return result, err
	}
	if !active {
		err := fmt.Errorf("service %s did not become active after start command", s.ServiceName)
		result.MarkFailed(err, "Service did not become active")
		return result, err
	}
