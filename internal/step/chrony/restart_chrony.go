...
func NewRestartChronyStepBuilder(ctx runtime.ExecutionContext, instanceName string) *RestartChronyStepBuilder {
	s := &RestartChronyStep{
...
func (s *RestartChronyStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.RestartService(ctx.GoContext(), conn, facts, s.ServiceName); err != nil {
		...
		result.MarkFailed(fmt.Errorf("failed to restart service '%s': %w", s.ServiceName, err), "Failed to restart service")
		return result, err
	}
	...
	if err != nil {
		result.MarkFailed(fmt.Errorf("failed to verify service status for %s after restarting: %w", s.ServiceName, err), "Failed to verify service status")
		return result, err
	}
	if !active {
