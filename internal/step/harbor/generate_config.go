...
func NewGenerateHarborConfigStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateHarborConfigStepBuilder {
...
func (s *GenerateHarborConfigStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		err := fmt.Errorf("failed to read official harbor.yml.tmpl: %w", err)
	...
	if err != nil {
		err := fmt.Errorf("failed to render official harbor template: %w", err)
		result.MarkFailed(err, err.Error())
		return result, err
	}
	...
	if err := os.WriteFile(s.RenderedConfigPath, []byte(finalContent), 0644); err != nil {
		err := fmt.Errorf("failed to write final harbor.yml: %w", err)
		result.MarkFailed(err, err.Error())
		return result, err
	}

	logger.Info("Successfully generated harbor.yml configuration file.")
	result.MarkCompleted("Harbor configuration file generated successfully")
	return result, nil
}
