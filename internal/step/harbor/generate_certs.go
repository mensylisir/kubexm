...
func NewGenerateHarborCertsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateHarborCertsStepBuilder {
...
func (s *GenerateHarborCertsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if cfg.Registry == nil || cfg.Registry.MirroringAndRewriting == nil || cfg.Registry.MirroringAndRewriting.PrivateRegistry == "" {
		err := fmt.Errorf("`registry.mirroringAndRewriting.privateRegistry` must be set to generate Harbor certificate")
	...
	if len(registryHosts) == 0 {
		err := fmt.Errorf("no host with role '%s' found", common.RoleRegistry)
		result.MarkFailed(err, err.Error())
		return result, err
	}
	...
	if err != nil {
		err := fmt.Errorf("failed to create/load Harbor CA: %w", err)
		result.MarkFailed(err, err.Error())
		return result, err
	}
	...
	if err := helpers.NewSignedCertificate(s.CertsDir, certFiles["serverCert"], certFiles["serverKey"], serverCertConfig, caCert, caKey); err != nil {
		err := fmt.Errorf("failed to generate Harbor server certificate: %w", err)
		result.MarkFailed(err, err.Error())
		return result, err
	}

	logger.Info("Successfully generated all Harbor certificates.")
	result.MarkCompleted("Harbor certificates generated successfully")
	return result, nil
}
