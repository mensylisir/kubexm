...
func NewDownloadHarborStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadHarborStepBuilder {
...
func (s *DownloadHarborStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(requiredArchs) == 0 {
		...
		result.MarkCompleted("No Harbor download required")
		return result, nil
	}
	...
	for arch := range requiredArchs {
		binaryInfo, err := provider.GetBinary(binary.ComponentHarbor, arch)
		if err != nil {
			err := fmt.Errorf("failed to get Harbor info for arch %s: %w", arch, err)
			result.MarkFailed(err, err.Error())
			return result, err
		}
		if binaryInfo == nil {
		...
		if err := s.downloadFile(ctx, binaryInfo); err != nil {
			err := fmt.Errorf("failed to download Harbor for arch %s: %w", arch, err)
			result.MarkFailed(err, err.Error())
			return result, err
		}
	}
	...
	result.MarkCompleted("Harbor installers downloaded successfully")
	return result, nil
}
