...
func NewStopAndRemoveHarborStepBuilder(ctx runtime.ExecutionContext, instanceName string) *StopAndRemoveHarborStepBuilder {
...
func (s *StopAndRemoveHarborStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if exists, _ := runner.Exists(ctx.GoContext(), conn, composeFilePath); !exists {
		...
		result.MarkCompleted("Harbor services already removed")
		return result, nil
	}
