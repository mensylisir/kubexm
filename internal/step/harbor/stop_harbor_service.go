...
func NewStopHarborServiceStepBuilder(ctx runtime.ExecutionContext, instanceName string) *StopHarborServiceStepBuilder {
...
func (s *StopHarborServiceStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if exists, _ := runner.Exists(ctx.GoContext(), conn, composeFilePath); !exists {
		...
		result.MarkCompleted("Harbor services already stopped")
		return result, nil
	}
