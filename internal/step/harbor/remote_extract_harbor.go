...
func NewExtractHarborInstallerStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ExtractHarborInstallerStepBuilder {
...
func (s *ExtractHarborInstallerStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if exists, _ := runner.Exists(ctx.GoContext(), conn, s.RemoteTempPath); !exists {
		err := fmt.Errorf("source archive '%s' not found on remote host, ensure upload step ran successfully", s.RemoteTempPath)
		result.MarkFailed(err, err.Error())
		return result, err
	}

	if err := runner.Mkdirp(ctx.GoContext(), conn, s.RemoteInstallRoot, "0755", s.Sudo); err != nil {
		err := fmt.Errorf("failed to create remote install root directory '%s': %w", s.RemoteInstallRoot, err)
		result.MarkFailed(err, err.Error())
		return result, err
	}
	...
	if _, err := runner.Run(ctx.GoContext(), conn, extractCmd, s.Sudo); err != nil {
		err := fmt.Errorf("failed to extract archive '%s' on remote host: %w", s.RemoteTempPath, err)
		result.MarkFailed(err, err.Error())
		return result, err
	}
	...
	result.MarkCompleted("Harbor installer extracted successfully")
	return result, nil
}
