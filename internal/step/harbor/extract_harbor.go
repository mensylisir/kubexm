...
func NewExtractHarborStepBuilder(ctx runtime.ExecutionContext, instanceName string) *ExtractHarborStepBuilder {
...
func (s *ExtractHarborStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(requiredArchs) == 0 {
		...
		result.MarkCompleted("No Harbor extraction required")
		return result, nil
	}
	...
	result.MarkCompleted("Harbor installer archives extracted successfully")
	return result, nil
}
