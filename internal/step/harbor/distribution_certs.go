...
func NewDistributeHarborCACertStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DistributeHarborCACertStepBuilder {
...
func (s *DistributeHarborCACertStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	localContent, err := os.ReadFile(s.LocalCACertPath)
	if err != nil {
		err := fmt.Errorf("failed to read local Harbor CA certificate '%s': %w", s.LocalCACertPath, err)
	...
	for _, targetDir := range targetPaths {
		...
		if err := runner.Mkdirp(ctx.GoContext(), conn, targetDir, "0755", s.Sudo); err != nil {
			err := fmt.Errorf("failed to create remote certs directory '%s': %w", targetDir, err)
			result.MarkFailed(err, err.Error())
			return result, err
		}
		...
		if err := helpers.WriteContentToRemote(ctx, conn, string(localContent), remoteCertPath, "0644", s.Sudo); err != nil {
			err := fmt.Errorf("failed to write remote CA certificate to '%s': %w", remoteCertPath, err)
			result.MarkFailed(err, err.Error())
			return result, err
		}
	}
	...
	result.MarkCompleted("Harbor CA certificate distributed successfully")
	return result, nil
}
