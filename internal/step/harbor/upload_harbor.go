...
func NewUploadHarborInstallerStepBuilder(ctx runtime.ExecutionContext, instanceName string) *UploadHarborInstallerStepBuilder {
...
func (s *UploadHarborInstallerStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, remoteDir, "0755", s.Sudo); err != nil {
		err := fmt.Errorf("failed to create remote upload directory '%s': %w", remoteDir, err)
		result.MarkFailed(err, err.Error())
		return result, err
	}
	...
	if err := runner.Upload(ctx.GoContext(), conn, localSourcePath, s.RemoteTempPath, s.Sudo); err != nil {
		err := fmt.Errorf("failed to upload '%s' to '%s': %w", localSourcePath, s.RemoteTempPath, err)
		result.MarkFailed(err, err.Error())
		return result, err
	}

	logger.Infof("Successfully uploaded Harbor installer to %s", s.RemoteTempPath)
	result.MarkCompleted("Harbor installer uploaded successfully")
	return result, nil
}
