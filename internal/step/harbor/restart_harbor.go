...
func NewRestartHarborStepBuilder(ctx runtime.ExecutionContext, instanceName string) *RestartHarborStepBuilder {
...
func (s *RestartHarborStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if exists, _ := runner.Exists(ctx.GoContext(), conn, composeFilePath); !exists {
		...
		result.MarkCompleted("Harbor docker-compose.yml not found, restart skipped")
		return result, nil
	}
