...
func NewStopNginxStepBuilder(ctx runtime.ExecutionContext, instanceName string) *StopNginxStepBuilder {
	s := &StopNginxStep{}
...
func (s *StopNginxStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.StopService(ctx.GoContext(), conn, facts, nginxServiceName); err != nil {
		result.MarkFailed(err, "failed to stop NGINX service")
		return result, err
	}

	logger.Info("NGINX service stopped successfully.")
	result.MarkCompleted("NGINX service stopped successfully")
	return result, nil
}
