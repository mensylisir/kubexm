...
func NewEnableNginxStepBuilder(ctx runtime.ExecutionContext, instanceName string) *EnableNginxStepBuilder {
	s := &EnableNginxStep{}
...
func (s *EnableNginxStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runner.EnableService(ctx.GoContext(), conn, facts, nginxServiceName); err != nil {
		result.MarkFailed(err, "failed to enable NGINX service")
		return result, err
	}

	logger.Info("NGINX service enabled successfully.")
	result.MarkCompleted("NGINX service enabled successfully")
	return result, nil
}
