...
func NewGenerateHAProxyConfigStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateHAProxyConfigStepBuilder {
...
func (s *GenerateHAProxyConfigStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := helpers.WriteContentToRemote(ctx, conn, string(renderedConfig), remoteConfigPath, "0644", true); err != nil {
		result.MarkFailed(err, "failed to upload HAProxy config file")
		return result, err
	}

	logger.Info("HAProxy configuration generated and uploaded successfully.")
	result.MarkCompleted("HAProxy configuration generated and uploaded successfully")
	return result, nil
}
