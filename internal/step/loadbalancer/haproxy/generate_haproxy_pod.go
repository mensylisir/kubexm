...
func NewGenerateHAProxyStaticPodStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateHAProxyStaticPodStepBuilder {
...
func (s *GenerateHAProxyStaticPodStep) renderContent(ctx runtime.ExecutionContext) ([]byte, error) {
	...
	templateContent, err := templates.Get("loadbalancer/haproxy/haproxy-static-pod.yaml.tmpl")
	if err != nil {
...
func (s *GenerateHAProxyStaticPodStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := helpers.WriteContentToRemote(ctx, conn, string(renderedConfig), remoteManifestPath, "0644", true); err != nil {
		result.MarkFailed(err, "failed to upload HAProxy static pod manifest")
		return result, err
	}

	logger.Info("HAProxy static pod manifest generated and uploaded successfully.")
	result.MarkCompleted("HAProxy static pod manifest generated and uploaded successfully")
	return result, nil
}
