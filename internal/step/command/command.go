...
func (s *CommandStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		...
		err := fmt.Errorf("failed to get connector for host %s for step %s: %w", ctx.GetHost().GetName(), s.Base.Meta.Name, err)
	...
	if runErr != nil {
		...
		if errors.As(runErr, &cmdErr) {
			if s.IgnoreError {
				...
				result.MarkCompleted("Command completed with ignored error")
				return result, nil
			}
			if cmdErr.ExitCode == s.ExpectedExitCode {
				...
				result.MarkCompleted(fmt.Sprintf("Command completed with expected exit code %d", cmdErr.ExitCode))
				return result, nil
			}
			...
			result.MarkFailed(cmdErr, fmt.Sprintf("unexpected exit code %d", cmdErr.ExitCode))
			return result, cmdErr
		}
		logger.Error(runErr, "Failed to execute command (non-CommandError).", "stdout", string(stdoutBytes), "stderr", string(stderrBytes))
		err := fmt.Errorf("command '%s' failed for step %s on host %s (non-CommandError): %w. Stdout: %s, Stderr: %s", s.Cmd, s.Base.Meta.Name, ctx.GetHost().GetName(), runErr, string(stdoutBytes), string(stderrBytes))
		result.MarkFailed(err, "command execution failed")
		return result, err
	}
	...
	if s.ExpectedExitCode == 0 {
		...
		result.MarkCompleted("Command completed successfully")
		return result, nil
	}
