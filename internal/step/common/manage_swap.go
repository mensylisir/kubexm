...
func (s *ManageSwapStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	fstabContent, err := s.readRemoteFile(ctx, "/etc/fstab")
	if err != nil && !os.IsNotExist(err) {
		result.MarkFailed(err, fmt.Sprintf("run: failed to read /etc/fstab: %v", err))
		return result, err
	}

	newFstabContent, err := s.rebuildFstabContent(fstabContent)
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to rebuild fstab content: %v", err))
		return result, err
	}
	...
	if err := s.atomicWriteRemoteFile(ctx, "/etc/fstab", newFstabContent); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to write fstab: %v", err))
		return result, err
	}
	...
	case SwapStateEnabled:
		...
		if err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to execute 'swapon -a': %v, stderr: %s", err, stderr))
			return result, err
		}
	default:
		result.MarkFailed(fmt.Errorf("unsupported swap state for run: %s", s.State), fmt.Sprintf("unsupported swap state: %s", s.State))
		return result, fmt.Errorf("unsupported swap state for run: %s", s.State)
	}

	logger.Infof("Swap successfully set to '%s'.", s.State)
	result.MarkCompleted(fmt.Sprintf("Swap state set to %s", s.State))
	return result, nil
}
