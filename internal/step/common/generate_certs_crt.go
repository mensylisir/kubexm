...
func NewGenerateCertsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateCertsStepBuilder {
	s := &GenerateCertsStep{
		LocalCertsDir:  ctx.GetKubernetesCertsDir(),
		CertDuration:   365 * 24 * time.Hour * 10,
		Permission:     "0755",
		CaCertFileName: common.CACertFileName,
		CaKeyFileName:  common.CAKeyFileName,
		CommonName:     "kubexm",
...
func (s *GenerateCertsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		err := fmt.Errorf("failed to load CA (cert: %s, key: %s), please ensure CA generation step ran successfully: %w", caCertPath, caKeyPath, err)
		result.MarkFailed(err, "failed to load CA")
		return result, err
	}
	...
	if err := helpers.NewSignedCertificate(s.LocalCertsDir, s.Cert, s.CertKey, certCfg, caCert, caKey); err != nil {
		err := fmt.Errorf("failed to generate signed certificate %s: %w", s.Cert, err)
	...
	result.MarkCompleted("Certificate generated successfully")
	return result, nil
}
