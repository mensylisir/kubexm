...
func (s *UploadFileStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			if s.AllowMissingSrc {
				...
				result.MarkSkipped("AllowMissingSrc: local file does not exist")
				return result, nil
			}
		}
		result.MarkFailed(err, fmt.Sprintf("run: failed to read local source file %s: %v", s.LocalSrcPath, err))
		return result, err
	}
	...
	conn, err := ctx.GetCurrentHostConnector()
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("run: failed to get connector for host %s: %v", ctx.GetHost().GetName(), err))
		return result, err
	}
	...
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to upload file to %s on host %s: %v", s.RemoteDestPath, ctx.GetHost().GetName(), err))
		return result, err
	}
	logger.Info("File uploaded successfully.")
	result.MarkCompleted(fmt.Sprintf("Uploaded %s to %s", s.LocalSrcPath, s.RemoteDestPath))
	return result, nil
}
