...
func (s *ManageServiceStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		...
		result.MarkFailed(err, fmt.Sprintf("systemctl command '%s' failed. Stderr: %s", cmd, string(stderr)))
		return result, err
	}

	logger.Info("Systemctl command executed successfully.", "command", cmd, "stdout", string(stdout))
	result.MarkCompleted(fmt.Sprintf("Service action '%s' completed successfully on %s", s.Action, s.ServiceName))
	return result, nil
}
