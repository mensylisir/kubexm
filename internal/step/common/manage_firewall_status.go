...
func (s *ManageFirewallStateStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	serviceName, tool, err := s.getFirewallServiceNameAndTool(ctx)
	if err != nil {
		result.MarkFailed(err, "failed to detect firewall service")
		return result, err
	}
	...
	if _, err := runner.Run(ctx.GoContext(), conn, runCmd, s.Sudo); err != nil {
		err := fmt.Errorf("failed to set firewall service state to '%s': %w", s.State, err)
		result.MarkFailed(err, "failed to set firewall state")
		return result, err
	}

	logger.Infof("Firewall service state successfully set to '%s'.", s.State)
	result.MarkCompleted("Firewall state set successfully")
	return result, nil
}
