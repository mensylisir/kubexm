...
func (b *PrintMessageStepBuilder) WithMessage(message string) *PrintMessageStepBuilder {
	b.Step.Message = util.GenerateASCIIArt(message, "")
	return b
}
...
func (s *PrintMessageStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "host", ctx.GetHost().GetName(), "phase", "Run")
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger.Info(s.Message)
	result.MarkCompleted("Print message completed")
	return result, nil
}
