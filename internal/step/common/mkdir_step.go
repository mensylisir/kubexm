...
func (s *MkdirStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		...
		result.MarkFailed(err, fmt.Sprintf("failed to create directory %s: %v", s.Path, err))
		return result, err
	}
	...
	if err != nil {
		...
		result.MarkFailed(err, fmt.Sprintf("failed to set permissions on directory %s: %v", s.Path, err))
		return result, err
	}

	logger.Info("Directory created/ensured successfully.", "path", s.Path)
	result.MarkCompleted(fmt.Sprintf("Successfully created directory %s with permissions %s", s.Path, permStr))
	return result, nil
}
