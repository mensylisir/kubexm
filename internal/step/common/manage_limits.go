...
func (s *ManageLimitsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "host", ctx.GetHost().GetName(), "phase", "Run")

	currentContent, err := ReadRemoteFile(ctx, limitsTargetPath, s.Sudo)
	if err != nil && !os.IsNotExist(err) {
		err := fmt.Errorf("run: failed to read %s: %w", limitsTargetPath, err)
		result.MarkFailed(err, "failed to read limits file")
		return result, err
	}

	newContent, err := s.rebuildLimitsContent(currentContent)
	if err != nil {
		err := fmt.Errorf("failed to rebuild limits content: %w", err)
		result.MarkFailed(err, "failed to rebuild limits content")
		return result, err
	}

	logger.Infof("Atomically writing limits configuration to %s", limitsTargetPath)
	if err := AtomicWriteRemoteFile(ctx, limitsTargetPath, newContent, s.Sudo); err != nil {
		result.MarkFailed(err, "failed to write limits file")
		return result, err
	}
	result.MarkCompleted("Security limits configured successfully")
	return result, nil
}
