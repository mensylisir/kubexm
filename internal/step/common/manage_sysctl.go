...
func (s *ManageSysctlStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to render sysctl config: %v", err))
		return result, err
	}
	...
	if err := AtomicWriteRemoteFile(ctx, sysctlTargetPath, content, s.Sudo); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to write sysctl config: %v", err))
		return result, err
	}
	...
	result.MarkCompleted(fmt.Sprintf("Sysctl configuration applied to %s", sysctlTargetPath))
	return result, nil
}
