...
func (s *ManageModulesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "host", ctx.GetHost().GetName(), "phase", "Run")

	modulesToEnsure, err := s.determineRequiredModules(ctx)
	if err != nil {
		err := fmt.Errorf("failed to determine required modules: %w", err)
		result.MarkFailed(err, "failed to determine required modules")
		return result, err
	}
	...
	if len(loadedModules) > 0 {
		...
		if err := atomicWriteRemoteFile(ctx, modulesTargetPath, []byte(content), s.Sudo); err != nil {
			result.MarkFailed(err, "failed to write modules config")
			return result, err
		}
	} else {
	...
	result.MarkCompleted("Kernel modules loaded successfully")
	return result, nil
}
