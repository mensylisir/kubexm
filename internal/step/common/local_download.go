...
func (s *DownloadFileStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := os.MkdirAll(destDir, 0755); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to create destination directory %s: %v", destDir, err))
		return result, err
	}
	...
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to create http request for %s: %v", s.URL, err))
		return result, err
	}
	resp, err := ctx.GetHttpClient().Do(req)
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to start download from %s: %v", s.URL, err))
		return result, err
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		err := fmt.Errorf("download request failed for %s: status %s", s.URL, resp.Status)
	...
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to create destination file %s: %v", tmpPath, err))
		return result, err
	}
	defer out.Close()
	...
	if err != nil {
		_ = os.Remove(tmpPath)
		result.MarkFailed(err, fmt.Sprintf("failed to write download content to %s: %v", tmpPath, err))
		return result, err
	}
	if err := os.Rename(tmpPath, s.DestPath); err != nil {
		_ = os.Remove(tmpPath)
		result.MarkFailed(err, fmt.Sprintf("failed to rename temp file to destination: %v", err))
		return result, err
	}
	if err := s.verifyChecksum(s.DestPath); err != nil {
		_ = os.Remove(s.DestPath)
		result.MarkFailed(err, fmt.Sprintf("downloaded file checksum verification failed for %s: %v", s.DestPath, err))
		return result, err
	}
	...
	result.MarkCompleted(fmt.Sprintf("Successfully downloaded to %s", s.DestPath))
	return result, nil
}
