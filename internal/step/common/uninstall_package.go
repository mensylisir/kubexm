...
func (s *UninstallPackageStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "host", ctx.GetHost().GetName(), "phase", "Run")
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	runnerSvc := ctx.GetRunner()
	conn, err := ctx.GetCurrentHostConnector()
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to get connector for host %s: %v", ctx.GetHost().GetName(), err))
		return result, err
	}
	facts, err := ctx.GetHostFacts(ctx.GetHost())
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to get host facts for %s: %v", ctx.GetHost().GetName(), err))
		return result, err
	}
	var toRemove []string
	for _, pkg := range s.Packages {
		installed, err := runnerSvc.IsPackageInstalled(ctx.GoContext(), conn, facts, pkg)
		if err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to check package %s: %v", pkg, err))
			return result, err
		}
		if installed {
	...
	if len(toRemove) == 0 {
		...
		result.MarkCompleted("All packages already uninstalled")
		return result, nil
	}
	...
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to uninstall packages %v on %s: %v", toRemove, ctx.GetHost().GetName(), err))
		return result, err
	}
	logger.Info("Packages uninstalled successfully.")
	result.MarkCompleted(fmt.Sprintf("Successfully uninstalled packages: %v", toRemove))
	return result, nil
}
