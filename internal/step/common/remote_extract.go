...
func (s *ExtractArchiverStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if !ok {
		err := fmt.Errorf("'download_path_key' is required but not provided in RuntimeConfig")
		result.MarkFailed(err, err.Error())
		return result, err
	}
	inputKey, isString := inputKeyVal.(string)
	if !isString || inputKey == "" {
		err := fmt.Errorf("invalid 'download_path_key' in RuntimeConfig: expected a non-empty string, got %T", inputKeyVal)
		result.MarkFailed(err, err.Error())
		return result, err
	}
	...
	if !archiveOk {
		err := fmt.Errorf("archive path not found in Task Cache using key '%s' for step %s on host %s", inputKey, s.GetBase().Meta.Name, ctx.GetHost().GetName())
		result.MarkFailed(err, err.Error())
		return result, err
	}
	archivePath, okStr := archivePathVal.(string)
	if !okStr || archivePath == "" {
		err := fmt.Errorf("invalid or empty archive path in Task Cache using key '%s' for step %s on host %s", inputKey, s.GetBase().Meta.Name, ctx.GetHost().GetName())
		result.MarkFailed(err, err.Error())
		return result, err
	}

	if s.ExtractionDir == "" {
		err := fmt.Errorf("ExtractionDir not set for step %s on host %s", s.GetBase().Meta.Name, ctx.GetHost().GetName())
		result.MarkFailed(err, err.Error())
		return result, err
	}
	runnerSvc := ctx.GetRunner()
	conn, err := ctx.GetCurrentHostConnector()
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to get connector for host %s for step %s: %v", ctx.GetHost().GetName(), s.GetBase().Meta.Name, err))
		return result, err
	}
	facts, err := ctx.GetHostFacts(ctx.GetHost())
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to get host facts for %s for step %s: %v", ctx.GetHost().GetName(), s.GetBase().Meta.Name, err))
		return result, err
	}
	...
	if errMkdir := runnerSvc.Mkdirp(ctx.GoContext(), conn, s.ExtractionDir, "0755", s.Sudo); errMkdir != nil {
		err := fmt.Errorf("failed to create extraction directory %s for step %s on host %s: %w", s.ExtractionDir, s.GetBase().Meta.Name, ctx.GetHost().GetName(), errMkdir)
		result.MarkFailed(err, err.Error())
		return result, err
	}
	...
	if errExtract := runnerSvc.Extract(ctx.GoContext(), conn, facts, archivePath, s.ExtractionDir, s.Sudo, s.PreserveOriginalArchive); errExtract != nil {
		err := fmt.Errorf("failed to extract archive %s to %s for step %s on host %s: %w", archivePath, s.ExtractionDir, s.GetBase().Meta.Name, ctx.GetHost().GetName(), errExtract)
		result.MarkFailed(err, err.Error())
		return result, err
	}
	...
	result.MarkCompleted(fmt.Sprintf("Extracted archive to %s", s.ExtractionDir))
	return result, nil
}
