...
func (s *ManageHostsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "host", ctx.GetHost().GetName(), "phase", "Run")

	hostsContent, err := s.readRemoteFile(ctx, "/etc/hosts")
	if err != nil && !os.IsNotExist(err) {
		err := fmt.Errorf("run: failed to read /etc/hosts: %w", err)
		result.MarkFailed(err, "failed to read /etc/hosts")
		return result, err
	}
	newContent, err := s.rebuildHostsContent([]byte(hostsContent))
	if err != nil {
		err := fmt.Errorf("failed to rebuild hosts content: %w", err)
		result.MarkFailed(err, "failed to rebuild hosts content")
		return result, err
	}

	logger.Info("Atomically writing updated content to /etc/hosts")
	if err := s.atomicWriteRemoteFile(ctx, "/etc/hosts", newContent); err != nil {
		result.MarkFailed(err, "failed to write /etc/hosts")
		return result, err
	}
	result.MarkCompleted("Hosts file updated successfully")
	return result, nil
}
