...
func (s *InstallPackagesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if len(s.packagesToInstall) == 0 {
		...
		result.MarkSkipped("All packages already installed")
		return result, nil
	}
	runnerSvc := ctx.GetRunner()
	conn, err := ctx.GetCurrentHostConnector()
	...
	facts, err := ctx.GetHostFacts(ctx.GetHost())
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to get host facts for %s", ctx.GetHost().GetName()))
		return result, err
	}
	...
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to install packages %v on %s", s.packagesToInstall, ctx.GetHost().GetName()))
		return result, err
	}
	result.MarkCompleted(fmt.Sprintf("Successfully installed packages: %v", s.packagesToInstall))
	return result, nil
}
