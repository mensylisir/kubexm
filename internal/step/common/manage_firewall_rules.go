...
func (s *ManageFirewallRuleStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "host", ctx.GetHost().GetName(), "phase", "Run")
	runnerSvc := ctx.GetRunner()
	conn, err := ctx.GetCurrentHostConnector()
	if err != nil {
		err := fmt.Errorf("run: failed to get connector: %w", err)
	...
	if err != nil {
		if s.State == FirewallRuleAbsent {
			...
			result.MarkCompleted("No firewall tool found, rule considered absent")
			return result, nil
		}
		result.MarkFailed(err, "failed to detect firewall tool")
		return result, err
	}

	_, runCmd, _, err := s.getFirewallCmds(tool)
	if err != nil {
		result.MarkFailed(err, "failed to get firewall commands")
		return result, err
	}
	...
	if _, err := runnerSvc.Run(ctx.GoContext(), conn, runCmd, s.Sudo); err != nil {
		err := fmt.Errorf("failed to manage firewall rule with %s: %w", tool, err)
		result.MarkFailed(err, "failed to manage firewall rule")
		return result, err
	}
	logger.Infof("Firewall rule successfully set to state '%s'.", s.State)
	result.MarkCompleted("Firewall rule set successfully")
	return result, nil
}
