...
func (s *RemoteFileChecksumStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("remote checksum verification failed for %s: %v", s.FilePath, err))
		return result, err
	}
