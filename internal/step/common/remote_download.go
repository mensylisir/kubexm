...
func (s *RemoteDownloadFileStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to get host facts: %v", err))
		return result, err
	}
	...
	if err := runnerSvc.Mkdirp(ctx.GoContext(), conn, remoteDir, "0755", s.Sudo); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to create remote directory %s: %v", remoteDir, err))
		return result, err
	}
	...
	if err := runnerSvc.Download(ctx.GoContext(), conn, facts, s.URL, s.DestPath, s.Sudo); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to download on remote host: %v", err))
		return result, err
	}
	...
	if s.Checksum != "" {
		...
		if err := runnerSvc.VerifyChecksum(ctx.GoContext(), conn, s.DestPath, s.Checksum, s.ChecksumType, s.Sudo); err != nil {
			...
			result.MarkFailed(err, fmt.Sprintf("checksum verification failed for downloaded file: %v", err))
			return result, err
		}
	...
	if outputKeyVal, ok := ctx.GetFromRuntimeConfig("outputCacheKey"); ok {
		if outputKey, isString := outputKeyVal.(string); isString && outputKey != "" {
			...
			err := fmt.Errorf("invalid 'outputCacheKey' in RuntimeConfig: not a non-empty string")
