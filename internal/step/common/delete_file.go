...
func (s *DeleteFileStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := runnerSvc.Remove(ctx.GoContext(), conn, s.RemotePath, s.Sudo, s.Recursive); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to remove %s: %v", s.RemotePath, err))
		return result, err
	}

	logger.Info("Remote path removed successfully.", "path", s.RemotePath)
	result.MarkCompleted(fmt.Sprintf("Successfully removed %s", s.RemotePath))
	return result, nil
}
