...
func (s *ManageSELinuxStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "host", ctx.GetHost().GetName(), "phase", "Run")
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())

	supported, err := s.isSELinuxSupported(ctx)
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to check SELinux support: %v", err))
		return result, err
	}
	if !supported {
		logger.Infof("SELinux is not supported or active on this OS. Skipping.")
		result.MarkSkipped("SELinux not supported on this OS")
		return result, nil
	}
	...
	conn, err := ctx.GetCurrentHostConnector()
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to get connector: %v", err))
		return result, err
	}
	...
	default:
		result.MarkFailed(fmt.Errorf("unsupported SELinux state: %s", s.State), fmt.Sprintf("unsupported SELinux state: %s", s.State))
		return result, fmt.Errorf("unsupported SELinux state: %s", s.State)
	}
	if setEnforceCmd != "" {
	...
	configContent, err := s.ReadRemoteFile(ctx, "/etc/selinux/config")
	if err != nil && !os.IsNotExist(err) {
		result.MarkFailed(err, fmt.Sprintf("run: failed to read /etc/selinux/config: %v", err))
		return result, err
	}

	newConfigContent, err := s.rebuildSELinuxConfig(configContent, s.State)
	if err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to rebuild selinux config: %v", err))
		return result, err
	}

	if err := s.AtomicWriteRemoteFile(ctx, "/etc/selinux/config", newConfigContent); err != nil {
		result.MarkFailed(err, fmt.Sprintf("failed to write selinux config: %v", err))
		return result, err
	}
	...
	result.MarkCompleted(fmt.Sprintf("SELinux state set to %s", s.State))
	return result, nil
}
