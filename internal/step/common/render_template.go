...
func (s *RenderTemplateStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		err := fmt.Errorf("failed to render template for %s: %w", s.RemoteDestPath, err)
		result.MarkFailed(err, "failed to render template")
		return result, err
	}
	...
	if err != nil {
		...
		err := fmt.Errorf("failed to write rendered template to %s: %w", s.RemoteDestPath, err)
		result.MarkFailed(err, "failed to write rendered template")
		return result, err
	}

	logger.Info("Template rendered and written successfully.")
	result.MarkCompleted(fmt.Sprintf("Template rendered to %s", s.RemoteDestPath))
	return result, nil
}
