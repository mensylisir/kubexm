...
func (s *ExtractArchiveStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err := os.MkdirAll(s.DestinationDir, 0755); err != nil {
		err := fmt.Errorf("failed to create destination directory %s: %w", s.DestinationDir, err)
	...
	if err != nil {
		err := fmt.Errorf("failed to open archive %s: %w", s.SourceArchivePath, err)
		result.MarkFailed(err, "failed to open archive")
		return result, err
	}
	defer file.Close()
	...
	if strings.HasSuffix(archiveLower, ".tar.gz") || strings.HasSuffix(archiveLower, ".tgz") {
		...
		if errGzip != nil {
			err := fmt.Errorf("failed to create gzip reader for %s: %w", s.SourceArchivePath, errGzip)
			result.MarkFailed(err, "failed to create gzip reader")
			return result, err
		}
		defer gzr.Close()
		reader = gzr
	...
	if strings.Contains(archiveLower, ".tar") {
		if err := s.extractTar(ctx, tar.NewReader(reader)); err != nil {
			result.MarkFailed(err, "failed to extract tar archive")
			return result, err
		}
		result.MarkCompleted("Tar archive extracted successfully")
		return result, nil
	}
	if strings.HasSuffix(archiveLower, ".zip") {
		fi, errStat := file.Stat()
		if errStat != nil {
			err := fmt.Errorf("failed to stat archive file %s for zip processing: %w", s.SourceArchivePath, errStat)
			result.MarkFailed(err, "failed to stat archive file")
			return result, err
		}
		if err := s.extractZip(ctx, file, fi.Size()); err != nil {
			result.MarkFailed(err, "failed to extract zip archive")
			return result, err
		}
		result.MarkCompleted("Zip archive extracted successfully")
		return result, nil
	}
	err = fmt.Errorf("unsupported archive format for %s", s.SourceArchivePath)
	result.MarkFailed(err, "unsupported archive format")
	return result, err
}
