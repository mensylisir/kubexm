...
func (s *PullImagesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	for _, img := range s.Images {
		...
		if _, err := runnerSvc.Run(ctx.GoContext(), conn, pullCmd, s.Sudo); err != nil {
			result.MarkFailed(err, fmt.Sprintf("failed to pull image %s: %v", img, err))
			return result, err
		}
	}
	logger.Info("All required images for this host pulled successfully.")
	result.MarkCompleted(fmt.Sprintf("Successfully pulled %d images", len(s.Images)))
	return result, nil
}
