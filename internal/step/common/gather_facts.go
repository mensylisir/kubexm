...
func (s *GatherFactsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		...
		result.MarkFailed(err, fmt.Sprintf("failed to gather facts from host %s", ctx.GetHost().GetName()))
		return result, err
	}
	...
	result.SetMetadata("os", facts.OS.PrettyName)
	result.SetMetadata("arch", facts.CPU.Architecture)
	result.SetMetadata("cores", cores)
	return result, nil
}
