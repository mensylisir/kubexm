...
func (s *ReportTableStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "host", ctx.GetHost().GetName(), "phase", "Run")
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	if len(s.Rows) == 0 {
		logger.Info("No data to display in table report.")
		result.MarkCompleted("No data to display")
		return result, nil
	}
	...
	result.MarkCompleted(fmt.Sprintf("Displayed table with %d rows", len(s.Rows)))
	return result, nil
}
