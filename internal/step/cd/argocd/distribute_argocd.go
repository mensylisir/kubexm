...
func NewDistributeArgoCDArtifactsStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DistributeArgoCDArtifactsStepBuilder {
...
func (s *DistributeArgoCDArtifactsStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		result.MarkFailed(fmt.Errorf("failed to read local values file %s: %w", localValuesPath, err), "Failed to read local values file")
		return result, err
	}
	chartContent, err := os.ReadFile(localChartPath)
	if err != nil {
		result.MarkFailed(fmt.Errorf("failed to read local chart file %s: %w", localChartPath, err), "Failed to read local chart file")
		return result, err
	}
	...
	if err := runner.Mkdirp(ctx.GoContext(), conn, remoteDir, "0755", s.Sudo); err != nil {
		result.MarkFailed(fmt.Errorf("failed to create remote directory %s on host %s: %w", remoteDir, ctx.GetHost().GetName(), err), "Failed to create remote directory")
		return result, err
	}
	...
	if err := helpers.WriteContentToRemote(ctx, conn, string(valuesContent), s.RemoteValuesPath, "0644", s.Sudo); err != nil {
		result.MarkFailed(fmt.Errorf("failed to upload values.yaml: %w", err), "Failed to upload values.yaml")
		return result, err
	}
	...
	if err := helpers.WriteContentToRemote(ctx, conn, string(chartContent), s.RemoteChartPath, "0644", s.Sudo); err != nil {
		result.MarkFailed(fmt.Errorf("failed to upload helm chart: %w", err), "Failed to upload helm chart")
		return result, err
	}

	logger.Info("Successfully distributed Argo CD artifacts to remote host.")
	result.MarkCompleted("Successfully distributed Argo CD artifacts to remote host")
	return result, nil
}
