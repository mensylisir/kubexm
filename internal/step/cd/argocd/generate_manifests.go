...
func NewGenerateArgoCDValuesStepBuilder(ctx runtime.ExecutionContext, instanceName string) *GenerateArgoCDValuesStepBuilder {
...
func (s *GenerateArgoCDValuesStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	result := types.NewStepResult(s.Base.Meta.Name, ctx.GetStepExecutionID(), ctx.GetHost())
	logger := ctx.GetLogger().With("step", s.Base.Meta.Name, "host", ctx.GetHost().GetName(), "phase", "Run")

	valuesTemplateContent, err := templates.Get("cd/argocd/values.yaml.tmpl")
	if err != nil {
		result.MarkFailed(fmt.Errorf("failed to get embedded argocd values.yaml.tmpl: %w", err), "Failed to get embedded argocd values.yaml.tmpl")
		return result, err
	}

	tmpl, err := template.New("argoCDValues").Parse(valuesTemplateContent)
	if err != nil {
		result.MarkFailed(fmt.Errorf("failed to parse argocd values.yaml.tmpl: %w", err), "Failed to parse argocd values.yaml.tmpl")
		return result, err
	}
	var valuesBuffer bytes.Buffer
	if err := tmpl.Execute(&valuesBuffer, s); err != nil {
		result.MarkFailed(fmt.Errorf("failed to render argocd values.yaml.tmpl: %w", err), "Failed to render argocd values.yaml.tmpl")
		return result, err
	}
	...
	if err := os.MkdirAll(filepath.Dir(localPath), 0755); err != nil {
		result.MarkFailed(fmt.Errorf("failed to create local directory for values file: %w", err), "Failed to create local directory for values file")
		return result, err
	}

	if err := os.WriteFile(localPath, valuesBuffer.Bytes(), 0644); err != nil {
		result.MarkFailed(fmt.Errorf("failed to write generated values file to %s: %w", localPath, err), "Failed to write generated values file")
		return result, err
	}

	logger.Info("Successfully generated Argo CD Helm values file.")
	result.MarkCompleted("Successfully generated Argo CD Helm values file")
	return result, nil
}
