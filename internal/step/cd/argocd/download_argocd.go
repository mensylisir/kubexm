...
func NewDownloadArgoCDChartStepBuilder(ctx runtime.ExecutionContext, instanceName string) *DownloadArgoCDChartStepBuilder {
...
func (s *DownloadArgoCDChartStep) Run(ctx runtime.ExecutionContext) (*types.StepResult, error) {
	...
	if err != nil {
		...
		result.MarkCompleted("Chart info unavailable, skipping download")
		return result, nil
	}
	...
	if err := os.MkdirAll(destDir, 0755); err != nil {
		result.MarkFailed(fmt.Errorf("failed to create local destination directory %s: %w", destDir, err), "Failed to create local destination directory")
		return result, err
	}
	...
	if output, err := repoAddCmd.CombinedOutput(); err != nil {
		if !strings.Contains(string(output), "already exists") {
			result.MarkFailed(fmt.Errorf("failed to add helm repo '%s': %w\nOutput: %s", chart.RepoName(), err, string(output)), "Failed to add helm repo")
			return result, err
		}
	}
	...
	if output, err := repoUpdateCmd.CombinedOutput(); err != nil {
		result.MarkFailed(fmt.Errorf("failed to update helm repo %s: %w\nOutput: %s", chart.RepoName(), err, string(output)), "Failed to update helm repo")
		return result, err
	}
	...
	if output, err := pullCmd.CombinedOutput(); err != nil {
		result.MarkFailed(fmt.Errorf("failed to pull Argo CD chart: %w\nOutput: %s", err, string(output)), "Failed to pull Argo CD chart")
		return result, err
	}

	logger.Info("Argo CD Helm chart has been downloaded successfully.")
	result.MarkCompleted("Argo CD Helm chart has been downloaded successfully")
	return result, nil
}
