...
import (
	...
	taskEtcd "github.com/mensylisir/kubexm/internal/task/etcd" // Renamed to avoid conflict
)
...
func NewEtcdModule() module.Module {
	...
	// The DeployEtcdClusterTask is the primary one for setting up ETCD.
	...
	// For now, let's assume DeployEtcdClusterTask handles what's needed or depends on a PKI task.
...
func (m *EtcdModule) Plan(ctx runtime.ModuleContext) (*plan.ExecutionFragment, error) {
	...
	moduleFragment := plan.NewExecutionFragment(m.Name() + "-Fragment") // Named fragment
	...
	generatePkiTask := taskEtcd.NewGenerateEtcdPKITask()
	...
	// Now plan the DeployEtcdClusterTask, which depends on the PKI task.
	installEtcdTask := taskEtcd.NewDeployEtcdClusterTask() // This is one of the tasks from m.Tasks()
