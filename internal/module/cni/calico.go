...
func (m *CalicoModule) Plan(ctx runtime.ModuleContext) (*plan.ExecutionFragment, error) {
	...
	if clusterConfig.Spec.Network == nil || clusterConfig.Spec.Network.Plugin != string(common.CNITypeCalico) {
		...
		logger.Info("CNI plugin is not Calico, or CNI not specified. Skipping Calico module planning.", "configured_plugin", pluginName)
	...
	for _, currentTask := range m.Tasks() {
		...
		if !isFirstEffectiveTask && len(previousTaskExitNodes) > 0 {
			if err := plan.LinkFragments(moduleFragment, previousTaskExitNodes, taskFrag.EntryNodes); err != nil {
				return nil, fmt.Errorf("failed to link CNI task fragment: %w", err)
			}
