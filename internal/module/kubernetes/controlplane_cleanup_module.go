...
import (
	...
	// taskK8s "github.com/mensylisir/kubexm/internal/task/kubernetes" // For actual tasks later
)

// ControlPlaneCleanupModule defines the module for cleaning up Kubernetes control plane components.
type ControlPlaneCleanupModule struct {
	module.BaseModule
}

// NewControlPlaneCleanupModule creates a new ControlPlaneCleanupModule.
func NewControlPlaneCleanupModule() module.Module {
	// TODO: Define actual tasks:
	// - NewStopAPIServerTask()
	// - NewRemoveAPIServerBinariesTask()
	// - NewStopSchedulerTask()
	// - NewRemoveSchedulerBinariesTask()
	// - NewStopControllerManagerTask()
	// - NewRemoveControllerManagerBinariesTask()
	// - NewRemoveControlPlanePKITask()
	// - NewRemoveKubeconfigsTask()
	moduleTasks := []task.Task{
		// Example: taskK8s.NewStopAPIServerTask(),
	}

	base := module.NewBaseModule("KubernetesControlPlaneCleanup", moduleTasks)
	m := &ControlPlaneCleanupModule{BaseModule: base}
	return m
}
...
func (m *ControlPlaneCleanupModule) Plan(ctx runtime.ModuleContext) (*plan.ExecutionFragment, error) {
	...
	if len(moduleFragment.Nodes) == 0 {
		...
		logger.Info("Kubernetes Control Plane Cleanup module planning complete.", "total_nodes", len(moduleFragment.Nodes))
	}

	return moduleFragment, nil
}

var _ module.Module = (*ControlPlaneCleanupModule)(nil)
