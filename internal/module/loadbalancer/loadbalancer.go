...
func (t *DeployLoadBalancerTask) Plan(ctx runtime.TaskContext) (*plan.ExecutionFragment, error) {
	...
	for _, lbTask := range tasks {
		...
		if len(previousTaskExitNodes) > 0 {
			if err := plan.LinkFragments(fragment, previousTaskExitNodes, taskFrag.EntryNodes); err != nil {
				return nil, fmt.Errorf("failed to link load balancer task fragment: %w", err)
			}
		}
