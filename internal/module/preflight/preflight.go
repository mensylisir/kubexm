...
func (m *PreflightModule) GetTasks() []task.Task {
	return []task.Task{
		taskpreflight.NewGreetingTask(),
		taskpre.NewConfirmTask("InitialConfirmation", "Proceed with KubeXM operations?", m.assumeYes),
		taskpreflight.NewExtractBundleTask(""),
		taskos.NewInstallPrerequisitesTask(),
		taskpreflight.NewPrepareAssetsTask(),
		taskpreflight.NewInstallToolBinariesTask(),
	...
	for i, currentTask := range definedTasks {
		...
		if i > 0 && len(previousTaskExitNodes) > 0 {
			if err := plan.LinkFragments(moduleFragment, previousTaskExitNodes, taskFrag.EntryNodes); err != nil {
				return nil, fmt.Errorf("failed to link fragments for task %s in module %s: %w", currentTask.Name(), m.Name(), err)
			}
		}
