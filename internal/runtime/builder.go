...
type Builder struct {
	...
	skipHostConnect        bool
	controlConnector       connector.Connector
...
func (b *Builder) WithSkipHostConnect(skip bool) *Builder {
	b.skipHostConnect = skip
	return b
}

func (b *Builder) WithControlConnector(conn connector.Connector) *Builder {
	b.controlConnector = conn
	return b
}

func (b *Builder) WithSkipConfigValidation(skip bool) *Builder {
	b.skipConfigValidation = skip
	return b
}
...
func (b *Builder) Build(ctx context.Context) (*Context, func(), error) {
	...
	if b.skipHostConnect {
		if err := b.initializeHostsWithoutConnect(runtimeCtx, connectorFactory); err != nil {
			cleanupFunc()
			return nil, nil, err
		}
	} else {
	...
	if runtimeCtx.controlNode == nil {
		controlNodes := runtimeCtx.GetHostsByRole(common.ControlNodeRole)
		if len(controlNodes) > 0 {
			runtimeCtx.controlNode = controlNodes[0]
		}
	}
...
func (b *Builder) getOrParseConfig() (*v1alpha1.Cluster, error) {
	...
	if b.configFilepath != "" {
		return config.ParseFromFile(b.configFilepath)
	}
...
func (b *Builder) initializeHostsWithoutConnect(rc *Context, factory connector.Factory) error {
	log := rc.Logger
	controlNodeSpec, err := b.buildControlNodeSpec()
	if err != nil {
		return err
	}
	controlHost := connector.NewHostFromSpec(controlNodeSpec)

	var controlConn connector.Connector
	if b.controlConnector != nil {
		controlConn = b.controlConnector
	} else {
		sshConn, err := factory.NewConnectorForHost(controlHost, rc.ConnectionPool)
		if err != nil {
			return fmt.Errorf("failed to create control node connector: %w", err)
		}
		controlConn = sshConn
	}
	connCfg, err := factory.NewConnectionCfg(controlHost, b.clusterConfig.Spec.Global.ConnectionTimeout)
	if err != nil {
		return err
	}
	if err := controlConn.Connect(rc.GoCtx, connCfg); err != nil {
	...
	rc.controlNode = controlHost

	for _, hostCfg := range rc.ClusterConfig.Spec.Hosts {
		host := connector.NewHostFromSpec(hostCfg)
		rc.hostInfoMap[host.GetName()] = &HostRuntimeInfo{
			Host:  host,
			Conn:  nil,
			Facts: nil,
		}
...
func (b *Builder) buildControlNodeSpec() (v1alpha1.HostSpec, error) {
	ip, err := netutil.DetectPrimaryIP()
	if err != nil {
		return v1alpha1.HostSpec{}, fmt.Errorf("failed to detect control node IP: %w", err)
	}
	spec := v1alpha1.HostSpec{
		...
		RoleTable: map[string]bool{
			common.ControlNodeRole: true,
		},
	}
	if b.clusterConfig != nil {
		v1alpha1.SetDefaults_HostSpec(&spec, b.clusterConfig)
	}
	return spec, nil
}
