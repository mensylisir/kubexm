...
// FileOpsContext is an interface for context objects that provide file operations support.
// This allows util package to avoid importing runtime, breaking import cycles.
type FileOpsContext interface {
	GetUploadDir() string
...
func UploadFile(ctx FileOpsContext, conn connector.Connector, localFile string, destFile string, permission string, sudo bool) error {
...
func WriteContentToRemote(ctx FileOpsContext, conn connector.Connector, content string, destFile string, permission string, sudo bool) error {
