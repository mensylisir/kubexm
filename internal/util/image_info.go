...
func GetImages(ctx runtime.ExecutionContext) []Image {
	i := Images{}
	i.Images = []Image{
		GetImage(ctx, "etcd"),
		GetImage(ctx, "pause"),
...
func GetImage(ctx runtime.ExecutionContext, name string) Image {
	kubeVersionStr := ctx.GetClusterConfig().Spec.Kubernetes.Version
	...
	if isV1_21_0 &&
		ctx.GetClusterConfig().Spec.Kubernetes.ContainerRuntime.Type != "" &&
		ctx.GetClusterConfig().Spec.Kubernetes.ContainerRuntime.Type != "docker" {
		pauseTag = "3.4.1"
	}
	...
	internalLBType := cfg.ControlPlaneEndpoint.InternalLoadBalancerType
	externalLBType := cfg.ControlPlaneEndpoint.ExternalLoadBalancerType
	if cfg.ControlPlaneEndpoint.HighAvailability != nil {
		ha := cfg.ControlPlaneEndpoint.HighAvailability
		if ha.External != nil && ha.External.Enabled != nil && *ha.External.Enabled && ha.External.Type != "" {
			externalLBType = common.ExternalLoadBalancerType(ha.External.Type)
		}
		if ha.Internal != nil && ha.Internal.Enabled != nil && *ha.Internal.Enabled && ha.Internal.Type != "" {
			internalLBType = common.InternalLoadBalancerType(ha.Internal.Type)
		}
	}
	...
	ImageList := map[string]Image{
		...
		"haproxy":                   {RepoAddr: privateRegistry, Namespace: "library", Repo: "haproxy", Tag: "2.9.6-alpine", Group: common.RoleWorker, Enable: internalLBType == common.InternalLBTypeHAProxy},
		"nginx":                     {RepoAddr: privateRegistry, Namespace: "library", Repo: "nginx", Tag: "2.9.6-alpine", Group: common.RoleWorker, Enable: internalLBType == common.InternalLBTypeNginx},
		"kubevip":                   {RepoAddr: privateRegistry, Namespace: "plndr", Repo: "kube-vip", Tag: "v0.7.2", Group: common.RoleMaster, Enable: internalLBType == common.InternalLBTypeKubeVIP || externalLBType == common.ExternalLBTypeKubeVIP},
	...
	if ctx.GetClusterConfig().Spec.Registry.MirroringAndRewriting.NamespaceOverride != "" {
		image.NamespaceOverride = ctx.GetClusterConfig().Spec.Registry.MirroringAndRewriting.NamespaceOverride
	} else if ctx.GetClusterConfig().Spec.Registry.MirroringAndRewriting.NamespaceRewrite != nil {
		image.NamespaceRewrite = ctx.GetClusterConfig().Spec.Registry.MirroringAndRewriting.NamespaceRewrite
	}
	return image
}
