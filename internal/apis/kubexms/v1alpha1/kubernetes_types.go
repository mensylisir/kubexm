...
func SetDefaults_Kubernetes(cfg *Kubernetes) {
	...
	if cfg.Type == "" {
		cfg.Type = string(common.KubernetesDeploymentTypeKubeadm)
	}
...
func Validate_Kubernetes(cfg *Kubernetes, verrs *validation.ValidationErrors, pathPrefix string) {
	...
	// 顶层字段校验
	if cfg.Type == "" {
		verrs.Add(p + ".type: is a required field")
	} else if !helpers.IsValidKubernetesDeploymentType(cfg.Type) {
		verrs.Add(fmt.Sprintf("%s.type: invalid type '%s', must be one of %v", p, cfg.Type, common.SupportedKubernetesDeploymentTypes))
	}
	if cfg.Version == "" {
