...
func SetDefault_Gateway(spec *GatewaySpec) {
	if spec.IngressNginx == nil {
		spec.IngressNginx = &IngressNginxSpec{Enabled: helpers.BoolPtr(false)}
	}
}
...
func Validate_HostSpec(spec *HostSpec, verrs *validation.ValidationErrors, pathPrefix string) {
	...
	if spec.Address == "" {
		...
		if strings.EqualFold(addr, "localhost") || addr == "127.0.0.1" {
			verrs.Add(fmt.Sprintf("%s.address: localhost/127.0.0.1 is not allowed", pathPrefix))
			...
			verrs.Add(fmt.Sprintf("%s.address: loopback address '%s' is not allowed", pathPrefix, spec.Address))
		}
	}

	if strings.TrimSpace(spec.InternalAddress) != "" {
		internalAddrs := strings.Split(spec.InternalAddress, ",")
		for i, raw := range internalAddrs {
			addr := strings.TrimSpace(raw)
			if addr == "" {
				verrs.Add(fmt.Sprintf("%s.internalAddress: address at index %d is empty", pathPrefix, i))
				continue
			}
			if strings.EqualFold(addr, "localhost") || addr == "127.0.0.1" {
				verrs.Add(fmt.Sprintf("%s.internalAddress: localhost/127.0.0.1 is not allowed", pathPrefix))
				continue
			}
			ip := net.ParseIP(addr)
			if ip == nil {
				verrs.Add(fmt.Sprintf("%s.internalAddress: invalid IP address format for '%s'", pathPrefix, addr))
				continue
			}
			if ip.IsLoopback() {
				verrs.Add(fmt.Sprintf("%s.internalAddress: loopback address '%s' is not allowed", pathPrefix, addr))
			}
		}
	}
