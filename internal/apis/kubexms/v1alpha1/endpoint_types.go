...
import (
	"fmt"
	"github.com/mensylisir/kubexm/internal/apis/kubexms/v1alpha1/helpers"
...
func SetDefaults_ControlPlaneEndpointSpec(cfg *ControlPlaneEndpointSpec) {
	...
	if cfg.Address != "" && helpers.IsValidIP(cfg.Address) && cfg.ExternalLoadBalancerType != "" {
		...
		if cfg.HighAvailability.External == nil {
			cfg.HighAvailability.External = &ExternalLoadBalancerConfig{}
		}
		...
		if cfg.HighAvailability.External.Keepalived != nil &&
			len(cfg.HighAvailability.External.Keepalived.VRRPInstances) > 0 &&
			len(cfg.HighAvailability.External.Keepalived.VRRPInstances[0].VirtualIPs) > 0 {
	...
	if cfg.InternalLoadBalancerType != "" {
		...
		if cfg.HighAvailability.Internal == nil {
			cfg.HighAvailability.Internal = &InternalLoadBalancerConfig{}
		}
		cfg.HighAvailability.Enabled = helpers.BoolPtr(true)
		...
		if cfg.ExternalLoadBalancerType == "" && cfg.Address == "" && cfg.InternalLoadBalancerType != common.InternalLBTypeKubeVIP {
...
func Validate_ControlPlaneEndpointSpec(cfg *ControlPlaneEndpointSpec, verrs *validation.ValidationErrors, pathPrefix string) {
	...
	if cfg.ExternalLoadBalancerType == "" && cfg.InternalLoadBalancerType == "" && strings.TrimSpace(cfg.Address) != "" {
		verrs.Add(pathPrefix + ": address must be empty when no loadbalancer is enabled")
	}
	if cfg.InternalLoadBalancerType == common.InternalLBTypeKubeVIP && strings.TrimSpace(cfg.Address) == "" {
		verrs.Add(pathPrefix + ": address must be specified when kube-vip internal loadbalancer is enabled")
	}
	if cfg.ExternalLoadBalancerType != "" && cfg.InternalLoadBalancerType != "" {
