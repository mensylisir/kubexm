...
func (t *UpdateWorkerNodesTask) Plan(ctx runtime.TaskContext) (*plan.ExecutionFragment, error) {
	...
	for _, host := range pureWorkerHosts {
		...
		distributeCertStep, err := kubexmstep.NewKubexmDistributeK8sPKIStepBuilder(runtimeCtx, fmt.Sprintf("DistributeWorkerCertsFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		distributeKubeconfigStep, err := kubexmstep.NewDistributeWorkerKubeconfigsStepBuilder(runtimeCtx, fmt.Sprintf("DistributeWorkerKubeconfigsFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		restartKubeletStep, err := kubelet.NewRestartKubeletStepBuilder(runtimeCtx, fmt.Sprintf("RestartKubeletFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		verifyKubeletStep, err := health.NewVerifyKubeletHealthStepBuilder(runtimeCtx, fmt.Sprintf("VerifyWorkerFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		restartKubeProxyStep, err := kube_proxy.NewRestartKubeProxyStepBuilder(runtimeCtx, fmt.Sprintf("RestartKubeProxyFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		verifyKubeProxyStep, err := health.NewVerifyKubeProxyHealthStepBuilder(runtimeCtx, fmt.Sprintf("VerifyKubeProxyFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		...
		barrierStep, err := stepcommon.NewNoOpStepBuilder(
	...
	if len(masterHosts) > 0 {
		...
		verifyClusterStep, err := health.NewCheckClusterHealthStepBuilder(runtimeCtx, "VerifyClusterHealthAfterWorkerRollout").Build()
		if err != nil {
			return nil, err
		}
