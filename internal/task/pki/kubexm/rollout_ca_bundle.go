...
func (t *RolloutK8sCABundleTask) Plan(ctx runtime.TaskContext) (*plan.ExecutionFragment, error) {
	...
	for _, host := range masterHosts {
		...
		distributeBundleStep, err := kubexmstep.NewKubexmDistributeK8sPKIStepBuilder(runtimeCtx, fmt.Sprintf("DistributeBundleFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}

		restartApiServerStep, err := kube_apiserver.NewRestartKubeApiServerStepBuilder(runtimeCtx, fmt.Sprintf("RestartApiServerFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		verifyApiServerStep, err := health.NewVerifyAPIServerHealthStepBuilder(runtimeCtx, fmt.Sprintf("VerifyApiServer-%s", hostName)).Build()
		if err != nil {
			return nil, err
		}

		restartCMStep, err := kube_controller_manager.NewRestartKubeControllerManagerStepBuilder(runtimeCtx, fmt.Sprintf("RestartControllerManagerFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		verifyCMStep, err := health.NewVerifyControllerManagerHealthStepBuilder(runtimeCtx, fmt.Sprintf("VerifyControllerManagerFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}

		restartSchedulerStep, err := kube_scheduler.NewRestartKubeSchedulerStepBuilder(runtimeCtx, fmt.Sprintf("RestartSchedulerFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		verifySchedulerStep, err := health.NewVerifySchedulerHealthStepBuilder(runtimeCtx, fmt.Sprintf("VerifySchedulerFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
	...
	verifyClusterStep, err := health.NewCheckClusterHealthStepBuilder(runtimeCtx, "VerifyClusterHealthAfterBundleRollout").Build()
	if err != nil {
		return nil, err
	}
