...
func (t *GenerateClusterCertsTask) Plan(ctx runtime.TaskContext) (*plan.ExecutionFragment, error) {
	...
	prepareAssetsStep, err := kubexmstep.NewKubexmPrepareCAAssetsStepBuilder(runtimeCtx, "PrepareCAAssets").Build()
	if err != nil {
		return nil, err
	}
	renewCaStep, err := kubexmstep.NewKubexmRenewK8sCAStepBuilder(runtimeCtx, "RenewK8sCA").Build()
	if err != nil {
		return nil, err
	}
	renewLeafsStep, err := kubexmstep.NewBinaryRenewAllLeafCertsStepBuilder(runtimeCtx, "RenewAllLeafCerts").Build()
	if err != nil {
		return nil, err
	}
	createBundleStep, err := kubexmstep.NewKubexmPrepareCATransitionStepBuilder(runtimeCtx, "CreateCABundle").Build()
	if err != nil {
		return nil, err
	}
