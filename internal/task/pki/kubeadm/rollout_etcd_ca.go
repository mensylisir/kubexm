...
func (t *RolloutEtcdCertsCATask) Plan(ctx runtime.TaskContext) (*plan.ExecutionFragment, error) {
	...
	for _, host := range etcdHosts {
		...
		distributeStep, err := kubeadm.NewKubeadmDistributeStackedEtcdPKIStepBuilder(runtimeCtx, fmt.Sprintf("DistributeCAFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		...
		verifyPodStep, err := kubeadm2.NewKubeadmVerifyEtcdPodHealthStepBuilder(runtimeCtx, fmt.Sprintf("VerifyEtcdPodFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
