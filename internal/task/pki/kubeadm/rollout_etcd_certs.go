...
import (
	...
	kubeadm2 "github.com/mensylisir/kubexm/internal/step/kubernetes/kubeadm"
...
func (t *RolloutEtcdLeafCertsTask) Plan(ctx runtime.TaskContext) (*plan.ExecutionFragment, error) {
	...
	if caRequiresRenewal {
		moveStep, err := kubeadm.NewKubeadmMoveNewAssetsStepBuilder(runtimeCtx, "MoveNewEtcdAssets").Build()
		if err != nil {
			return nil, err
		}
	...
	for _, host := range etcdHosts {
		...
		distributeStep, err := kubeadm.NewKubeadmDistributeStackedEtcdLeafCertsStepBuilder(runtimeCtx, fmt.Sprintf("DistributeLeafsFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		restartStep, err := kubeadm.NewKubeadmRestartEtcdStepBuilder(runtimeCtx, fmt.Sprintf("RestartEtcdFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		verifyPodStep, err := kubeadm2.NewKubeadmVerifyEtcdPodHealthStepBuilder(runtimeCtx, fmt.Sprintf("VerifyPodFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
	...
	verifyClusterStep, err := kubeadm2.NewKubeadmVerifyEtcdClusterHealthStepBuilder(runtimeCtx, "VerifyClusterHealth").Build()
	if err != nil {
		return nil, err
	}
