...
func (t *RolloutMasterCertsCATask) Plan(ctx runtime.TaskContext) (*plan.ExecutionFragment, error) {
	...
	for _, host := range masterHosts {
		...
		backupPkiStep, err := kubeadmstep.NewKubeadmBackupRemotePKIStepBuilder(runtimeCtx, fmt.Sprintf("BackupPKIFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		distributeBundleStep, err := kubeadmstep.NewKubeadmDistributeK8sPKIStepBuilder(runtimeCtx, fmt.Sprintf("DistributeBundleFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		restartControlPlaneStep, err := kubeadmstep.NewKubeadmRestartControlPlaneStepBuilder(runtimeCtx, fmt.Sprintf("RestartControlPlaneFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		verifyControlPlaneStep, err := kubeadm.NewKubeadmVerifyControlPlaneHealthStepBuilder(runtimeCtx, fmt.Sprintf("VerifyControlPlaneFor%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
	...
	verifyClusterStep, err := kubeadm.NewKubeadmVerifyClusterHealthStepBuilder(runtimeCtx, "VerifyClusterHealthAfterBundleRollout").Build()
	if err != nil {
		return nil, err
	}
