...
func (t *RolloutMasterLeafCertsTask) Plan(ctx runtime.TaskContext) (*plan.ExecutionFragment, error) {
	...
	for _, host := range masterHosts {
		...
		distributeStep, err := kubeadmstep.NewKubeadmDistributeLeafCertsStepBuilder(runtimeCtx, fmt.Sprintf("DistributeLeafs-For-%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		restartStep, err := kubeadmstep.NewKubeadmRestartControlPlaneStepBuilder(runtimeCtx, fmt.Sprintf("RestartCP-ForLeafs-%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
		verifyStep, err := kubeadm.NewKubeadmVerifyControlPlaneHealthStepBuilder(runtimeCtx, fmt.Sprintf("VerifyCP-ForLeafs-%s", hostName)).Build()
		if err != nil {
			return nil, err
		}
	...
	verifyClusterStep, err := kubeadm.NewKubeadmVerifyClusterHealthStepBuilder(runtimeCtx, "VerifyClusterHealthAfterLeafsRollout").Build()
	if err != nil {
		return nil, err
	}
