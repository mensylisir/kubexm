...
func (t *DeployNewLeafCertsRollingTask) Plan(ctx runtime.TaskContext) (*plan.ExecutionFragment, error) {
	...
	if isCARenewal {
		moveAssetsStep, err := etcdstep.NewMoveNewAssetsStepBuilder(runtimeCtx, "MoveNewAssetsToMainDir").Build()
		if err != nil {
			return nil, err
		}
	...
	for _, node := range etcdNodes {
		...
		distLeafsStep, err := etcdstep.NewDistributeLeafCertsStepBuilder(runtimeCtx, fmt.Sprintf("DistributeLeafCerts_%s", nodeName)).Build()
		if err != nil {
			return nil, err
		}
