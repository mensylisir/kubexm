...
func (t *CleanNginxAsDaemonTask) Plan(ctx runtime.TaskContext) (*plan.ExecutionFragment, error) {
	...
	stopService, err := nginxstep.NewStopNginxStepBuilder(runtimeCtx, "StopNginxService").Build()
	if err != nil {
		return nil, err
	}
	disableService, err := nginxstep.NewDisableNginxStepBuilder(runtimeCtx, "DisableNginxService").Build()
	if err != nil {
		return nil, err
	}
	cleanFilesAndPackage, err := nginxstep.NewCleanNginxStepBuilder(runtimeCtx, "CleanNginxPackage").Build()
	if err != nil {
		return nil, err
	}
