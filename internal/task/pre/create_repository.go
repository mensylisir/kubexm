...
func (t *CreateRepositoryTask) Plan(ctx runtime.TaskContext) (*plan.ExecutionFragment, error) {
	...
	repoHostList := []remotefw.Host{repoHost}
	...
	packageManagerType := string(facts.PackageManager.Type)
	...
	default:
		return nil, fmt.Errorf("unsupported package manager '%s' on repository host '%s'", packageManagerType, repoHost.GetName())
	}
