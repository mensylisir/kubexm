...
import (
	...
	controllermanagerstep "github.com/mensylisir/kubexm/internal/step/kubernetes/kube-controller-manager"
...
func (t *GenerateControlPlaneKubeconfigsTask) Plan(ctx runtime.TaskContext) (*plan.ExecutionFragment, error) {
	...
	createCMKubeconfig, err := controllermanagerstep.NewCreateControllerManagerKubeconfigStepBuilder(runtimeCtx, "CreateControllerManagerKubeconfig").Build()
	if err != nil {
		return nil, err
	}
	createSchedulerKubeconfig, err := schedulerstep.NewCreateSchedulerKubeconfigStepBuilder(runtimeCtx, "CreateSchedulerKubeconfig").Build()
	if err != nil {
		return nil, err
	}
