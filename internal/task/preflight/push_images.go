...
func (t *PushImagesToRegistryTask) Plan(ctx runtime.TaskContext) (*plan.ExecutionFragment, error) {
	...
	pushImages, err := imagesstep.NewPushImagesStepBuilder(runtimeCtx, "PushAllSavedImages").Build()
	if err != nil {
		return nil, err
	}
	pushManifests, err := imagesstep.NewPushManifestListStepBuilder(runtimeCtx, "PushMultiArchManifests").Build()
	if err != nil {
		return nil, err
	}
