...
- 不允许使用匿名嵌套结构体，应通过定义具名结构体并进行组合的方式来替代。
...
- 不删除或修改已有字段。
- 为所有新字段选择正确的Go类型。
- **【关键】** 不允许使用匿名嵌套结构体，应通过定义具名结构体并进行组合的方式来替代。
- 添加正确的yaml和json标签。
...
**请注意**: 为了满足“不允许使用匿名嵌套结构体，应通过定义具名结构体并进行组合的方式来替代。”的要求，设计会变得不那么符合Go的惯例（通常会用嵌套struct来组织配置），但完全符合您的指令。
...
- **扁平化处理**: 遵照“不允许使用匿名嵌套结构体，应通过定义具名结构体并进行组合的方式来替代。”的指令。
